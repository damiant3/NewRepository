...
# The 16 files from drop-15 (indices 15..30)
BIG=(
...
test_add() {
    local EXTRA="$1"
    local COMBINED=/tmp/d15-$$
    > "$COMBINED"
    for f in "${BIG[@]}"; do cat "$REPO/$f" >> "$COMBINED"; printf '\n' >> "$COMBINED"; done
    if [ -n "$EXTRA" ]; then cat "$REPO/$EXTRA" >> "$COMBINED"; printf '\n' >> "$COMBINED"; fi
    local SZ=$(wc -c < "$COMBINED")
    local PIPE=/tmp/d15p-$$; local RAW=/tmp/d15r-$$
    ...
    if grep -qa 'SIZE:' "$RAW"; then echo "OK     ${SZ}b  +${EXTRA:-baseline}"
    else echo "CRASH  ${SZ}b  +${EXTRA:-baseline}"
    fi
    rm -f "$RAW"
}

test_add ""
test_add "Syntax/Token.codex"
