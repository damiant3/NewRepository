...
COMBINED=/tmp/inc-combined-$$
> "$COMBINED"
COUNT=0

for f in "${FILES[@]}"; do
    FULL="$REPO/Codex.Codex/$f"
    [ -f "$FULL" ] || continue
    cat "$FULL" >> "$COMBINED"
    printf '\n' >> "$COMBINED"
    COUNT=$((COUNT + 1))
    ...
    PIPE=/tmp/inc-pipe-$$; RAW=/tmp/inc-raw-$$
    ...
    if grep -qa 'SIZE:' "$RAW"; then
        echo "OK   ${COUNT} files  ${TOTAL}b  +$f"
    else
        echo "CRASH  ${COUNT} files  ${TOTAL}b  +$f"
        rm -f "$RAW" "$COMBINED"; exit 0
    fi
    rm -f "$RAW"
done
rm -f "$COMBINED"
