...
test_src() {
    ...
    if grep -qa 'SIZE:' "$RAW"; then echo "OK     $LABEL"
    else echo "CRASH  $LABEL"
    fi
    rm -f "$RAW"
}

echo "=== Section/prose hypothesis ==="

test_src "with-sections" 'Chapter: Tiny

Section: Types

  Foo = record { x : Integer }
...
test_src "no-sections-prose" 'Chapter: Tiny
...
test_src "no-sections-no-prose" 'Chapter: Tiny
...
test_src "empty-chapter" 'Chapter: Tiny
