...
Console.WriteLine(Codex__tco_test.main());

public sealed record State(List<long> subs, long next_id);

public abstract record Ty;

public sealed record IntT : Ty;
public sealed record VarT(long Field0) : Ty;
...
public static class Codex__tco_test
{
    public static bool has_sub(long id, List<long> subs)
    {
        return ((((long)subs.Count) == 0L) ? false : ((subs[(int)0L] == id) ? true : false));
    }

    public static Ty do_lookup(long id, List<long> subs)
    {
        return ((((long)subs.Count) == 0L) ? new VarT(id) : new IntT());
    }

    public static Ty resolve(State st, Ty ty)
    {
        while (true)
        {
            var _tco_s = ty;
            if (_tco_s is VarT _tco_m0)
            {
                ...
                var j = (i + 10L);
                var k = (j + 11L);
                if (has_sub(id, st.subs))
                {
                    var _tco_0 = st;
                    var _tco_1 = do_lookup(id, st.subs);
                    st = _tco_0;
                    ty = _tco_1;
                    continue;
                }
    ...
    {
        return ((Func<State, long>)((st) => ((Func<Ty, long>)((r) => (r is IntT _mIntT0_ ? 1L : ((Func<Ty, long>)((_) => 0L))(r))))(resolve(st, new VarT(42L)))))(new State(new List<long>() { 42L }, 10L));
    }
