...
public static class Codex_peek
{
    ...
    {
        ((Func<object>)(() => {
                ((Func<List<string>, object>)((args) => ((Func<long, object>)((argc) => ((argc < 2L) ? Console.WriteLine("Usage: codex-peek <file> [start] [end]") : ((Func<string, object>)((file) => ((Func<long, object>)((start) => ((Func<long, object>)((end_line) => (File.Exists(file) ? Console.WriteLine(peek_output(file, File.ReadAllText(file), start, end_line)) : Console.WriteLine(string.Concat("File not found: ", file)))))(((argc > 3L) ? parse_int_or(args[(int)3L], 0L) : 0L))))(((argc > 2L) ? parse_int_or(args[(int)2L], 1L) : 1L))))(args[(int)1L]))))(((long)args.Count))))(new List<string>(Environment.GetCommandLineArgs()));
                return null;
            }))();
        return null;
    }
