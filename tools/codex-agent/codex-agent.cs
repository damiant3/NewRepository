...
public static class Codex_codex_agent
{
    ...
    {
        ((Func<object>)(() => {
                ((Func<List<string>, object>)((args) => ((Func<long, object>)((argc) => ((argc < 2L) ? print_help() : ((Func<string, object>)((cmd) => ((cmd == "peek") ? do_peek(args) : ((cmd == "stat") ? do_stat(args) : ((cmd == "snap") ? do_snap(args) : ((cmd == "status") ? do_status(args) : ((cmd == "plan") ? do_plan(args) : ((cmd == "check") ? do_check(args) : ((cmd == "doctor") ? do_doctor(args) : ((cmd == "handoff") ? do_handoff(args) : ((cmd == "log") ? do_log(args) : ((cmd == "recall") ? do_recall(args) : ((cmd == "build") ? do_build(args) : ((cmd == "test") ? do_test(args) : ((cmd == "help") ? print_help() : Console.WriteLine(string.Concat("Unknown command: ", string.Concat(cmd, ". Run 'codex-agent help' for usage."))))))))))))))))))(args[(int)1L]))))(((long)args.Count))))(new List<string>(Environment.GetCommandLineArgs()));
                return null;
            }))();
        return null;
    }
