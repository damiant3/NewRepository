...
| # | Item | Notes |
|---|------|-------|
| C1 | **Same-name chapter collision is silent (loader fix still pending)** | Two `.codex` files that both declare the same `Chapter:` name both contribute definitions to that chapter slug. `ChapterScoper`'s collision detection (`find-collisions-loop` + `appears-in-different-chapter` in `Codex.Codex/Semantics/ChapterScoper.codex:26-42`) compares **chapter slugs**, so duplicate defs from same-named chapters are *not* flagged and *not* renamed — they pass through with the same name and one silently shadows the other (loader order wins via `CompositeChapterLoader`'s first-match). The original on-disk footgun (`Codex.Codex/Core/Hamt.codex` and `foreword/Hamt.codex` both `Chapter: Hamt`, with incompatible signatures for `HamtMap`/`hamt-set`/`hamt-get`) is resolved by renaming the self-host's flat-probe-table chapter to `OffsetTable` in this branch — but the underlying detection bug remains: any future same-named chapter pair will silently shadow. **Fix direction:** detect at chapter-load time when two distinct sources both want the same chapter slug — error (or require an explicit alias) rather than letting them coexist. |
| C2 | **Silent corruption: `let X = effectfulCall` outside do-bind** *(FIXED, merged via CDX2033)* | Binding the result of an effectful function (return type `[E] T`) with `let X = call` instead of `X <- call` (do-bind) compiled without error and silently bound the wrapper. Fixed by typechecker hard-error CDX2033 in both ref + self-host. |
| C5 | **Bare-metal codegen hangs when `list-length (rec.field)` is an inline arg to another call** | Hit 2026-04-14 while landing CDX-C4 partial fix. Writing `list-with-capacity (list-length (st2.call-patches))` inline as the 4th arg of `collect-call-patches (...) (...) 0 (list-with-capacity (list-length (st2.call-patches)))` caused `hello.codex` to hang at `PH:finalize` on bare metal. Let-binding the length first works. The hang manifests as an infinite #GP fault loop at RIP inside `build-offset-table-loop` reading `fo.name` from a corrupt list. **Exception dispatch fixed 2026-04-14 (commit 6d4c0c1)** so the fault now prints `!EXC=0d RIP=<hex>...` instead of silently looping. **Root cause traced, not yet fixed:** the `let ha = heap-advance N` binding in `x86-64-init-codegen-streaming` gets wrapped in a scalar-reclaim region (because heap-advance returns Integer), and the region resets HeapReg back to the pre-advance mark — undoing the side effect. This makes `text-buf-addr == rodata-buf-addr == initial-heap`, and list allocations from subsequent `list-with-capacity` calls inside the same CodegenState record allocate into the "text buffer" range. Text writes during emit-defs then overwrite the list metadata. For simple samples the overwrite happens to miss the length field of `func-offsets`; for samples with slightly more state it hits it, and `list-length (st2.func-offsets)` returns garbage — causing the billion-iter / non-canonical-pointer hang. Attempted fix (skip region wrap for `let X = heap-advance N`) in Lowering.cs broke the self-host because `main.codex` contains `let hr = heap-restore watermark` after each def emission and relies on the reclaim being a no-op (otherwise the post-restore CodegenState points into reclaimed heap). So the "fix" would also require restructuring emit-defs to not retain pointers across the heap-restore. Parking here — proper fix needs coordinated changes in main.codex + Lowering.cs + probably escape-copy semantics on bare-metal. |
