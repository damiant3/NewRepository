...
| # | Item | Notes |
|---|------|-------|
| C1 | **Same-name chapter collision is silent (loader fix still pending)** | Two `.codex` files that both declare the same `Chapter:` name both contribute definitions to that chapter slug. `ChapterScoper`'s collision detection (`find-collisions-loop` + `appears-in-different-chapter` in `Codex.Codex/Semantics/ChapterScoper.codex:26-42`) compares **chapter slugs**, so duplicate defs from same-named chapters are *not* flagged and *not* renamed — they pass through with the same name and one silently shadows the other (loader order wins via `CompositeChapterLoader`'s first-match). **Fix direction:** detect at chapter-load time when two distinct sources both want the same chapter slug — error (or require an explicit alias) rather than letting them coexist. Covered by the quire model design at `docs/Designs/Language/QUIRES.md`. |
| C6 | **Bare-metal self-host: first 5 bytes of emitted binary are corrupted** | Hit 2026-04-14 after closing CDX-C5. Self-host kernel (ref-built, booted in QEMU) accepts hello.codex over serial, reports `SIZE:4218032`, streams 4.2MB of bytes — everything except the first 5 bytes. Observed start-of-stream: `07 07 0c 2d 0b 01 01 00 ...`. Expected: `7f 45 4c 46 01 01 01 00 ...` (ELF magic + EI_CLASS). Bytes 5+ match the reference-built bare-metal hello.elf exactly. **Verified**: ref compiler's x86-64 emission for `elf-magic = [127, 69, 76, 70]` produces correct machine code (`mov rax, 0x7f` etc. confirmed via objdump at virt ~0x12CAD5 in the self-host ELF). So the emitted code for elf-magic is right; something corrupts the first 5 elements of `result.bytes` between construction (via `elf32-header-bytes` → `elf-ident-32` → `elf-magic ++ [...] ++ pad-zeros 9`) and `write-binary` streaming it. Very specifically 5 bytes: 4 from elf-magic + first element of `[elf-class-32, elf-data-lsb, elf-version-current]`. Most likely candidates: (a) a subtle off-by-N in `__list_concat_many` phase 3 affecting exactly the first N destination slots, (b) something writing over the first 5 qwords of the elf-buf after `buf-write-bytes elf-buf 0 hdr`, (c) `heap-save` / initial heap pointer landing 40 bytes (5 qwords) inside a previously-written region. **Probe ideas**: (1) make `elf32-header-bytes` return a static test pattern of sentinel bytes and see exactly which positions drift; (2) instrument `bin-finalize` with `print-line` before/after `buf-write-bytes elf-buf 0 hdr` dumping hdr's first-5 elements as hex to isolate whether hdr is wrong or the buffer gets clobbered after; (3) dump `elf-buf[0..8]` via serial right before `buf-read-bytes elf-buf 0 file-size` call — tells us whether the buffer holds correct bytes at that point. Not blocking — self-host runs, streams, and survives; just produces a non-booting ELF. |
