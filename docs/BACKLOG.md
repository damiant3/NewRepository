...
| C5 | **Bare-metal codegen hangs when `list-length (rec.field)` is an inline arg to another call** | Hit 2026-04-14 while landing CDX-C4 partial fix. Writing `list-with-capacity (list-length (st2.call-patches))` inline as the 4th arg of `collect-call-patches (...) (...) 0 (list-with-capacity (list-length (st2.call-patches)))` caused `hello.codex` to hang at `PH:finalize` on bare metal. Replacing with a `let n-call = list-length (st2.call-patches) in ... list-with-capacity n-call` works in ~8s. Likely register clobber in nested-call argument evaluation — the inner call-args of `list-with-capacity` (a builtin) collide with the outer call-args of `collect-call-patches` (a user function). Affects only `target=x86-64-bare`; ref (C#) path was not exercised. Minimal repro: rebuild bare-metal with `(list-length (st2.call-patches))` inline and run any `samples/*.codex` via `tools/binary-diag-sample.sh` — hangs at PH:finalize. |
