...
| 4 | Reference compiler: `if ... then do ... else` parse failure | The C# reference compiler rejects `if X then do { multi-line } else Y` — reports `Expected an expression, found ElseKeyword` at the `else` keyword. The `let P = do ... in Y` form also fails in the same way (the `in` gets parsed as part of the `do` body and the outer binder is lost — see `/* error: in */` markers in ref-compiler output on such inputs). The self-hosted compiler parses both correctly (verified via bootstrap fixed-point). Workaround: extract multi-line `then` / `else` / `let-rhs` `do`-bodies into named helper functions. Normally not blocking since the workaround is cheap — **however**, this bug now blocks testing T2 (`emit-do` typed-value behavior) because every interesting trigger pattern for T2 goes through one of these ref-parser-rejected constructs, so we can't send a repro through `Codex.Bootstrap` (which uses the ref compiler as its front end). Fixing #4 unblocks T2 verification. Fix lives in the C# parser's `if` / `let` expression parse routines (src/Codex.Syntax) and requires admitting multi-line `do` blocks as legal RHS for both — the grammar and the self-hosted parser already handle it. |
