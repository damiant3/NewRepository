...
| # | Item | Notes |
|---|------|-------|
| C2 | **Silent corruption: `let X = effectfulCall` outside do-bind** | Binding the result of an effectful function (return type `[E] T`) with `let X = call` instead of `X <- call` (do-bind) compiles without error or warning, but the resulting `X` is **not** the unwrapped `T` — it's the unevaluated effect/wrapper. Subsequent reads of `X` as if it were `T` get garbage memory. **Bisect evidence:** introduced as a regression in `67a7a61` (emitdefs); commit `e014553` (immediately before) compiled hello.codex on bare-metal cleanly; `67a7a61` hangs at PH:finalize because the inlined `emit-binary-diag` does `let cg = emit-defs-diag-loop ...` (where the call returns `[Console] CodegenState`) instead of `cg <- emit-defs-diag-loop ...`. `cg` is then garbage; `bin-finalize cg` reads bogus `func-offsets`; `build-offset-table` iterates corrupt entries; `djb2-hash` hits the dangling-pointer infinite-loop pattern. Workaround `ae69afb` switched the call to the production `emit-defs-binary-gated` (non-effectful), avoiding the binding mismatch entirely — but the underlying compiler bug remains and will silently bite anyone who writes `let X = effectfulCall`. **Fix direction:** typechecker should reject `let X = e` when `e : [Effect] T` outside a do-bind context (cleanest), or the lowering pass should auto-desugar to do-bind. Hard-error preferred — silent acceptance is the worst kind of bug. After the fix, audit existing `let X = ...` sites for any others that were silently relying on this. |
