...
{
    public static string show_color(Color c) => c switch { Red { } => "red", Green { } => "green", Blue(var n) => "blue",  };

    public static long get_x(Point p) => p.x;

    public static Point add_points(Point a, Point b) => new Point(x: (a.x + b.x), y: (a.y + b.y));

    public static List<T18> map_list<T8, T18>(Func<T8, T18> f, List<T8> xs) => map_list_loop(f, xs, 0, list_length(xs), new List<T31>());

    public static List<T31> map_list_loop<T30, T31>(Func<T30, T31> f, List<T30> xs, long i, long len, List<T31> acc) => ((i == len) ? acc : map_list_loop(f, xs, (i + 1), len, (acc + new List<T31> { f(list_at(xs)(i)) })));
