...
Console.WriteLine(Codex_tco_sumtype_test.main());

public abstract record Tree;

public sealed record Leaf(long Field0) : Tree;
public sealed record Branch(Tree Field0, long Field1, Tree Field2) : Tree;
...
public static class Codex_tco_sumtype_test
{
    public static List<long> collect_right(Tree t, List<long> acc)
    {
        while (true)
        {
            var _tco_s = t;
            if (_tco_s is Branch _tco_m0)
            {
                var left = _tco_m0.Field0;
                var val = _tco_m0.Field1;
                var right = _tco_m0.Field2;
                var _tco_0 = left;
                var _tco_1 = Enumerable.Concat(new List<long>() { val }, acc).ToList();
                t = _tco_0;
                acc = _tco_1;
                continue;
            }
            else if (_tco_s is Leaf _tco_m1)
            {
                var v = _tco_m1.Field0;
                return Enumerable.Concat(new List<long>() { v }, acc).ToList();
            }
        }
    }
    ...
    {
        return ((long)collect_right(new Branch(new Branch(new Leaf(1L), 2L, new Leaf(3L)), 4L, new Leaf(5L)), new List<long>()).Count);
    }
