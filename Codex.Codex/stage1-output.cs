...
public static class Codex_Codex_Codex
{
    ...
    public static T401 desugar_do_stmt<T401>(object s) => s switch { DoBindStmt(var tok, var val) => ADoBindStmt(make_name(tok.text))(desugar_expr(val)), DoExprStmt(var e) => ADoExprStmt(desugar_expr(e)),  };
    ...
    public static T426 desugar_type_expr<T426>(object t) => t switch { NamedType(var tok) => ANamedType(make_name(tok.text)), FunType(var param, var ret) => AFunType(desugar_type_expr(param))(desugar_type_expr(ret)), AppType(var ctor, var args) => AAppType(desugar_type_expr(ctor))(map_list(desugar_type_expr)(args)), ParenType(var inner) => desugar_type_expr(inner), ListType(var elem) => AAppType(ANamedType(make_name("List")))(new List<object> { desugar_type_expr(elem) }), LinearTypeExpr(var inner) => desugar_type_expr(inner),  };
    ...
    public static T451 desugar_type_def<T451>(object td) => td.body switch { RecordBody(var fields) => ARecordTypeDef(make_name(td.name.text))(map_list(make_type_param_name)(td.type_params))(map_list(desugar_record_field_def)(fields)), VariantBody(var ctors) => AVariantTypeDef(make_name(td.name.text))(map_list(make_type_param_name)(td.type_params))(map_list(desugar_variant_ctor_def)(ctors)),  };

    public static T470 make_type_param_name<T470>(object tok) => make_name(tok.text);
    ...
    public static T487 map_list<T487>(object f, object xs) => map_list_loop(f)(xs)(0)(list_length(xs))(new List<object>());

    public static List<T500> map_list_loop<T500>(object f, object xs, object i, object len, object acc) => ((i == len) ? acc : map_list_loop(f)(xs)((i + 1))(len)((acc + new List<object> { f(list_at(xs)(i)) })));

    public static T511 fold_list<T511>(object f, object z, object xs) => fold_list_loop(f)(z)(xs)(0)(list_length(xs));

    public static T526 fold_list_loop<T526>(object f, object z, object xs, object i, object len) => ((i == len) ? z : fold_list_loop(f)(f(z)(list_at(xs)(i)))(xs)((i + 1))(len));
    ...
    public static T832 emit_type_expr<T832>(object te) => emit_type_expr_tp(te)(new List<object>());
    ...
    public static T890 collect_type_var_ids<T890>(object ty, object acc) => ty switch { TypeVar(var id) => (list_contains_int(acc)(id) ? acc : list_append_int(acc)(id)), FunTy(var p, var r) => collect_type_var_ids(r)(collect_type_var_ids(p)(acc)), ListTy(var elem) => collect_type_var_ids(elem)(acc), ForAllTy(var id, var body) => collect_type_var_ids(body)(acc), ConstructedTy(var name, var args) => collect_type_var_ids_list(args)(acc), _ => acc,  };

    public static T912 collect_type_var_ids_list<T912>(object types, object acc) => collect_type_var_ids_list_loop(types)(acc)(0)(list_length(types));

    public static T925 collect_type_var_ids_list_loop<T925>(object types, object acc, object i, object len) => ((i == len) ? acc : collect_type_var_ids_list_loop(types)(collect_type_var_ids(list_at(types)(i))(acc))((i + 1))(len));

    public static T933 list_contains_int<T933>(object xs, object n) => list_contains_int_loop(xs)(n)(0)(list_length(xs));
    ...
    public static List<T947> list_append_int<T947>(object xs, object n) => (xs + new List<object> { n });
    ...
    public static T985 get_return_type<T985>(object ty, object n) => ((n == 0) ? strip_forall(ty) : strip_forall(ty) switch { FunTy(var p, var r) => get_return_type(r)((n - 1)), _ => ty,  });

    public static T990 strip_forall<T990>(object ty) => ty switch { ForAllTy(var id, var body) => strip_forall(body), _ => ty,  };
    ...
    public static T1035 lookup_type<T1035>(object bindings, object name) => lookup_type_loop(bindings)(name)(0)(list_length(bindings));
    ...
    public static T1057 strip_forall_ty<T1057>(object ty) => ty switch { ForAllTy(var id, var body) => strip_forall_ty(body), _ => ty,  };
    ...
    public static T1066 lower_expr<T1066>(object e, object ty) => e switch { ALitExpr(var text, var kind) => lower_literal(text)(kind), ANameExpr(var name) => IrName(name.value)(ty), AApplyExpr(var f, var a) => lower_apply(f)(a)(ty), ABinaryExpr(var l, var op, var r) => IrBinary(lower_bin_op(op)(ty))(lower_expr(l)(ty))(lower_expr(r)(ty))(ty), AUnaryExpr(var operand) => IrNegate(lower_expr(operand)(IntegerTy)), AIfExpr(var c, var t, var e2) => IrIf(lower_expr(c)(BooleanTy))(lower_expr(t)(ty))(lower_expr(e2)(ty))(ty), ALetExpr(var binds, var body) => lower_let(binds)(body)(ty), ALambdaExpr(var @params, var body) => lower_lambda(@params)(body)(ty), AMatchExpr(var scrut, var arms) => lower_match(scrut)(arms)(ty), AListExpr(var elems) => lower_list(elems)(ty), ARecordExpr(var name, var fields) => lower_record(name)(fields)(ty), AFieldAccess(var rec, var field) => IrFieldAccess(lower_expr(rec)(ty))(field.value)(ty), ADoExpr(var stmts) => lower_do(stmts)(ty), AErrorExpr(var msg) => IrError(msg)(ty),  };

    public static T1123 lower_literal<T1123>(object text, object kind) => kind switch { IntLit { } => IrIntLit(text_to_integer(text)), NumLit { } => IrIntLit(text_to_integer(text)), TextLit { } => IrTextLit(text), BoolLit { } => IrBoolLit((text == "True")),  };

    public static T1140 lower_apply<T1140>(object f, object a, object ty) => IrApply(lower_expr(f)(ty))(lower_expr(a)(ty))(ty);

    public static T1159 lower_let<T1159>(object binds, object body, object ty) => ((list_length(binds) == 0) ? lower_expr(body)(ty) : ((object b = list_at(binds)(0)) is var _ ? IrLet(b.name.value)(ty)(lower_expr(b.value)(ErrorTy))(lower_let_rest(binds)(body)(ty)(1)) : default));

    public static T1179 lower_let_rest<T1179>(object binds, object body, object ty, object i) => ((i == list_length(binds)) ? lower_expr(body)(ty) : ((object b = list_at(binds)(i)) is var _ ? IrLet(b.name.value)(ty)(lower_expr(b.value)(ErrorTy))(lower_let_rest(binds)(body)(ty)((i + 1))) : default));

    public static T1195 lower_lambda<T1195>(object @params, object body, object ty) => ((object stripped = strip_forall_ty(ty)) is var _ ? IrLambda(lower_lambda_params(@params)(stripped)(0))(lower_expr(body)(get_lambda_return(stripped)(list_length(@params))))(ty) : default);
    ...
    public static T1226 lower_match<T1226>(object scrut, object arms, object ty) => IrMatch(lower_expr(scrut)(ty))(map_list(lower_arm(ty))(arms))(ty);
    ...
    public static T1250 lower_list<T1250>(object elems, object ty) => ((object elem_ty = ty switch { ListTy(var e) => e, _ => ErrorTy,  }) is var _ ? IrList(map_list(lower_elem(elem_ty))(elems))(elem_ty) : default);

    public static T1255 lower_elem<T1255>(object ty, object e) => lower_expr(e)(ty);

    public static T1265 lower_record<T1265>(object name, object fields, object ty) => IrRecord(name.value)(map_list(lower_field_val(ty))(fields))(ty);

    public static object lower_field_val(object ty, object f) => new IRFieldVal(name: f.name.value, value: lower_expr(f.value)(ty));

    public static T1276 lower_do<T1276>(object stmts, object ty) => IrDo(map_list(lower_do_stmt(ty))(stmts))(ty);

    public static T1280 lower_do_stmt<T1280>(object ty, object s) => s switch { ADoBindStmt(var name, var val) => IrDoBind(name.value)(ty)(lower_expr(val)(ty)), ADoExprStmt(var e) => IrDoExec(lower_expr(e)(ty)),  };
    ...
    public static List<T1338> lower_defs<T1338>(object defs, object types, object ust, object i) => ((i == list_length(defs)) ? new List<object>() : (new List<object> { lower_def(list_at(defs)(i))(types)(ust) } + lower_defs(defs)(types)(ust)((i + 1))));

    public static object empty_scope() => new Scope(names: new List<object>());

    public static T1351 scope_has<T1351>(object sc, object name) => scope_has_loop(sc.names)(name)(0)(list_length(sc.names));
    ...
    public static T1408 list_contains<T1408>(object xs, object name) => list_contains_loop(xs)(name)(0)(list_length(xs));
    ...
    public static T1473 build_all_names_scope<T1473>(object top_names, object ctor_names, object builtins) => ((object sc = add_names_to_scope(empty_scope)(top_names)(0)(list_length(top_names))) is var _ ? ((object sc2 = add_names_to_scope(sc)(ctor_names)(0)(list_length(ctor_names))) is var _ ? add_names_to_scope(sc2)(builtins)(0)(list_length(builtins)) : default) : default);

    public static T1486 add_names_to_scope<T1486>(object sc, object names, object i, object len) => ((i == len) ? sc : add_names_to_scope(scope_add(sc)(list_at(names)(i)))(names)((i + 1))(len));

    public static List<T1491> resolve_expr<T1491>(object sc, object expr) => expr switch { ALitExpr(var val, var kind) => new List<object>(), ANameExpr(var name) => ((scope_has(sc)(name.value) || is_type_name(name.value)) ? new List<object>() : new List<object> { make_error("CDX3002")(("Undefined name: " + name.value)) }), ABinaryExpr(var left, var op, var right) => (resolve_expr(sc)(left) + resolve_expr(sc)(right)), AUnaryExpr(var operand) => resolve_expr(sc)(operand), AApplyExpr(var func, var arg) => (resolve_expr(sc)(func) + resolve_expr(sc)(arg)), AIfExpr(var cond, var then_e, var else_e) => ((resolve_expr(sc)(cond) + resolve_expr(sc)(then_e)) + resolve_expr(sc)(else_e)), ALetExpr(var bindings, var body) => resolve_let(sc)(bindings)(body)(0)(list_length(bindings))(new List<object>()), ALambdaExpr(var @params, var body) => ((object sc2 = add_lambda_params(sc)(@params)(0)(list_length(@params))) is var _ ? resolve_expr(sc2)(body) : default), AMatchExpr(var scrutinee, var arms) => (resolve_expr(sc)(scrutinee) + resolve_match_arms(sc)(arms)(0)(list_length(arms))(new List<object>())), AListExpr(var elems) => resolve_list_elems(sc)(elems)(0)(list_length(elems))(new List<object>()), ARecordExpr(var name, var fields) => resolve_record_fields(sc)(fields)(0)(list_length(fields))(new List<object>()), AFieldAccess(var obj, var field) => resolve_expr(sc)(obj), ADoExpr(var stmts) => resolve_do_stmts(sc)(stmts)(0)(list_length(stmts))(new List<object>()), AErrorExpr(var msg) => new List<object>(),  };

    public static T1582 resolve_let<T1582>(object sc, object bindings, object body, object i, object len, object errs) => ((i == len) ? (errs + resolve_expr(sc)(body)) : ((object b = list_at(bindings)(i)) is var _ ? ((object bind_errs = resolve_expr(sc)(b.value)) is var _ ? ((object sc2 = scope_add(sc)(b.name.value)) is var _ ? resolve_let(sc2)(bindings)(body)((i + 1))(len)((errs + bind_errs)) : default) : default) : default));

    public static T1595 add_lambda_params<T1595>(object sc, object @params, object i, object len) => ((i == len) ? sc : ((object p = list_at(@params)(i)) is var _ ? add_lambda_params(scope_add(sc)(p.value))(@params)((i + 1))(len) : default));

    public static T1612 resolve_match_arms<T1612>(object sc, object arms, object i, object len, object errs) => ((i == len) ? errs : ((object arm = list_at(arms)(i)) is var _ ? ((object sc2 = collect_pattern_names(sc)(arm.pattern)) is var _ ? ((object arm_errs = resolve_expr(sc2)(arm.body)) is var _ ? resolve_match_arms(sc)(arms)((i + 1))(len)((errs + arm_errs)) : default) : default) : default));

    public static T1616 collect_pattern_names<T1616>(object sc, object pat) => pat switch { AVarPat(var name) => scope_add(sc)(name.value), ACtorPat(var name, var subs) => collect_ctor_pat_names(sc)(subs)(0)(list_length(subs)), ALitPat(var val, var kind) => sc, AWildPat { } => sc,  };

    public static T1636 collect_ctor_pat_names<T1636>(object sc, object subs, object i, object len) => ((i == len) ? sc : ((object sub = list_at(subs)(i)) is var _ ? collect_ctor_pat_names(collect_pattern_names(sc)(sub))(subs)((i + 1))(len) : default));

    public static T1651 resolve_list_elems<T1651>(object sc, object elems, object i, object len, object errs) => ((i == len) ? errs : ((object errs2 = resolve_expr(sc)(list_at(elems)(i))) is var _ ? resolve_list_elems(sc)(elems)((i + 1))(len)((errs + errs2)) : default));

    public static T1666 resolve_record_fields<T1666>(object sc, object fields, object i, object len, object errs) => ((i == len) ? errs : ((object f = list_at(fields)(i)) is var _ ? ((object errs2 = resolve_expr(sc)(f.value)) is var _ ? resolve_record_fields(sc)(fields)((i + 1))(len)((errs + errs2)) : default) : default));

    public static T1675 resolve_do_stmts<T1675>(object sc, object stmts, object i, object len, object errs) => ((i == len) ? errs : ((object stmt = list_at(stmts)(i)) is var _ ? stmt switch { ADoExprStmt(var e) => ((object errs2 = resolve_expr(sc)(e)) is var _ ? resolve_do_stmts(sc)(stmts)((i + 1))(len)((errs + errs2)) : default), ADoBindStmt(var name, var e) => ((object errs2 = resolve_expr(sc)(e)) is var _ ? ((object sc2 = scope_add(sc)(name.value)) is var _ ? resolve_do_stmts(sc2)(stmts)((i + 1))(len)((errs + errs2)) : default) : default),  } : default));

    public static T1711 resolve_all_defs<T1711>(object sc, object defs, object i, object len, object errs) => ((i == len) ? errs : ((object def = list_at(defs)(i)) is var _ ? ((object def_scope = add_def_params(sc)(def.@params)(0)(list_length(def.@params))) is var _ ? ((object errs2 = resolve_expr(def_scope)(def.body)) is var _ ? resolve_all_defs(sc)(defs)((i + 1))(len)((errs + errs2)) : default) : default) : default));

    public static T1724 add_def_params<T1724>(object sc, object @params, object i, object len) => ((i == len) ? sc : ((object p = list_at(@params)(i)) is var _ ? add_def_params(scope_add(sc)(p.name.value))(@params)((i + 1))(len) : default));
    ...
    public static T1767 skip_spaces<T1767>(object st) => (is_at_end(st) ? st : ((peek_char(st) == " ") ? skip_spaces(advance_char(st)) : st));

    public static T1773 scan_ident_rest<T1773>(object st) => (is_at_end(st) ? st : ((object ch = peek_char(st)) is var _ ? (is_letter(ch) ? scan_ident_rest(advance_char(st)) : (is_digit(ch) ? scan_ident_rest(advance_char(st)) : ((ch == "_") ? scan_ident_rest(advance_char(st)) : ((ch == "-") ? ((object next = advance_char(st)) is var _ ? (is_at_end(next) ? st : (is_letter(peek_char(next)) ? scan_ident_rest(next) : st)) : default) : st)))) : default));

    public static T1790 scan_digits<T1790>(object st) => (is_at_end(st) ? st : ((object ch = peek_char(st)) is var _ ? (is_digit(ch) ? scan_digits(advance_char(st)) : ((ch == "_") ? scan_digits(advance_char(st)) : st)) : default));

    public static T1807 scan_string_body<T1807>(object st) => (is_at_end(st) ? st : ((object ch = peek_char(st)) is var _ ? ((ch == "\\\"") ? advance_char(st) : ((ch == "\\n") ? st : ((ch == "\\\\") ? scan_string_body(advance_char(advance_char(st))) : scan_string_body(advance_char(st))))) : default));
    ...
    public static T1823 extract_text<T1823>(object st, object start, object end_st) => substring(st.source)(start)((end_st.offset - start));
    ...
    public static T1960 scan_operator<T1960>(object s) => ((object ch = peek_char(s)) is var _ ? ((object next = advance_char(s)) is var _ ? ((ch == "(") ? LexToken(make_token(LeftParen)("(")(s))(next) : ((ch == ")") ? LexToken(make_token(RightParen)(")")(s))(next) : ((ch == "[") ? LexToken(make_token(LeftBracket)("[")(s))(next) : ((ch == "]") ? LexToken(make_token(RightBracket)("]")(s))(next) : ((ch == "{") ? LexToken(make_token(LeftBrace)("{")(s))(next) : ((ch == "}") ? LexToken(make_token(RightBrace)("}")(s))(next) : ((ch == ",") ? LexToken(make_token(Comma)(",")(s))(next) : ((ch == ".") ? LexToken(make_token(Dot)(".")(s))(next) : ((ch == "^") ? LexToken(make_token(Caret)("^")(s))(next) : ((ch == "&") ? LexToken(make_token(Ampersand)("&")(s))(next) : scan_multi_char_operator(s))))))))))) : default) : default);

    public static T2080 scan_multi_char_operator<T2080>(object s) => ((object ch = peek_char(s)) is var _ ? ((object next = advance_char(s)) is var _ ? ((object next_ch = (is_at_end(next) ? "" : peek_char(next))) is var _ ? ((ch == "+") ? ((next_ch == "+") ? LexToken(make_token(PlusPlus)("++")(s))(advance_char(next)) : LexToken(make_token(Plus)("+")(s))(next)) : ((ch == "-") ? ((next_ch == ">") ? LexToken(make_token(Arrow)("->")(s))(advance_char(next)) : LexToken(make_token(Minus)("-")(s))(next)) : ((ch == "*") ? LexToken(make_token(Star)("*")(s))(next) : ((ch == "/") ? ((next_ch == "=") ? LexToken(make_token(NotEquals)("/=")(s))(advance_char(next)) : LexToken(make_token(Slash)("/")(s))(next)) : ((ch == "=") ? ((next_ch == "=") ? ((object next2 = advance_char(next)) is var _ ? ((object next2_ch = (is_at_end(next2) ? "" : peek_char(next2))) is var _ ? ((next2_ch == "=") ? LexToken(make_token(TripleEquals)("===")(s))(advance_char(next2)) : LexToken(make_token(DoubleEquals)("==")(s))(next2)) : default) : default) : LexToken(make_token(Equals)("=")(s))(next)) : ((ch == ":") ? ((next_ch == ":") ? LexToken(make_token(ColonColon)("::")(s))(advance_char(next)) : LexToken(make_token(Colon)(":")(s))(next)) : ((ch == "|") ? ((next_ch == "-") ? LexToken(make_token(Turnstile)("|-")(s))(advance_char(next)) : LexToken(make_token(Pipe)("|")(s))(next)) : ((ch == "<") ? ((next_ch == "=") ? LexToken(make_token(LessOrEqual)("<=")(s))(advance_char(next)) : ((next_ch == "-") ? LexToken(make_token(LeftArrow)("<-")(s))(advance_char(next)) : LexToken(make_token(LessThan)("<")(s))(next))) : ((ch == ">") ? ((next_ch == "=") ? LexToken(make_token(GreaterOrEqual)(">=")(s))(advance_char(next)) : LexToken(make_token(GreaterThan)(">")(s))(next)) : LexToken(make_token(ErrorToken)(char_at(s.source)(s.offset))(s))(next)))))))))) : default) : default) : default);
    ...
    public static T2096 tokenize<T2096>(object src) => tokenize_loop(make_lex_state(src))(new List<object>());

    public static object make_parse_state(object toks) => new ParseState(tokens: toks, pos: 0);

    public static T2102 current<T2102>(object st) => list_at(st.tokens)(st.pos);
    ...
    public static T2205 expect<T2205>(object kind, object st) => (is_done(st) ? st : advance(st));

    public static T2210 skip_newlines<T2210>(object st) => (is_done(st) ? st : current_kind(st) switch { Newline { } => skip_newlines(advance(st)), Indent { } => skip_newlines(advance(st)), Dedent { } => skip_newlines(advance(st)), _ => st,  });

    public static T2221 parse_type<T2221>(object st) => ((object result = parse_type_atom(st)) is var _ ? unwrap_type_ok(result)(parse_type_continue) : default);

    public static T2233 parse_type_continue<T2233>(object left, object st) => (is_arrow(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? ((object right_result = parse_type(st2)) is var _ ? unwrap_type_ok(right_result)(make_fun_type(left)) : default) : default) : TypeOk(left)(st));

    public static T2241 make_fun_type<T2241>(object left, object right, object st) => TypeOk(FunType(left)(right))(st);

    public static T2245 unwrap_type_ok<T2245>(object r, object f) => r switch { TypeOk(var t, var st) => f(t)(st),  };

    public static T2272 parse_type_atom<T2272>(object st) => (is_ident(current_kind(st)) ? ((object tok = current(st)) is var _ ? parse_type_args(NamedType(tok))(advance(st)) : default) : (is_type_ident(current_kind(st)) ? ((object tok = current(st)) is var _ ? parse_type_args(NamedType(tok))(advance(st)) : default) : (is_left_paren(current_kind(st)) ? parse_paren_type(advance(st)) : ((object tok = current(st)) is var _ ? TypeOk(NamedType(tok))(advance(st)) : default))));

    public static T2277 parse_paren_type<T2277>(object st) => ((object inner = parse_type(st)) is var _ ? unwrap_type_ok(inner)(finish_paren_type) : default);

    public static T2285 finish_paren_type<T2285>(object t, object st) => ((object st2 = expect(RightParen)(st)) is var _ ? TypeOk(ParenType(t))(st2) : default);

    public static T2297 parse_type_args<T2297>(object base_type, object st) => (is_done(st) ? TypeOk(base_type)(st) : (is_type_arg_start(current_kind(st)) ? parse_type_arg_next(base_type)(st) : TypeOk(base_type)(st)));

    public static T2304 parse_type_arg_next<T2304>(object base_type, object st) => ((object arg_result = parse_type_atom(st)) is var _ ? unwrap_type_ok(arg_result)(continue_type_args(base_type)) : default);

    public static T2312 continue_type_args<T2312>(object base_type, object arg, object st) => parse_type_args(AppType(base_type)(new List<object> { arg }))(st);

    public static T2348 parse_pattern<T2348>(object st) => (is_underscore(current_kind(st)) ? PatOk(WildPat(current(st)))(advance(st)) : (is_literal(current_kind(st)) ? PatOk(LitPat(current(st)))(advance(st)) : (is_type_ident(current_kind(st)) ? ((object tok = current(st)) is var _ ? parse_ctor_pattern_fields(tok)(new List<object>())(advance(st)) : default) : (is_ident(current_kind(st)) ? PatOk(VarPat(current(st)))(advance(st)) : PatOk(WildPat(current(st)))(advance(st))))));

    public static T2364 parse_ctor_pattern_fields<T2364>(object ctor, object acc, object st) => (is_left_paren(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? ((object sub = parse_pattern(st2)) is var _ ? unwrap_pat_ok(sub)(continue_ctor_fields(ctor)(acc)) : default) : default) : PatOk(CtorPat(ctor)(acc))(st));

    public static T2374 continue_ctor_fields<T2374>(object ctor, object acc, object p, object st) => ((object st2 = expect(RightParen)(st)) is var _ ? parse_ctor_pattern_fields(ctor)((acc + new List<object> { p }))(st2) : default);

    public static T2378 unwrap_pat_ok<T2378>(object r, object f) => r switch { PatOk(var p, var st) => f(p)(st),  };

    public static T2384 parse_expr<T2384>(object st) => parse_binary(st)(0);

    public static T2388 unwrap_expr_ok<T2388>(object r, object f) => r switch { ExprOk(var e, var st) => f(e)(st),  };

    public static T2397 parse_binary<T2397>(object st, object min_prec) => ((object left_result = parse_unary(st)) is var _ ? unwrap_expr_ok(left_result)(start_binary_loop(min_prec)) : default);

    public static T2404 start_binary_loop<T2404>(object min_prec, object left, object st) => parse_binary_loop(left)(st)(min_prec);

    public static T2425 parse_binary_loop<T2425>(object left, object st, object min_prec) => (is_done(st) ? ExprOk(left)(st) : ((object prec = operator_precedence(current_kind(st))) is var _ ? ((prec < min_prec) ? ExprOk(left)(st) : ((object op = current(st)) is var _ ? ((object st2 = skip_newlines(advance(st))) is var _ ? ((object right_result = parse_binary(st2)((prec + 1))) is var _ ? unwrap_expr_ok(right_result)(continue_binary(left)(op)(min_prec)) : default) : default) : default)) : default));

    public static T2437 continue_binary<T2437>(object left, object op, object min_prec, object right, object st) => parse_binary_loop(BinExpr(left)(op)(right))(st)(min_prec);

    public static T2448 parse_unary<T2448>(object st) => (is_minus(current_kind(st)) ? ((object op = current(st)) is var _ ? ((object result = parse_unary(advance(st))) is var _ ? unwrap_expr_ok(result)(finish_unary(op)) : default) : default) : parse_application(st));

    public static T2456 finish_unary<T2456>(object op, object operand, object st) => ExprOk(UnaryExpr(op)(operand))(st);

    public static T2461 parse_application<T2461>(object st) => ((object func_result = parse_atom(st)) is var _ ? unwrap_expr_ok(func_result)(parse_app_loop) : default);

    public static T2478 parse_app_loop<T2478>(object func, object st) => (is_compound(func) ? parse_field_access(func)(st) : (is_done(st) ? ExprOk(func)(st) : (is_app_start(current_kind(st)) ? ((object arg_result = parse_atom(st)) is var _ ? unwrap_expr_ok(arg_result)(continue_app(func)) : default) : parse_field_access(func)(st))));

    public static T2486 continue_app<T2486>(object func, object arg, object st) => parse_app_loop(AppExpr(func)(arg))(st);

    public static T2529 parse_atom<T2529>(object st) => (is_literal(current_kind(st)) ? ExprOk(LitExpr(current(st)))(advance(st)) : (is_ident(current_kind(st)) ? parse_field_access(NameExpr(current(st)))(advance(st)) : (is_type_ident(current_kind(st)) ? parse_atom_type_ident(st) : (is_left_paren(current_kind(st)) ? parse_paren_expr(advance(st)) : (is_left_bracket(current_kind(st)) ? parse_list_expr(st) : (is_if_keyword(current_kind(st)) ? parse_if_expr(st) : (is_let_keyword(current_kind(st)) ? parse_let_expr(st) : (is_when_keyword(current_kind(st)) ? parse_match_expr(st) : (is_do_keyword(current_kind(st)) ? parse_do_expr(st) : ExprOk(ErrExpr(current(st)))(advance(st)))))))))));

    public static T2543 parse_field_access<T2543>(object node, object st) => (is_dot(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? ((object field = current(st2)) is var _ ? ((object st3 = advance(st2)) is var _ ? parse_field_access(FieldExpr(node)(field))(st3) : default) : default) : default) : ExprOk(node)(st));

    public static T2554 parse_atom_type_ident<T2554>(object st) => ((object tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? (is_left_brace(current_kind(st2)) ? parse_record_expr(tok)(st2) : ExprOk(NameExpr(tok))(st2)) : default) : default);

    public static T2560 parse_paren_expr<T2560>(object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object inner = parse_expr(st2)) is var _ ? unwrap_expr_ok(inner)(finish_paren_expr) : default) : default);

    public static T2569 finish_paren_expr<T2569>(object e, object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object st3 = expect(RightParen)(st2)) is var _ ? ExprOk(ParenExpr(e))(st3) : default) : default);

    public static T2578 parse_record_expr<T2578>(object type_name, object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? parse_record_expr_fields(type_name)(new List<object>())(st3) : default) : default);

    public static T2598 parse_record_expr_fields<T2598>(object type_name, object acc, object st) => (is_right_brace(current_kind(st)) ? ExprOk(RecordExpr(type_name)(acc))(advance(st)) : (is_ident(current_kind(st)) ? parse_record_field(type_name)(acc)(st) : ExprOk(RecordExpr(type_name)(acc))(st)));

    public static T2612 parse_record_field<T2612>(object type_name, object acc, object st) => ((object field_name = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Equals)(st2)) is var _ ? ((object val_result = parse_expr(st3)) is var _ ? unwrap_expr_ok(val_result)(finish_record_field(type_name)(acc)(field_name)) : default) : default) : default) : default);

    public static T2629 finish_record_field<T2629>(object type_name, object acc, object field_name, object v, object st) => ((object field = new RecordFieldExpr(name: field_name, value: v)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_record_expr_fields(type_name)((acc + new List<object> { field }))(skip_newlines(advance(st2))) : parse_record_expr_fields(type_name)((acc + new List<object> { field }))(st2)) : default) : default);

    public static T2636 parse_list_expr<T2636>(object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? parse_list_elements(new List<object>())(st3) : default) : default);

    public static T2649 parse_list_elements<T2649>(object acc, object st) => (is_right_bracket(current_kind(st)) ? ExprOk(ListExpr(acc))(advance(st)) : ((object elem = parse_expr(st)) is var _ ? unwrap_expr_ok(elem)(finish_list_element(acc)) : default));

    public static T2662 finish_list_element<T2662>(object acc, object e, object st) => ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_list_elements((acc + new List<object> { e }))(skip_newlines(advance(st2))) : parse_list_elements((acc + new List<object> { e }))(st2)) : default);

    public static T2669 parse_if_expr<T2669>(object st) => ((object st2 = skip_newlines(advance(st))) is var _ ? ((object cond = parse_expr(st2)) is var _ ? unwrap_expr_ok(cond)(parse_if_then) : default) : default);

    public static T2680 parse_if_then<T2680>(object c, object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object st3 = expect(ThenKeyword)(st2)) is var _ ? ((object st4 = skip_newlines(st3)) is var _ ? ((object then_result = parse_expr(st4)) is var _ ? unwrap_expr_ok(then_result)(parse_if_else(c)) : default) : default) : default) : default);

    public static T2693 parse_if_else<T2693>(object c, object t, object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object st3 = expect(ElseKeyword)(st2)) is var _ ? ((object st4 = skip_newlines(st3)) is var _ ? ((object else_result = parse_expr(st4)) is var _ ? unwrap_expr_ok(else_result)(finish_if(c)(t)) : default) : default) : default) : default);

    public static T2703 finish_if<T2703>(object c, object t, object e, object st) => ExprOk(IfExpr(c)(t)(e))(st);

    public static T2710 parse_let_expr<T2710>(object st) => ((object st2 = skip_newlines(advance(st))) is var _ ? parse_let_bindings(new List<object>())(st2) : default);

    public static T2729 parse_let_bindings<T2729>(object acc, object st) => (is_ident(current_kind(st)) ? parse_let_binding(acc)(st) : (is_in_keyword(current_kind(st)) ? ((object st2 = skip_newlines(advance(st))) is var _ ? ((object body = parse_expr(st2)) is var _ ? unwrap_expr_ok(body)(finish_let(acc)) : default) : default) : ((object body = parse_expr(st)) is var _ ? unwrap_expr_ok(body)(finish_let(acc)) : default)));

    public static T2737 finish_let<T2737>(object acc, object b, object st) => ExprOk(LetExpr(acc)(b))(st);

    public static T2749 parse_let_binding<T2749>(object acc, object st) => ((object name_tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Equals)(st2)) is var _ ? ((object val_result = parse_expr(st3)) is var _ ? unwrap_expr_ok(val_result)(finish_let_binding(acc)(name_tok)) : default) : default) : default) : default);

    public static T2763 finish_let_binding<T2763>(object acc, object name_tok, object v, object st) => ((object binding = new LetBind(name: name_tok, value: v)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_let_bindings((acc + new List<object> { binding }))(skip_newlines(advance(st2))) : parse_let_bindings((acc + new List<object> { binding }))(st2)) : default) : default);

    public static T2769 parse_match_expr<T2769>(object st) => ((object st2 = advance(st)) is var _ ? ((object scrut = parse_expr(st2)) is var _ ? unwrap_expr_ok(scrut)(start_match_branches) : default) : default);

    public static T2777 start_match_branches<T2777>(object s, object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_match_branches(s)(new List<object>())(st2) : default);

    public static T2790 parse_match_branches<T2790>(object scrut, object acc, object st) => (is_if_keyword(current_kind(st)) ? parse_one_match_branch(scrut)(acc)(st) : ExprOk(MatchExpr(scrut)(acc))(st));

    public static T2794 unwrap_pat_for_expr<T2794>(object r, object f) => r switch { PatOk(var p, var st) => f(p)(st),  };

    public static T2806 parse_one_match_branch<T2806>(object scrut, object acc, object st) => ((object st2 = advance(st)) is var _ ? ((object pat = parse_pattern(st2)) is var _ ? unwrap_pat_for_expr(pat)(parse_match_branch_body(scrut)(acc)) : default) : default);

    public static T2820 parse_match_branch_body<T2820>(object scrut, object acc, object p, object st) => ((object st2 = expect(Arrow)(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? ((object body = parse_expr(st3)) is var _ ? unwrap_expr_ok(body)(finish_match_branch(scrut)(acc)(p)) : default) : default) : default);

    public static T2830 finish_match_branch<T2830>(object scrut, object acc, object p, object b, object st) => ((object arm = new MatchArm(pattern: p, body: b)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? parse_match_branches(scrut)((acc + new List<object> { arm }))(st2) : default) : default);

    public static T2837 parse_do_expr<T2837>(object st) => ((object st2 = skip_newlines(advance(st))) is var _ ? parse_do_stmts(new List<object>())(st2) : default);

    public static T2854 parse_do_stmts<T2854>(object acc, object st) => (is_done(st) ? ExprOk(DoExpr(acc))(st) : (is_dedent(current_kind(st)) ? ExprOk(DoExpr(acc))(st) : (is_do_bind(st) ? parse_do_bind_stmt(acc)(st) : parse_do_expr_stmt(acc)(st))));

    public static bool is_do_bind(object st) => (is_ident(current_kind(st)) ? is_left_arrow(peek_kind(st)(1)) : false);

    public static T2872 parse_do_bind_stmt<T2872>(object acc, object st) => ((object name_tok = current(st)) is var _ ? ((object st2 = advance(advance(st))) is var _ ? ((object val_result = parse_expr(st2)) is var _ ? unwrap_expr_ok(val_result)(finish_do_bind(acc)(name_tok)) : default) : default) : default);

    public static T2882 finish_do_bind<T2882>(object acc, object name_tok, object v, object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_do_stmts((acc + new List<object> { DoBindStmt(name_tok)(v) }))(st2) : default);

    public static T2889 parse_do_expr_stmt<T2889>(object acc, object st) => ((object expr_result = parse_expr(st)) is var _ ? unwrap_expr_ok(expr_result)(finish_do_expr(acc)) : default);

    public static T2897 finish_do_expr<T2897>(object acc, object e, object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_do_stmts((acc + new List<object> { DoExprStmt(e) }))(st2) : default);

    public static T2903 parse_type_annotation<T2903>(object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Colon)(st2)) is var _ ? parse_type(st3) : default) : default);

    public static T2914 parse_definition<T2914>(object st) => (is_done(st) ? DefNone(st) : (is_ident(current_kind(st)) ? try_parse_def(st) : (is_type_ident(current_kind(st)) ? try_parse_def(st) : DefNone(st))));

    public static T2924 try_parse_def<T2924>(object st) => (is_colon(peek_kind(st)(1)) ? ((object ann_result = parse_type_annotation(st)) is var _ ? unwrap_type_for_def(ann_result) : default) : parse_def_body_with_ann(new List<object>())(st));

    public static T2927 unwrap_type_for_def<T2927>(object r) => r switch { TypeOk(var ann_type, var st) => ((object name_tok = new Token(kind: Identifier, text: "", offset: 0, line: 0, column: 0)) is var _ ? ((object ann = new List<object> { new TypeAnn(name: name_tok, type_expr: ann_type) }) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? parse_def_body_with_ann(ann)(st2) : default) : default) : default),  };

    public static T2940 parse_def_body_with_ann<T2940>(object ann, object st) => ((object name_tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? parse_def_params_then(ann)(name_tok)(new List<object>())(st2) : default) : default);

    public static T2966 parse_def_params_then<T2966>(object ann, object name_tok, object acc, object st) => (is_left_paren(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? (is_ident(current_kind(st2)) ? ((object param = current(st2)) is var _ ? ((object st3 = advance(st2)) is var _ ? ((object st4 = expect(RightParen)(st3)) is var _ ? parse_def_params_then(ann)(name_tok)((acc + new List<object> { param }))(st4) : default) : default) : default) : finish_def(ann)(name_tok)(acc)(st)) : default) : finish_def(ann)(name_tok)(acc)(st));

    public static T2979 finish_def<T2979>(object ann, object name_tok, object @params, object st) => ((object st2 = expect(Equals)(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? ((object body_result = parse_expr(st3)) is var _ ? unwrap_def_body(body_result)(ann)(name_tok)(@params) : default) : default) : default);

    public static T2985 unwrap_def_body<T2985>(object r, object ann, object name_tok, object @params) => r switch { ExprOk(var b, var st) => DefOk(new Def(name: name_tok, @params: @params, ann: ann, body: b))(st),  };

    public static T3008 parse_type_def<T3008>(object st) => (is_type_ident(current_kind(st)) ? ((object name_tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? (is_equals(current_kind(st2)) ? ((object st3 = skip_newlines(advance(st2))) is var _ ? (is_record_keyword(current_kind(st3)) ? parse_record_type(name_tok)(st3) : (is_pipe(current_kind(st3)) ? parse_variant_type(name_tok)(st3) : TypeDefNone(st))) : default) : TypeDefNone(st)) : default) : default) : TypeDefNone(st));

    public static T3019 parse_record_type<T3019>(object name_tok, object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = expect(LeftBrace)(st2)) is var _ ? ((object st4 = skip_newlines(st3)) is var _ ? parse_record_fields_loop(name_tok)(new List<object>())(st4) : default) : default) : default);

    public static T3035 parse_record_fields_loop<T3035>(object name_tok, object acc, object st) => (is_right_brace(current_kind(st)) ? TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: RecordBody(acc)))(advance(st)) : (is_ident(current_kind(st)) ? parse_one_record_field(name_tok)(acc)(st) : TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: RecordBody(acc)))(st)));

    public static T3048 parse_one_record_field<T3048>(object name_tok, object acc, object st) => ((object field_name = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Colon)(st2)) is var _ ? ((object field_type_result = parse_type(st3)) is var _ ? unwrap_record_field_type(name_tok)(acc)(field_name)(field_type_result) : default) : default) : default) : default);

    public static T3054 unwrap_record_field_type<T3054>(object name_tok, object acc, object field_name, object r) => r switch { TypeOk(var ft, var st) => ((object field = new RecordFieldDef(name: field_name, type_expr: ft)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_record_fields_loop(name_tok)((acc + new List<object> { field }))(skip_newlines(advance(st2))) : parse_record_fields_loop(name_tok)((acc + new List<object> { field }))(st2)) : default) : default),  };

    public static T3072 parse_variant_type<T3072>(object name_tok, object st) => parse_variant_ctors(name_tok)(new List<object>())(st);

    public static T3090 parse_variant_ctors<T3090>(object name_tok, object acc, object st) => (is_pipe(current_kind(st)) ? ((object st2 = skip_newlines(advance(st))) is var _ ? ((object ctor_name = current(st2)) is var _ ? ((object st3 = advance(st2)) is var _ ? parse_ctor_fields(ctor_name)(new List<object>())(st3)(name_tok)(acc) : default) : default) : default) : TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: VariantBody(acc)))(st));

    public static T3109 parse_ctor_fields<T3109>(object ctor_name, object fields, object st, object name_tok, object acc) => (is_left_paren(current_kind(st)) ? ((object field_result = parse_type(advance(st))) is var _ ? unwrap_ctor_field(field_result)(ctor_name)(fields)(name_tok)(acc) : default) : ((object st2 = skip_newlines(st)) is var _ ? ((object ctor = new VariantCtorDef(name: ctor_name, fields: fields)) is var _ ? parse_variant_ctors(name_tok)((acc + new List<object> { ctor }))(st2) : default) : default));

    public static T3116 unwrap_ctor_field<T3116>(object r, object ctor_name, object fields, object name_tok, object acc) => r switch { TypeOk(var ty, var st) => ((object st2 = expect(RightParen)(st)) is var _ ? parse_ctor_fields(ctor_name)((fields + new List<object> { ty }))(st2)(name_tok)(acc) : default),  };

    public static T3131 parse_document<T3131>(object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_top_level(new List<object>())(new List<object>())(st2) : default);
    ...
    public static T3330 infer_let<T3330>(object st, object env, object bindings, object body) => ((object env2 = infer_let_bindings(st)(env)(bindings)(0)(list_length(bindings))) is var _ ? infer_expr(env2.state)(env2.env)(body) : default);
    ...
    public static T3393 wrap_fun_type<T3393>(object param_types, object result) => wrap_fun_type_loop(param_types)(result)((list_length(param_types) - 1));

    public static T3404 wrap_fun_type_loop<T3404>(object param_types, object result, object i) => ((i < 0) ? result : wrap_fun_type_loop(param_types)(FunTy(list_at(param_types)(i))(result))((i - 1)));
    ...
    public static T3460 unify_list_elems<T3460>(object st, object env, object elems, object elem_ty, object i, object len) => ((i == len) ? st : ((object er = infer_expr(st)(env)(list_at(elems)(i))) is var _ ? ((object r = unify(er.state)(er.inferred_type)(elem_ty)) is var _ ? unify_list_elems(r.state)(env)(elems)(elem_ty)((i + 1))(len) : default) : default));
    ...
    public static T3503 infer_match_arms<T3503>(object st, object env, object scrut_ty, object result_ty, object arms, object i, object len) => ((i == len) ? st : ((object arm = list_at(arms)(i)) is var _ ? ((object env2 = bind_pattern(env)(arm.pattern)(scrut_ty)) is var _ ? ((object br = infer_expr(st)(env2)(arm.body)) is var _ ? ((object r = unify(br.state)(br.inferred_type)(result_ty)) is var _ ? infer_match_arms(r.state)(env)(scrut_ty)(result_ty)(arms)((i + 1))(len) : default) : default) : default) : default));

    public static T3508 bind_pattern<T3508>(object env, object pat, object ty) => pat switch { AVarPat(var name) => env_bind(env)(name.value)(ty), AWildPat { } => env, _ => env,  };

    public static T3522 infer_do<T3522>(object st, object env, object stmts) => infer_do_loop(st)(env)(stmts)(0)(list_length(stmts))(NothingTy);
    ...
    public static T3607 resolve_type_expr<T3607>(object texpr) => texpr switch { ANamedType(var name) => resolve_type_name(name.value), AFunType(var param, var ret) => FunTy(resolve_type_expr(param))(resolve_type_expr(ret)), AAppType(var ctor, var args) => resolve_type_expr(ctor),  };
    ...
    public static T3690 check_module<T3690>(object mod) => ((object env = register_all_defs(empty_unification_state)(builtin_type_env)(mod.defs)(0)(list_length(mod.defs))) is var _ ? check_all_defs(env.state)(env.env)(mod.defs)(0)(list_length(mod.defs))(new List<object>()) : default);
    ...
    public static T3740 env_lookup<T3740>(object env, object name) => env_lookup_loop(env.bindings)(name)(0)(list_length(env.bindings));
    ...
    public static T3759 env_has<T3759>(object env, object name) => env_has_loop(env.bindings)(name)(0)(list_length(env.bindings));
    ...
    public static T3941 builtin_type_env<T3941>() => ((T3941 e = empty_type_env) is var _ ? ((T3941 e2 = env_bind(e)("negate")(FunTy(IntegerTy)(IntegerTy))) is var _ ? ((T3941 e3 = env_bind(e2)("text-length")(FunTy(TextTy)(IntegerTy))) is var _ ? ((T3941 e4 = env_bind(e3)("integer-to-text")(FunTy(IntegerTy)(TextTy))) is var _ ? ((T3941 e5 = env_bind(e4)("char-at")(FunTy(TextTy)(FunTy(IntegerTy)(TextTy)))) is var _ ? ((T3941 e6 = env_bind(e5)("substring")(FunTy(TextTy)(FunTy(IntegerTy)(FunTy(IntegerTy)(TextTy))))) is var _ ? ((T3941 e7 = env_bind(e6)("is-letter")(FunTy(TextTy)(BooleanTy))) is var _ ? ((T3941 e8 = env_bind(e7)("is-digit")(FunTy(TextTy)(BooleanTy))) is var _ ? ((T3941 e9 = env_bind(e8)("is-whitespace")(FunTy(TextTy)(BooleanTy))) is var _ ? ((T3941 e10 = env_bind(e9)("char-code")(FunTy(TextTy)(IntegerTy))) is var _ ? ((T3941 e11 = env_bind(e10)("code-to-char")(FunTy(IntegerTy)(TextTy))) is var _ ? ((T3941 e12 = env_bind(e11)("text-replace")(FunTy(TextTy)(FunTy(TextTy)(FunTy(TextTy)(TextTy))))) is var _ ? ((T3941 e13 = env_bind(e12)("text-to-integer")(FunTy(TextTy)(IntegerTy))) is var _ ? ((T3941 e14 = env_bind(e13)("show")(ForAllTy(0)(FunTy(TypeVar(0))(TextTy)))) is var _ ? ((T3941 e15 = env_bind(e14)("print-line")(FunTy(TextTy)(NothingTy))) is var _ ? ((T3941 e16 = env_bind(e15)("list-length")(ForAllTy(0)(FunTy(ListTy(TypeVar(0)))(IntegerTy)))) is var _ ? ((T3941 e17 = env_bind(e16)("list-at")(ForAllTy(0)(FunTy(ListTy(TypeVar(0)))(FunTy(IntegerTy)(TypeVar(0)))))) is var _ ? ((T3941 e18 = env_bind(e17)("map")(ForAllTy(0)(ForAllTy(1)(FunTy(FunTy(TypeVar(0))(TypeVar(1)))(FunTy(ListTy(TypeVar(0)))(ListTy(TypeVar(1)))))))) is var _ ? ((T3941 e19 = env_bind(e18)("filter")(ForAllTy(0)(FunTy(FunTy(TypeVar(0))(BooleanTy))(FunTy(ListTy(TypeVar(0)))(ListTy(TypeVar(0))))))) is var _ ? ((T3941 e20 = env_bind(e19)("fold")(ForAllTy(0)(ForAllTy(1)(FunTy(FunTy(TypeVar(1))(FunTy(TypeVar(0))(TypeVar(1))))(FunTy(TypeVar(1))(FunTy(ListTy(TypeVar(0)))(TypeVar(1)))))))) is var _ ? ((T3941 e21 = env_bind(e20)("read-line")(TextTy)) is var _ ? e21 : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default);
    ...
    public static T3945 fresh_var<T3945>(object st) => TypeVar(st.next_id);
    ...
    public static T3957 subst_lookup<T3957>(object var_id, object entries) => subst_lookup_loop(var_id)(entries)(0)(list_length(entries));
    ...
    public static T3976 has_subst<T3976>(object var_id, object entries) => has_subst_loop(var_id)(entries)(0)(list_length(entries));
    ...
    public static T3991 resolve<T3991>(object st, object ty) => ty switch { TypeVar(var id) => (has_subst(id)(st.substitutions) ? resolve(st)(subst_lookup(id)(st.substitutions)) : ty), _ => ty,  };
    ...
    public static T4032 unify<T4032>(object st, object a, object b) => ((object ra = resolve(st)(a)) is var _ ? ((object rb = resolve(st)(b)) is var _ ? unify_resolved(st)(ra)(rb) : default) : default);
    ...
    public static T4111 unify_fun<T4111>(object st, object pa, object ra, object pb, object rb) => ((object r1 = unify(st)(pa)(pb)) is var _ ? (r1.success ? unify(r1.state)(ra)(rb) : r1) : default);
