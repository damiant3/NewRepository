...
public static class Codex_Codex_Codex
{
    ...
    public static IRExpr lower_apply(AExpr f, AExpr a, CodexType ty, LowerCtx ctx) => ((IRExpr func_ir = lower_expr(f, ErrorTy, ctx)) is var _ ? ((CodexType func_ty = deep_resolve(ctx.ust, ir_expr_type(func_ir))) is var _ ? ((CodexType arg_ty = peel_fun_param(func_ty)) is var _ ? ((CodexType ret_ty = peel_fun_return(func_ty)) is var _ ? ((object actual_ret = ret_ty switch { ErrorTy { } => ty, _ => ret_ty,  }) is var _ ? ((IRExpr arg_ir = lower_expr(a, arg_ty, ctx)) is var _ ? new IrApply(func_ir, arg_ir, actual_ret) : default) : default) : default) : default) : default) : default);
    ...
    public static List<RecordField> build_record_fields_for_lower(List<ARecordFieldDef> fields, long i, long len, List<RecordField> acc) => ((i == len) ? acc : ((object f = list_at(fields)(i)) is var _ ? ((object rfield = new RecordField(name: f.name, type_val: resolve_type_expr_for_lower(f.type_expr))) is var _ ? build_record_fields_for_lower(fields, (i + 1), len, (acc + new List<RecordField> { rfield })) : default) : default));

    public static CodexType build_record_ctor_type_for_lower(List<ARecordFieldDef> fields, CodexType result, long i, long len) => ((i == len) ? result : ((object f = list_at(fields)(i)) is var _ ? ((CodexType rest = build_record_ctor_type_for_lower(fields, result, (i + 1), len)) is var _ ? new FunTy(resolve_type_expr_for_lower(f.type_expr), rest) : default) : default));
    ...
    public static CodexType lookup_type_def_loop(List<TypeBinding> tdm, string name, long i, long len) => ((i == len) ? new ConstructedTy(new Name(value: name), new List<CodexType>()) : ((object b = list_at(tdm)(i)) is var _ ? ((b.name == name) ? b.bound_type : lookup_type_def_loop(tdm, name, (i + 1), len)) : default));
    ...
    public static List<TypeBinding> build_type_def_map(List<ATypeDef> tdefs, long i, long len, List<TypeBinding> acc) => ((i == len) ? acc : ((object td = list_at(tdefs)(i)) is var _ ? ((object entry = td switch { AVariantTypeDef(var name, var type_params, var ctors) => ((List<SumCtor> sum_ctors = build_sum_ctors(tdefs, ctors, 0, list_length(ctors), new List<SumCtor>(), acc)) is var _ ? new TypeBinding(name: name.value, bound_type: new SumTy(name, sum_ctors)) : default), ARecordTypeDef(var name, var type_params, var fields) => ((List<RecordField> rec_fields = build_record_fields_for_map(tdefs, fields, 0, list_length(fields), new List<RecordField>(), acc)) is var _ ? new TypeBinding(name: name.value, bound_type: new RecordTy(name, rec_fields)) : default),  }) is var _ ? build_type_def_map(tdefs, (i + 1), len, (acc + new List<TypeBinding> { entry })) : default) : default));

    public static List<SumCtor> build_sum_ctors(List<ATypeDef> tdefs, List<AVariantCtorDef> ctors, long i, long len, List<SumCtor> acc, List<TypeBinding> partial_tdm) => ((i == len) ? acc : ((object c = list_at(ctors)(i)) is var _ ? ((List<CodexType> field_types = resolve_type_expr_list_for_map(tdefs, c.fields, 0, list_length(c.fields), new List<CodexType>(), partial_tdm)) is var _ ? ((object sc = new SumCtor(name: c.name, fields: field_types)) is var _ ? build_sum_ctors(tdefs, ctors, (i + 1), len, (acc + new List<SumCtor> { sc }), partial_tdm) : default) : default) : default));

    public static List<RecordField> build_record_fields_for_map(List<ATypeDef> tdefs, List<ARecordFieldDef> fields, long i, long len, List<RecordField> acc, List<TypeBinding> partial_tdm) => ((i == len) ? acc : ((object f = list_at(fields)(i)) is var _ ? ((object rfield = new RecordField(name: f.name, type_val: resolve_type_expr(partial_tdm, f.type_expr))) is var _ ? build_record_fields_for_map(tdefs, fields, (i + 1), len, (acc + new List<RecordField> { rfield }), partial_tdm) : default) : default));
    ...
    public static List<RecordField> build_record_fields(List<TypeBinding> tdm, List<ARecordFieldDef> fields, long i, long len, List<RecordField> acc) => ((i == len) ? acc : ((object f = list_at(fields)(i)) is var _ ? ((object rfield = new RecordField(name: f.name, type_val: resolve_type_expr(tdm, f.type_expr))) is var _ ? build_record_fields(tdm, fields, (i + 1), len, (acc + new List<RecordField> { rfield })) : default) : default));
    ...
    public static LetBindResult register_variant_ctors(UnificationState st, TypeEnv env, List<TypeBinding> tdm, List<AVariantCtorDef> ctors, CodexType result_ty, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((object ctor = list_at(ctors)(i)) is var _ ? ((CodexType ctor_ty = build_ctor_type(tdm, ctor.fields, result_ty, 0, list_length(ctor.fields))) is var _ ? ((TypeEnv env2 = env_bind(env, ctor.name.value, ctor_ty)) is var _ ? register_variant_ctors(st, env2, tdm, ctors, result_ty, (i + 1), len) : default) : default) : default));
    ...
    public static CodexType build_record_ctor_type(List<TypeBinding> tdm, List<ARecordFieldDef> fields, CodexType result, long i, long len) => ((i == len) ? result : ((object f = list_at(fields)(i)) is var _ ? ((CodexType rest = build_record_ctor_type(tdm, fields, result, (i + 1), len)) is var _ ? new FunTy(resolve_type_expr(tdm, f.type_expr), rest) : default) : default));
