...
public static class Codex_Codex_Codex
{
    ...
    public static ATypeExpr desugar_type_expr(TypeExpr t) => t switch { NamedType(var tok) => new ANamedType(make_name(tok.text)), FunType(var param, var ret) => new AFunType(desugar_type_expr(param), desugar_type_expr(ret)), AppType(var ctor, var args) => new AAppType(desugar_type_expr(ctor), (_p0_) => map_list(desugar_type_expr(args), _p0_)), ParenType(var inner) => desugar_type_expr(inner), ListType(var elem) => new AAppType(new ANamedType(make_name("List")), new List<object> { desugar_type_expr(elem) }), LinearTypeExpr(var inner) => desugar_type_expr(inner),  };

    public static ADef desugar_def(Def d) => ((ADef ann_types = desugar_annotations(d.ann)) is var _ ? new ADef(name: make_name(d.name.text), @params: (_p0_) => map_list(desugar_param(d.@params), _p0_), declared_type: ann_types, body: desugar_expr(d.body)) : default);
    ...
    public static List<b> map_list(Func<a, b> f, List<a> xs) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => map_list_loop(f(xs(0)(list_length(xs))(new List<b>())), _p0_, _p1_, _p2_, _p3_);

    public static List<b> map_list_loop(Func<a, b> f, List<a> xs, long i, long len, List<b> acc) => ((i == len) ? acc : (_p0_) => (_p1_) => (_p2_) => (_p3_) => map_list_loop(f(xs((i + 1))(len((acc + new List<b> { f(list_at(xs(i))) })))), _p0_, _p1_, _p2_, _p3_));
    ...
    public static List<ArityEntry> build_arity_map(List<IRDef> defs, long i) => ((i == list_length(defs)) ? new List<ArityEntry>() : ((List<ArityEntry> d = list_at(defs(i))) is var _ ? (new List<ArityEntry> { new ArityEntry(name: d.name, arity: list_length(d.@params)) } + (_p0_) => build_arity_map(defs((i + 1)), _p0_)) : default));
    ...
    public static long lookup_arity_loop(List<ArityEntry> entries, string name, long i, long len) => ((i == len) ? (0 - 1) : ((long e = list_at(entries(i))) is var _ ? ((e.name == name) ? e.arity : (_p0_) => (_p1_) => (_p2_) => lookup_arity_loop(entries(name((i + 1))(len)), _p0_, _p1_, _p2_)) : default));
    ...
    public static bool is_upper_letter(string c) => ((bool code = char_code(c)) is var _ ? ((code >= 65) && (code <= 90)) : default);
    ...
    public static string emit_apply(IRExpr e, List<ArityEntry> arities) => ((string chain = (_p0_) => collect_apply_chain(e(new List<object>()), _p0_)) is var _ ? ((string root = chain.root) is var _ ? ((string args = chain.args) is var _ ? root switch { IrName(var n, var ty) => (((text_length(n) > 0) && is_upper_letter(char_at(n(0)))) ? ("new " + (sanitize(n) + ("(" + ((_p0_) => (_p1_) => emit_apply_args(args(arities(0)), _p0_, _p1_) + ")")))) : ((string ar = (_p0_) => lookup_arity(arities(n), _p0_)) is var _ ? (((ar > 1) && (list_length(args) == ar)) ? (sanitize(n) + ("(" + ((_p0_) => (_p1_) => emit_apply_args(args(arities(0)), _p0_, _p1_) + ")"))) : (((ar > 1) && (list_length(args) < ar)) ? ((string remaining = (ar - list_length(args))) is var _ ? (emit_partial_wrappers(0, remaining) + (sanitize(n) + ("(" + ((_p0_) => (_p1_) => emit_apply_args(args(arities(0)), _p0_, _p1_) + (", " + (emit_partial_params(0, remaining) + ")")))))) : default) : (_p0_) => emit_expr_curried(e(arities), _p0_))) : default)), _ => (_p0_) => emit_expr_curried(e(arities), _p0_),  } : default) : default) : default);
    ...
    public static string emit_lambda(List<IRParam> @params, IRExpr body, List<ArityEntry> arities) => ((list_length(@params) == 0) ? ("(() => " + ((_p0_) => emit_expr(body(arities), _p0_) + ")")) : ((list_length(@params) == 1) ? ((string p = list_at(@params(0))) is var _ ? ("((" + (cs_type(p.type_val) + (" " + (sanitize(p.name) + (") => " + ((_p0_) => emit_expr(body(arities), _p0_) + ")")))))) : default) : ("(() => " + ((_p0_) => emit_expr(body(arities), _p0_) + ")"))));
    ...
    public static string emit_match_arms(List<IRBranch> branches, long i, List<ArityEntry> arities) => ((i == list_length(branches)) ? "" : ((string arm = list_at(branches(i))) is var _ ? (emit_pattern(arm.pattern) + (" => " + ((_p0_) => emit_expr(arm.body(arities), _p0_) + (", " + (_p0_) => (_p1_) => emit_match_arms(branches((i + 1))(arities), _p0_, _p1_))))) : default));
    ...
    public static string emit_sub_patterns(List<IRPat> subs, long i) => ((i == list_length(subs)) ? "" : ((string sub = list_at(subs(i))) is var _ ? (emit_sub_pattern(sub) + (((i < (list_length(subs) - 1)) ? ", " : "") + (_p0_) => emit_sub_patterns(subs((i + 1)), _p0_))) : default));
    ...
    public static string emit_do_stmts(List<IRDoStmt> stmts, long i, List<ArityEntry> arities) => ((i == list_length(stmts)) ? "" : ((string s = list_at(stmts(i))) is var _ ? ((_p0_) => emit_do_stmt(s(arities), _p0_) + (" " + (_p0_) => (_p1_) => emit_do_stmts(stmts((i + 1))(arities), _p0_, _p1_))) : default));
    ...
    public static string emit_record_fields(List<IRFieldVal> fields, long i, List<ArityEntry> arities) => ((i == list_length(fields)) ? "" : ((string f = list_at(fields(i))) is var _ ? (sanitize(f.name) + (": " + ((_p0_) => emit_expr(f.value(arities), _p0_) + (((i < (list_length(fields) - 1)) ? ", " : "") + (_p0_) => (_p1_) => emit_record_fields(fields((i + 1))(arities), _p0_, _p1_))))) : default));
    ...
    public static string emit_record_field_defs(List<ARecordFieldDef> fields, List<Name> tparams, long i) => ((i == list_length(fields)) ? "" : ((string f = list_at(fields(i))) is var _ ? ((_p0_) => emit_type_expr_tp(f.type_expr(tparams), _p0_) + (" " + (sanitize(f.name.value) + (((i < (list_length(fields) - 1)) ? ", " : "") + (_p0_) => (_p1_) => emit_record_field_defs(fields(tparams((i + 1))), _p0_, _p1_))))) : default));

    public static string emit_variant_ctors(List<AVariantCtorDef> ctors, Name base_name, List<Name> tparams, long i) => ((i == list_length(ctors)) ? "" : ((string c = list_at(ctors(i))) is var _ ? ((_p0_) => (_p1_) => emit_variant_ctor(c(base_name(tparams)), _p0_, _p1_) + (_p0_) => (_p1_) => (_p2_) => emit_variant_ctors(ctors(base_name(tparams((i + 1)))), _p0_, _p1_, _p2_)) : default));
    ...
    public static string generic_suffix(CodexType ty) => ((string ids = (_p0_) => collect_type_var_ids(ty(new List<object>()), _p0_)) is var _ ? ((list_length(ids) == 0) ? "" : ("<" + ((_p0_) => emit_type_params(ids(0), _p0_) + ">"))) : default);
    ...
    public static string emit_def(IRDef d, List<ArityEntry> arities) => ((string ret = (_p0_) => get_return_type(d.type_val(list_length(d.@params)), _p0_)) is var _ ? ((string gen = generic_suffix(d.type_val)) is var _ ? ("    public static " + (cs_type(ret) + (" " + (sanitize(d.name) + (gen + ("(" + ((_p0_) => emit_def_params(d.@params(0), _p0_) + (") => " + ((_p0_) => emit_expr(d.body(arities), _p0_) + ";\\n"))))))))) : default) : default);
    ...
    public static string emit_def_params(List<IRParam> @params, long i) => ((i == list_length(@params)) ? "" : ((string p = list_at(@params(i))) is var _ ? (cs_type(p.type_val) + (" " + (sanitize(p.name) + (((i < (list_length(@params) - 1)) ? ", " : "") + (_p0_) => emit_def_params(@params((i + 1)), _p0_))))) : default));

    public static string emit_full_module(IRModule m, List<ATypeDef> type_defs) => ((string arities = (_p0_) => build_arity_map(m.defs(0), _p0_)) is var _ ? ("using System;\\nusing System.Collections.Generic;\\nusing System.Linq;\\n\\n" + ((_p0_) => emit_type_defs(type_defs(0), _p0_) + (emit_class_header(m.name.value) + ((_p0_) => (_p1_) => emit_defs(m.defs(0)(arities), _p0_, _p1_) + "}\\n")))) : default);

    public static string emit_module(IRModule m) => ((string arities = (_p0_) => build_arity_map(m.defs(0), _p0_)) is var _ ? ("using System;\\nusing System.Collections.Generic;\\nusing System.Linq;\\n\\n" + (emit_class_header(m.name.value) + ((_p0_) => (_p1_) => emit_defs(m.defs(0)(arities), _p0_, _p1_) + "}\\n"))) : default);
    ...
    public static CodexType lookup_type_loop(List<TypeBinding> bindings, string name, long i, long len) => ((i == len) ? ErrorTy : ((CodexType b = list_at(bindings(i))) is var _ ? ((b.name == name) ? b.bound_type : (_p0_) => (_p1_) => (_p2_) => lookup_type_loop(bindings(name((i + 1))(len)), _p0_, _p1_, _p2_)) : default));
    ...
    public static IRExpr lower_expr(AExpr e, CodexType ty) => e switch { ALitExpr(var text, var kind) => (_p0_) => lower_literal(text(kind), _p0_), ANameExpr(var name) => new IrName(name.value(ty)), AApplyExpr(var f, var a) => (_p0_) => (_p1_) => lower_apply(f(a(ty)), _p0_, _p1_), ABinaryExpr(var l, var op, var r) => new IrBinary((_p0_) => lower_bin_op(op(ty), _p0_), (_p0_) => lower_expr(l(ty), _p0_), (_p0_) => lower_expr(r(ty), _p0_), ty), AUnaryExpr(var operand) => new IrNegate((_p0_) => lower_expr(operand(IntegerTy), _p0_)), AIfExpr(var c, var t, var e2) => new IrIf((_p0_) => lower_expr(c(BooleanTy), _p0_), (_p0_) => lower_expr(t(ty), _p0_), (_p0_) => lower_expr(e2(ty), _p0_), ty), ALetExpr(var binds, var body) => (_p0_) => (_p1_) => lower_let(binds(body(ty)), _p0_, _p1_), ALambdaExpr(var @params, var body) => (_p0_) => (_p1_) => lower_lambda(@params(body(ty)), _p0_, _p1_), AMatchExpr(var scrut, var arms) => (_p0_) => (_p1_) => lower_match(scrut(arms(ty)), _p0_, _p1_), AListExpr(var elems) => (_p0_) => lower_list(elems(ty), _p0_), ARecordExpr(var name, var fields) => (_p0_) => (_p1_) => lower_record(name(fields(ty)), _p0_, _p1_), AFieldAccess(var rec, var field) => new IrFieldAccess((_p0_) => lower_expr(rec(ty), _p0_), field.value(ty)), ADoExpr(var stmts) => (_p0_) => lower_do(stmts(ty), _p0_), AErrorExpr(var msg) => new IrError(msg(ty)),  };
    ...
    public static IRExpr lower_apply(AExpr f, AExpr a, CodexType ty) => new IrApply((_p0_) => lower_expr(f(ty), _p0_), (_p0_) => lower_expr(a(ty), _p0_), ty);

    public static IRExpr lower_let(List<ALetBind> binds, AExpr body, CodexType ty) => ((list_length(binds) == 0) ? (_p0_) => lower_expr(body(ty), _p0_) : ((IRExpr b = list_at(binds(0))) is var _ ? new IrLet(b.name.value(ty((_p0_) => lower_expr(b.value(ErrorTy), _p0_))((_p0_) => (_p1_) => (_p2_) => lower_let_rest(binds(body(ty(1))), _p0_, _p1_, _p2_)))) : default));

    public static IRExpr lower_let_rest(List<ALetBind> binds, AExpr body, CodexType ty, long i) => ((i == list_length(binds)) ? (_p0_) => lower_expr(body(ty), _p0_) : ((IRExpr b = list_at(binds(i))) is var _ ? new IrLet(b.name.value(ty((_p0_) => lower_expr(b.value(ErrorTy), _p0_))((_p0_) => (_p1_) => (_p2_) => lower_let_rest(binds(body(ty((i + 1)))), _p0_, _p1_, _p2_)))) : default));

    public static IRExpr lower_lambda(List<Name> @params, AExpr body, CodexType ty) => ((IRExpr stripped = strip_forall_ty(ty)) is var _ ? new IrLambda((_p0_) => (_p1_) => lower_lambda_params(@params(stripped(0)), _p0_, _p1_), (_p0_) => lower_expr(body((_p0_) => get_lambda_return(stripped(list_length(@params)), _p0_)), _p0_), ty) : default);

    public static List<IRParam> lower_lambda_params(List<Name> @params, CodexType ty, long i) => ((i == list_length(@params)) ? new List<IRParam>() : ((List<IRParam> p = list_at(@params(i))) is var _ ? ((List<IRParam> param_ty = peel_fun_param(ty)) is var _ ? ((List<IRParam> rest_ty = peel_fun_return(ty)) is var _ ? (new List<IRParam> { new IRParam(name: p.value, type_val: param_ty) } + (_p0_) => (_p1_) => lower_lambda_params(@params(rest_ty((i + 1))), _p0_, _p1_)) : default) : default) : default));
    ...
    public static IRExpr lower_match(AExpr scrut, List<AMatchArm> arms, CodexType ty) => new IrMatch((_p0_) => lower_expr(scrut(ty), _p0_), map_list((_p0_) => lower_arm(ty, _p0_), arms), ty);

    public static IRBranch lower_arm(CodexType ty, AMatchArm arm) => new IRBranch(pattern: lower_pattern(arm.pattern), body: (_p0_) => lower_expr(arm.body(ty), _p0_));
    ...
    public static IRExpr lower_list(List<AExpr> elems, CodexType ty) => ((IRExpr elem_ty = ty switch { ListTy(var e) => e, _ => ErrorTy,  }) is var _ ? new IrList(map_list((_p0_) => lower_elem(elem_ty, _p0_), elems), elem_ty) : default);

    public static IRExpr lower_elem(CodexType ty, AExpr e) => (_p0_) => lower_expr(e(ty), _p0_);

    public static IRExpr lower_record(Name name, List<AFieldExpr> fields, CodexType ty) => new IrRecord(name.value(map_list((_p0_) => lower_field_val(ty, _p0_), fields))(ty));

    public static IRFieldVal lower_field_val(CodexType ty, AFieldExpr f) => new IRFieldVal(name: f.name.value, value: (_p0_) => lower_expr(f.value(ty), _p0_));

    public static IRExpr lower_do(List<ADoStmt> stmts, CodexType ty) => new IrDo(map_list((_p0_) => lower_do_stmt(ty, _p0_), stmts), ty);

    public static IRDoStmt lower_do_stmt(CodexType ty, ADoStmt s) => s switch { ADoBindStmt(var name, var val) => new IrDoBind(name.value(ty((_p0_) => lower_expr(val(ty), _p0_)))), ADoExprStmt(var e) => new IrDoExec((_p0_) => lower_expr(e(ty), _p0_)),  };

    public static IRDef lower_def(ADef d, List<TypeBinding> types, UnificationState ust) => ((IRDef raw_type = (_p0_) => lookup_type(types(d.name.value), _p0_)) is var _ ? ((IRDef full_type = (_p0_) => deep_resolve(ust(raw_type), _p0_)) is var _ ? ((IRDef stripped = strip_forall_ty(full_type)) is var _ ? ((IRDef @params = (_p0_) => (_p1_) => lower_def_params(d.@params(stripped(0)), _p0_, _p1_)) is var _ ? ((IRDef ret_type = (_p0_) => get_return_type_n(stripped(list_length(d.@params)), _p0_)) is var _ ? new IRDef(name: d.name.value, @params: @params, type_val: full_type, body: (_p0_) => lower_expr(d.body(ret_type), _p0_)) : default) : default) : default) : default) : default);

    public static List<IRParam> lower_def_params(List<AParam> @params, CodexType ty, long i) => ((i == list_length(@params)) ? new List<IRParam>() : ((List<IRParam> p = list_at(@params(i))) is var _ ? ((List<IRParam> param_ty = peel_fun_param(ty)) is var _ ? ((List<IRParam> rest_ty = peel_fun_return(ty)) is var _ ? (new List<IRParam> { new IRParam(name: p.name.value, type_val: param_ty) } + (_p0_) => (_p1_) => lower_def_params(@params(rest_ty((i + 1))), _p0_, _p1_)) : default) : default) : default));
    ...
    public static IRModule lower_module(AModule m, List<TypeBinding> types, UnificationState ust) => new IRModule(name: m.name, defs: (_p0_) => (_p1_) => (_p2_) => lower_defs(m.defs(types(ust(0))), _p0_, _p1_, _p2_));
    ...
    public static CtorCollectResult collect_ctor_names(List<ATypeDef> type_defs, long i, long len, List<string> type_acc, List<string> ctor_acc) => ((i == len) ? new CtorCollectResult(type_names: type_acc, ctor_names: ctor_acc) : ((CtorCollectResult td = list_at(type_defs(i))) is var _ ? td switch { AVariantTypeDef(var name, var @params, var ctors) => ((CtorCollectResult new_type_acc = (type_acc + new List<object> { name.value })) is var _ ? ((CtorCollectResult new_ctor_acc = (_p0_) => (_p1_) => (_p2_) => collect_variant_ctors(ctors(0)(list_length(ctors))(ctor_acc), _p0_, _p1_, _p2_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => collect_ctor_names(type_defs((i + 1))(len(new_type_acc(new_ctor_acc))), _p0_, _p1_, _p2_, _p3_) : default) : default), ARecordTypeDef(var name, var @params, var fields) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => collect_ctor_names(type_defs((i + 1))(len((type_acc + new List<object> { name.value }))(ctor_acc)), _p0_, _p1_, _p2_, _p3_),  } : default));

    public static List<string> collect_variant_ctors(List<AVariantCtorDef> ctors, long i, long len, List<string> acc) => ((i == len) ? acc : ((List<string> ctor = list_at(ctors(i))) is var _ ? (_p0_) => (_p1_) => (_p2_) => collect_variant_ctors(ctors((i + 1))(len((acc + new List<string> { ctor.name.value }))), _p0_, _p1_, _p2_) : default));

    public static Scope build_all_names_scope(List<string> top_names, List<string> ctor_names, List<string> builtins) => ((Scope sc = (_p0_) => (_p1_) => (_p2_) => add_names_to_scope(empty_scope(top_names(0)(list_length(top_names))), _p0_, _p1_, _p2_)) is var _ ? ((Scope sc2 = (_p0_) => (_p1_) => (_p2_) => add_names_to_scope(sc(ctor_names(0)(list_length(ctor_names))), _p0_, _p1_, _p2_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => add_names_to_scope(sc2(builtins(0)(list_length(builtins))), _p0_, _p1_, _p2_) : default) : default);
    ...
    public static List<Diagnostic> resolve_expr(Scope sc, AExpr expr) => expr switch { ALitExpr(var val, var kind) => new List<Diagnostic>(), ANameExpr(var name) => (((_p0_) => scope_has(sc(name.value), _p0_) || is_type_name(name.value)) ? new List<Diagnostic>() : new List<Diagnostic> { make_error("CDX3002", ("Undefined name: " + name.value)) }), ABinaryExpr(var left, var op, var right) => ((_p0_) => resolve_expr(sc(left), _p0_) + (_p0_) => resolve_expr(sc(right), _p0_)), AUnaryExpr(var operand) => (_p0_) => resolve_expr(sc(operand), _p0_), AApplyExpr(var func, var arg) => ((_p0_) => resolve_expr(sc(func), _p0_) + (_p0_) => resolve_expr(sc(arg), _p0_)), AIfExpr(var cond, var then_e, var else_e) => ((_p0_) => resolve_expr(sc(cond), _p0_) + ((_p0_) => resolve_expr(sc(then_e), _p0_) + (_p0_) => resolve_expr(sc(else_e), _p0_))), ALetExpr(var bindings, var body) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => resolve_let(sc(bindings(body(0)(list_length(bindings))(new List<Diagnostic>()))), _p0_, _p1_, _p2_, _p3_, _p4_), ALambdaExpr(var @params, var body) => ((List<Diagnostic> sc2 = (_p0_) => (_p1_) => (_p2_) => add_lambda_params(sc(@params(0)(list_length(@params))), _p0_, _p1_, _p2_)) is var _ ? (_p0_) => resolve_expr(sc2(body), _p0_) : default), AMatchExpr(var scrutinee, var arms) => ((_p0_) => resolve_expr(sc(scrutinee), _p0_) + (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_match_arms(sc(arms(0)(list_length(arms))(new List<Diagnostic>())), _p0_, _p1_, _p2_, _p3_)), AListExpr(var elems) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_list_elems(sc(elems(0)(list_length(elems))(new List<Diagnostic>())), _p0_, _p1_, _p2_, _p3_), ARecordExpr(var name, var fields) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_record_fields(sc(fields(0)(list_length(fields))(new List<Diagnostic>())), _p0_, _p1_, _p2_, _p3_), AFieldAccess(var obj, var field) => (_p0_) => resolve_expr(sc(obj), _p0_), ADoExpr(var stmts) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_do_stmts(sc(stmts(0)(list_length(stmts))(new List<Diagnostic>())), _p0_, _p1_, _p2_, _p3_), AErrorExpr(var msg) => new List<Diagnostic>(),  };

    public static List<Diagnostic> resolve_let(Scope sc, List<ALetBind> bindings, AExpr body, long i, long len, List<Diagnostic> errs) => ((i == len) ? (errs + (_p0_) => resolve_expr(sc(body), _p0_)) : ((List<Diagnostic> b = list_at(bindings(i))) is var _ ? ((List<Diagnostic> bind_errs = (_p0_) => resolve_expr(sc(b.value), _p0_)) is var _ ? ((List<Diagnostic> sc2 = (_p0_) => scope_add(sc(b.name.value), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => resolve_let(sc2(bindings(body((i + 1))(len((errs + bind_errs))))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default) : default));

    public static Scope add_lambda_params(Scope sc, List<Name> @params, long i, long len) => ((i == len) ? sc : ((Scope p = list_at(@params(i))) is var _ ? (_p0_) => (_p1_) => add_lambda_params((_p0_) => scope_add(sc(p.value), _p0_), @params((i + 1))(len), _p0_, _p1_) : default));

    public static List<Diagnostic> resolve_match_arms(Scope sc, List<AMatchArm> arms, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((List<Diagnostic> arm = list_at(arms(i))) is var _ ? ((List<Diagnostic> sc2 = (_p0_) => collect_pattern_names(sc(arm.pattern), _p0_)) is var _ ? ((List<Diagnostic> arm_errs = (_p0_) => resolve_expr(sc2(arm.body), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_match_arms(sc(arms((i + 1))(len((errs + arm_errs)))), _p0_, _p1_, _p2_, _p3_) : default) : default) : default));
    ...
    public static Scope collect_ctor_pat_names(Scope sc, List<APat> subs, long i, long len) => ((i == len) ? sc : ((Scope sub = list_at(subs(i))) is var _ ? (_p0_) => (_p1_) => collect_ctor_pat_names((_p0_) => collect_pattern_names(sc(sub), _p0_), subs((i + 1))(len), _p0_, _p1_) : default));

    public static List<Diagnostic> resolve_list_elems(Scope sc, List<AExpr> elems, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((List<Diagnostic> errs2 = (_p0_) => resolve_expr(sc(list_at(elems(i))), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_list_elems(sc(elems((i + 1))(len((errs + errs2)))), _p0_, _p1_, _p2_, _p3_) : default));

    public static List<Diagnostic> resolve_record_fields(Scope sc, List<AFieldExpr> fields, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((List<Diagnostic> f = list_at(fields(i))) is var _ ? ((List<Diagnostic> errs2 = (_p0_) => resolve_expr(sc(f.value), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_record_fields(sc(fields((i + 1))(len((errs + errs2)))), _p0_, _p1_, _p2_, _p3_) : default) : default));

    public static List<Diagnostic> resolve_do_stmts(Scope sc, List<ADoStmt> stmts, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((List<Diagnostic> stmt = list_at(stmts(i))) is var _ ? stmt switch { ADoExprStmt(var e) => ((List<Diagnostic> errs2 = (_p0_) => resolve_expr(sc(e), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_do_stmts(sc(stmts((i + 1))(len((errs + errs2)))), _p0_, _p1_, _p2_, _p3_) : default), ADoBindStmt(var name, var e) => ((List<Diagnostic> errs2 = (_p0_) => resolve_expr(sc(e), _p0_)) is var _ ? ((List<Diagnostic> sc2 = (_p0_) => scope_add(sc(name.value), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_do_stmts(sc2(stmts((i + 1))(len((errs + errs2)))), _p0_, _p1_, _p2_, _p3_) : default) : default),  } : default));

    public static List<Diagnostic> resolve_all_defs(Scope sc, List<ADef> defs, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((List<Diagnostic> def = list_at(defs(i))) is var _ ? ((List<Diagnostic> def_scope = (_p0_) => (_p1_) => (_p2_) => add_def_params(sc(def.@params(0)(list_length(def.@params))), _p0_, _p1_, _p2_)) is var _ ? ((List<Diagnostic> errs2 = (_p0_) => resolve_expr(def_scope(def.body), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_all_defs(sc(defs((i + 1))(len((errs + errs2)))), _p0_, _p1_, _p2_, _p3_) : default) : default) : default));

    public static Scope add_def_params(Scope sc, List<AParam> @params, long i, long len) => ((i == len) ? sc : ((Scope p = list_at(@params(i))) is var _ ? (_p0_) => (_p1_) => add_def_params((_p0_) => scope_add(sc(p.name.value), _p0_), @params((i + 1))(len), _p0_, _p1_) : default));

    public static ResolveResult resolve_module(AModule mod) => ((ResolveResult top = (_p0_) => (_p1_) => (_p2_) => (_p3_) => collect_top_level_names(mod.defs(0)(list_length(mod.defs))(new List<object>())(new List<object>()), _p0_, _p1_, _p2_, _p3_)) is var _ ? ((ResolveResult ctors = (_p0_) => (_p1_) => (_p2_) => (_p3_) => collect_ctor_names(mod.type_defs(0)(list_length(mod.type_defs))(new List<object>())(new List<object>()), _p0_, _p1_, _p2_, _p3_)) is var _ ? ((ResolveResult sc = (_p0_) => (_p1_) => build_all_names_scope(top.names(ctors.ctor_names(builtin_names)), _p0_, _p1_)) is var _ ? ((ResolveResult expr_errs = (_p0_) => (_p1_) => (_p2_) => (_p3_) => resolve_all_defs(sc(mod.defs(0)(list_length(mod.defs))(new List<object>())), _p0_, _p1_, _p2_, _p3_)) is var _ ? new ResolveResult(errors: (top.errors + expr_errs), top_level_names: top.names, type_names: ctors.type_names, ctor_names: ctors.ctor_names) : default) : default) : default) : default);
    ...
    public static LexResult scan_token(LexState st) => ((LexResult s = skip_spaces(st)) is var _ ? (is_at_end(s) ? LexEnd : ((LexResult ch = peek_char(s)) is var _ ? ((ch == "\\n") ? new LexToken(make_token(Newline, "\\n", s), advance_char(s)) : ((ch == "\\\"") ? ((LexResult start = (s.offset + 1)) is var _ ? ((LexResult after = scan_string_body(advance_char(s))) is var _ ? ((LexResult text_len = ((after.offset - start) - 1)) is var _ ? new LexToken(make_token(TextLiteral, substring(s.source(start(text_len))), s), after) : default) : default) : default) : (is_letter(ch) ? ((LexResult start = s.offset) is var _ ? ((LexResult after = scan_ident_rest(advance_char(s))) is var _ ? ((LexResult word = (_p0_) => (_p1_) => extract_text(s(start(after)), _p0_, _p1_)) is var _ ? new LexToken((_p0_) => make_token(classify_word(word), word(s), _p0_), after) : default) : default) : default) : ((ch == "_") ? ((LexResult start = s.offset) is var _ ? ((LexResult after = scan_ident_rest(advance_char(s))) is var _ ? ((LexResult word = (_p0_) => (_p1_) => extract_text(s(start(after)), _p0_, _p1_)) is var _ ? ((text_length(word) == 1) ? new LexToken(make_token(Underscore, "_", s), after) : new LexToken((_p0_) => make_token(classify_word(word), word(s), _p0_), after)) : default) : default) : default) : (is_digit(ch) ? ((LexResult start = s.offset) is var _ ? ((LexResult after = scan_digits(advance_char(s))) is var _ ? (is_at_end(after) ? new LexToken(make_token(IntegerLiteral, (_p0_) => (_p1_) => extract_text(s(start(after)), _p0_, _p1_), s), after) : ((peek_char(after) == ".") ? ((LexResult after2 = scan_digits(advance_char(after))) is var _ ? new LexToken(make_token(NumberLiteral, (_p0_) => (_p1_) => extract_text(s(start(after2)), _p0_, _p1_), s), after2) : default) : new LexToken(make_token(IntegerLiteral, (_p0_) => (_p1_) => extract_text(s(start(after)), _p0_, _p1_), s), after))) : default) : default) : scan_operator(s)))))) : default)) : default);

    public static LexResult scan_operator(LexState s) => ((LexResult ch = peek_char(s)) is var _ ? ((LexResult next = advance_char(s)) is var _ ? ((ch == "(") ? new LexToken(make_token(LeftParen, "(", s), next) : ((ch == ")") ? new LexToken(make_token(RightParen, ")", s), next) : ((ch == "[") ? new LexToken(make_token(LeftBracket, "[", s), next) : ((ch == "]") ? new LexToken(make_token(RightBracket, "]", s), next) : ((ch == "{") ? new LexToken(make_token(LeftBrace, "{", s), next) : ((ch == "}") ? new LexToken(make_token(RightBrace, "}", s), next) : ((ch == ",") ? new LexToken(make_token(Comma, ",", s), next) : ((ch == ".") ? new LexToken(make_token(Dot, ".", s), next) : ((ch == "^") ? new LexToken(make_token(Caret, "^", s), next) : ((ch == "&") ? new LexToken(make_token(Ampersand, "&", s), next) : scan_multi_char_operator(s))))))))))) : default) : default);

    public static LexResult scan_multi_char_operator(LexState s) => ((LexResult ch = peek_char(s)) is var _ ? ((LexResult next = advance_char(s)) is var _ ? ((LexResult next_ch = (is_at_end(next) ? "" : peek_char(next))) is var _ ? ((ch == "+") ? ((next_ch == "+") ? new LexToken(make_token(PlusPlus, "++", s), advance_char(next)) : new LexToken(make_token(Plus, "+", s), next)) : ((ch == "-") ? ((next_ch == ">") ? new LexToken(make_token(Arrow, "->", s), advance_char(next)) : new LexToken(make_token(Minus, "-", s), next)) : ((ch == "*") ? new LexToken(make_token(Star, "*", s), next) : ((ch == "/") ? ((next_ch == "=") ? new LexToken(make_token(NotEquals, "/=", s), advance_char(next)) : new LexToken(make_token(Slash, "/", s), next)) : ((ch == "=") ? ((next_ch == "=") ? ((LexResult next2 = advance_char(next)) is var _ ? ((LexResult next2_ch = (is_at_end(next2) ? "" : peek_char(next2))) is var _ ? ((next2_ch == "=") ? new LexToken(make_token(TripleEquals, "===", s), advance_char(next2)) : new LexToken(make_token(DoubleEquals, "==", s), next2)) : default) : default) : new LexToken(make_token(Equals, "=", s), next)) : ((ch == ":") ? ((next_ch == ":") ? new LexToken(make_token(ColonColon, "::", s), advance_char(next)) : new LexToken(make_token(Colon, ":", s), next)) : ((ch == "|") ? ((next_ch == "-") ? new LexToken(make_token(Turnstile, "|-", s), advance_char(next)) : new LexToken(make_token(Pipe, "|", s), next)) : ((ch == "<") ? ((next_ch == "=") ? new LexToken(make_token(LessOrEqual, "<=", s), advance_char(next)) : ((next_ch == "-") ? new LexToken(make_token(LeftArrow, "<-", s), advance_char(next)) : new LexToken(make_token(LessThan, "<", s), next))) : ((ch == ">") ? ((next_ch == "=") ? new LexToken(make_token(GreaterOrEqual, ">=", s), advance_char(next)) : new LexToken(make_token(GreaterThan, ">", s), next)) : new LexToken(make_token(ErrorToken, char_at(s.source(s.offset)), s), next)))))))))) : default) : default) : default);

    public static List<Token> tokenize_loop(LexState st, List<Token> acc) => scan_token(st) switch { LexToken(var tok, var next) => ((tok.kind == EndOfFile) ? (acc + new List<Token> { tok }) : (_p0_) => tokenize_loop(next((acc + new List<Token> { tok })), _p0_)), LexEnd { } => (acc + new List<Token> { make_token(EndOfFile, "", st) }),  };
    ...
    public static ParseTypeResult parse_type_continue(TypeExpr left, ParseState st) => (is_arrow(current_kind(st)) ? ((ParseTypeResult st2 = advance(st)) is var _ ? ((ParseTypeResult right_result = parse_type(st2)) is var _ ? (_p0_) => unwrap_type_ok(right_result((_p0_) => (_p1_) => make_fun_type(left, _p0_, _p1_)), _p0_) : default) : default) : new TypeOk(left(st)));
    ...
    public static ParseTypeResult parse_type_atom(ParseState st) => (is_ident(current_kind(st)) ? ((ParseTypeResult tok = current(st)) is var _ ? parse_type_args(new NamedType(tok), advance(st)) : default) : (is_type_ident(current_kind(st)) ? ((ParseTypeResult tok = current(st)) is var _ ? parse_type_args(new NamedType(tok), advance(st)) : default) : (is_left_paren(current_kind(st)) ? parse_paren_type(advance(st)) : ((ParseTypeResult tok = current(st)) is var _ ? new TypeOk(new NamedType(tok), advance(st)) : default))));
    ...
    public static ParseTypeResult finish_paren_type(TypeExpr t, ParseState st) => ((ParseTypeResult st2 = expect(RightParen, st)) is var _ ? new TypeOk(new ParenType(t), st2) : default);
    ...
    public static ParsePatResult parse_pattern(ParseState st) => (is_underscore(current_kind(st)) ? new PatOk(new WildPat(current(st)), advance(st)) : (is_literal(current_kind(st)) ? new PatOk(new LitPat(current(st)), advance(st)) : (is_type_ident(current_kind(st)) ? ((ParsePatResult tok = current(st)) is var _ ? (_p0_) => (_p1_) => parse_ctor_pattern_fields(tok(new List<object>())(advance(st)), _p0_, _p1_) : default) : (is_ident(current_kind(st)) ? new PatOk(new VarPat(current(st)), advance(st)) : new PatOk(new WildPat(current(st)), advance(st))))));

    public static ParsePatResult parse_ctor_pattern_fields(Token ctor, List<Pat> acc, ParseState st) => (is_left_paren(current_kind(st)) ? ((ParsePatResult st2 = advance(st)) is var _ ? ((ParsePatResult sub = parse_pattern(st2)) is var _ ? (_p0_) => unwrap_pat_ok(sub((_p0_) => (_p1_) => (_p2_) => continue_ctor_fields(ctor(acc), _p0_, _p1_, _p2_)), _p0_) : default) : default) : new PatOk(new CtorPat(ctor(acc)), st));

    public static ParsePatResult continue_ctor_fields(Token ctor, List<Pat> acc, Pat p, ParseState st) => ((ParsePatResult st2 = expect(RightParen, st)) is var _ ? (_p0_) => (_p1_) => parse_ctor_pattern_fields(ctor((acc + new List<object> { p }))(st2), _p0_, _p1_) : default);
    ...
    public static ParseExprResult parse_binary_loop(Expr left, ParseState st, long min_prec) => (is_done(st) ? new ExprOk(left(st)) : ((ParseExprResult prec = operator_precedence(current_kind(st))) is var _ ? ((prec < min_prec) ? new ExprOk(left(st)) : ((ParseExprResult op = current(st)) is var _ ? ((ParseExprResult st2 = skip_newlines(advance(st))) is var _ ? ((ParseExprResult next_min = (is_right_assoc(op.kind) ? prec : (prec + 1))) is var _ ? ((ParseExprResult right_result = (_p0_) => parse_binary(st2(next_min), _p0_)) is var _ ? (_p0_) => unwrap_expr_ok(right_result((_p0_) => (_p1_) => (_p2_) => (_p3_) => continue_binary(left(op(min_prec)), _p0_, _p1_, _p2_, _p3_)), _p0_) : default) : default) : default) : default)) : default));
    ...
    public static ParseExprResult parse_unary(ParseState st) => (is_minus(current_kind(st)) ? ((ParseExprResult op = current(st)) is var _ ? ((ParseExprResult result = parse_unary(advance(st))) is var _ ? (_p0_) => unwrap_expr_ok(result((_p0_) => (_p1_) => finish_unary(op, _p0_, _p1_)), _p0_) : default) : default) : parse_application(st));
    ...
    public static ParseExprResult parse_field_access(Expr node, ParseState st) => (is_dot(current_kind(st)) ? ((ParseExprResult st2 = advance(st)) is var _ ? ((ParseExprResult field = current(st2)) is var _ ? ((ParseExprResult st3 = advance(st2)) is var _ ? parse_field_access(new FieldExpr(node(field)), st3) : default) : default) : default) : (is_app_start(current_kind(st)) ? ((ParseExprResult arg_result = parse_atom(st)) is var _ ? (_p0_) => unwrap_expr_ok(arg_result((_p0_) => (_p1_) => continue_app(node, _p0_, _p1_)), _p0_) : default) : new ExprOk(node(st))));

    public static ParseExprResult parse_dot_only(Expr node, ParseState st) => (is_dot(current_kind(st)) ? ((ParseExprResult st2 = advance(st)) is var _ ? ((ParseExprResult field = current(st2)) is var _ ? ((ParseExprResult st3 = advance(st2)) is var _ ? parse_dot_only(new FieldExpr(node(field)), st3) : default) : default) : default) : new ExprOk(node(st)));

    public static ParseExprResult parse_atom_type_ident(ParseState st) => ((ParseExprResult tok = current(st)) is var _ ? ((ParseExprResult st2 = advance(st)) is var _ ? (is_left_brace(current_kind(st2)) ? (_p0_) => parse_record_expr(tok(st2), _p0_) : new ExprOk(new NameExpr(tok), st2)) : default) : default);

    public static ParseExprResult parse_paren_expr(ParseState st) => ((ParseExprResult st2 = skip_newlines(st)) is var _ ? ((ParseExprResult inner = parse_expr(st2)) is var _ ? (_p0_) => unwrap_expr_ok(inner(finish_paren_expr), _p0_) : default) : default);

    public static ParseExprResult finish_paren_expr(Expr e, ParseState st) => ((ParseExprResult st2 = skip_newlines(st)) is var _ ? ((ParseExprResult st3 = expect(RightParen, st2)) is var _ ? new ExprOk(new ParenExpr(e), st3) : default) : default);

    public static ParseExprResult parse_record_expr(Token type_name, ParseState st) => ((ParseExprResult st2 = advance(st)) is var _ ? ((ParseExprResult st3 = skip_newlines(st2)) is var _ ? (_p0_) => (_p1_) => parse_record_expr_fields(type_name(new List<object>())(st3), _p0_, _p1_) : default) : default);
    ...
    public static ParseExprResult parse_if_expr(ParseState st) => ((ParseExprResult st2 = skip_newlines(advance(st))) is var _ ? ((ParseExprResult cond = parse_expr(st2)) is var _ ? (_p0_) => unwrap_expr_ok(cond(parse_if_then), _p0_) : default) : default);
    ...
    public static ParseExprResult parse_if_else(Expr c, Expr t, ParseState st) => ((ParseExprResult st2 = skip_newlines(st)) is var _ ? ((ParseExprResult st3 = expect(ElseKeyword, st2)) is var _ ? ((ParseExprResult st4 = skip_newlines(st3)) is var _ ? ((ParseExprResult else_result = parse_expr(st4)) is var _ ? (_p0_) => unwrap_expr_ok(else_result((_p0_) => (_p1_) => (_p2_) => finish_if(c(t), _p0_, _p1_, _p2_)), _p0_) : default) : default) : default) : default);
    ...
    public static ParseExprResult parse_match_expr(ParseState st) => ((ParseExprResult st2 = advance(st)) is var _ ? ((ParseExprResult scrut = parse_expr(st2)) is var _ ? (_p0_) => unwrap_expr_ok(scrut(start_match_branches), _p0_) : default) : default);

    public static ParseExprResult start_match_branches(Expr s, ParseState st) => ((ParseExprResult st2 = skip_newlines(st)) is var _ ? (_p0_) => (_p1_) => parse_match_branches(s(new List<object>())(st2), _p0_, _p1_) : default);
    ...
    public static ParseExprResult parse_one_match_branch(Expr scrut, List<MatchArm> acc, ParseState st) => ((ParseExprResult st2 = advance(st)) is var _ ? ((ParseExprResult pat = parse_pattern(st2)) is var _ ? (_p0_) => unwrap_pat_for_expr(pat((_p0_) => (_p1_) => (_p2_) => parse_match_branch_body(scrut(acc), _p0_, _p1_, _p2_)), _p0_) : default) : default);

    public static ParseExprResult parse_match_branch_body(Expr scrut, List<MatchArm> acc, Pat p, ParseState st) => ((ParseExprResult st2 = expect(Arrow, st)) is var _ ? ((ParseExprResult st3 = skip_newlines(st2)) is var _ ? ((ParseExprResult body = parse_expr(st3)) is var _ ? (_p0_) => unwrap_expr_ok(body((_p0_) => (_p1_) => (_p2_) => (_p3_) => finish_match_branch(scrut(acc(p)), _p0_, _p1_, _p2_, _p3_)), _p0_) : default) : default) : default);

    public static ParseExprResult finish_match_branch(Expr scrut, List<MatchArm> acc, Pat p, Expr b, ParseState st) => ((ParseExprResult arm = new MatchArm(pattern: p, body: b)) is var _ ? ((ParseExprResult st2 = skip_newlines(st)) is var _ ? (_p0_) => (_p1_) => parse_match_branches(scrut((acc + new List<object> { arm }))(st2), _p0_, _p1_) : default) : default);
    ...
    public static ParseExprResult parse_do_bind_stmt(List<DoStmt> acc, ParseState st) => ((ParseExprResult name_tok = current(st)) is var _ ? ((ParseExprResult st2 = advance(advance(st))) is var _ ? ((ParseExprResult val_result = parse_expr(st2)) is var _ ? (_p0_) => unwrap_expr_ok(val_result((_p0_) => (_p1_) => (_p2_) => finish_do_bind(acc(name_tok), _p0_, _p1_, _p2_)), _p0_) : default) : default) : default);

    public static ParseExprResult finish_do_bind(List<DoStmt> acc, Token name_tok, Expr v, ParseState st) => ((ParseExprResult st2 = skip_newlines(st)) is var _ ? parse_do_stmts((acc + new List<object> { new DoBindStmt(name_tok(v)) }), st2) : default);
    ...
    public static ParseExprResult finish_do_expr(List<DoStmt> acc, Expr e, ParseState st) => ((ParseExprResult st2 = skip_newlines(st)) is var _ ? parse_do_stmts((acc + new List<object> { new DoExprStmt(e) }), st2) : default);
    ...
    public static ParseDefResult try_parse_def(ParseState st) => (is_colon((_p0_) => peek_kind(st(1), _p0_)) ? ((ParseDefResult ann_result = parse_type_annotation(st)) is var _ ? unwrap_type_for_def(ann_result) : default) : parse_def_body_with_ann(new List<object>(), st));

    public static ParseDefResult unwrap_type_for_def(ParseTypeResult r) => r switch { TypeOk(var ann_type, var st) => ((ParseDefResult name_tok = new Token(kind: Identifier, text: "", offset: 0, line: 0, column: 0)) is var _ ? ((ParseDefResult ann = new List<object> { new TypeAnn(name: name_tok, type_expr: ann_type) }) is var _ ? ((ParseDefResult st2 = skip_newlines(st)) is var _ ? (_p0_) => parse_def_body_with_ann(ann(st2), _p0_) : default) : default) : default),  };

    public static ParseDefResult parse_def_body_with_ann(List<TypeAnn> ann, ParseState st) => ((ParseDefResult name_tok = current(st)) is var _ ? ((ParseDefResult st2 = advance(st)) is var _ ? (_p0_) => (_p1_) => (_p2_) => parse_def_params_then(ann(name_tok(new List<object>())(st2)), _p0_, _p1_, _p2_) : default) : default);

    public static ParseDefResult parse_def_params_then(List<TypeAnn> ann, Token name_tok, List<Token> acc, ParseState st) => (is_left_paren(current_kind(st)) ? ((ParseDefResult st2 = advance(st)) is var _ ? (is_ident(current_kind(st2)) ? ((ParseDefResult param = current(st2)) is var _ ? ((ParseDefResult st3 = advance(st2)) is var _ ? ((ParseDefResult st4 = expect(RightParen, st3)) is var _ ? (_p0_) => (_p1_) => (_p2_) => parse_def_params_then(ann(name_tok((acc + new List<object> { param }))(st4)), _p0_, _p1_, _p2_) : default) : default) : default) : (_p0_) => (_p1_) => (_p2_) => finish_def(ann(name_tok(acc(st))), _p0_, _p1_, _p2_)) : default) : (_p0_) => (_p1_) => (_p2_) => finish_def(ann(name_tok(acc(st))), _p0_, _p1_, _p2_));

    public static ParseDefResult finish_def(List<TypeAnn> ann, Token name_tok, List<Token> @params, ParseState st) => ((ParseDefResult st2 = expect(Equals, st)) is var _ ? ((ParseDefResult st3 = skip_newlines(st2)) is var _ ? ((ParseDefResult body_result = parse_expr(st3)) is var _ ? (_p0_) => (_p1_) => (_p2_) => unwrap_def_body(body_result(ann(name_tok(@params))), _p0_, _p1_, _p2_) : default) : default) : default);
    ...
    public static ParseTypeDefResult parse_record_type(Token name_tok, ParseState st) => ((ParseTypeDefResult st2 = advance(st)) is var _ ? ((ParseTypeDefResult st3 = expect(LeftBrace, st2)) is var _ ? ((ParseTypeDefResult st4 = skip_newlines(st3)) is var _ ? (_p0_) => (_p1_) => parse_record_fields_loop(name_tok(new List<object>())(st4), _p0_, _p1_) : default) : default) : default);
    ...
    public static ParseTypeDefResult parse_one_record_field(Token name_tok, List<RecordFieldDef> acc, ParseState st) => ((ParseTypeDefResult field_name = current(st)) is var _ ? ((ParseTypeDefResult st2 = advance(st)) is var _ ? ((ParseTypeDefResult st3 = expect(Colon, st2)) is var _ ? ((ParseTypeDefResult field_type_result = parse_type(st3)) is var _ ? (_p0_) => (_p1_) => (_p2_) => unwrap_record_field_type(name_tok(acc(field_name(field_type_result))), _p0_, _p1_, _p2_) : default) : default) : default) : default);
    ...
    public static ParseTypeDefResult parse_variant_ctors(Token name_tok, List<VariantCtorDef> acc, ParseState st) => (is_pipe(current_kind(st)) ? ((ParseTypeDefResult st2 = skip_newlines(advance(st))) is var _ ? ((ParseTypeDefResult ctor_name = current(st2)) is var _ ? ((ParseTypeDefResult st3 = advance(st2)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => parse_ctor_fields(ctor_name(new List<object>())(st3(name_tok(acc))), _p0_, _p1_, _p2_, _p3_) : default) : default) : default) : new TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: new VariantBody(acc)), st));

    public static ParseTypeDefResult parse_ctor_fields(Token ctor_name, List<TypeExpr> fields, ParseState st, Token name_tok, List<VariantCtorDef> acc) => (is_left_paren(current_kind(st)) ? ((ParseTypeDefResult field_result = parse_type(advance(st))) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => unwrap_ctor_field(field_result(ctor_name(fields(name_tok(acc)))), _p0_, _p1_, _p2_, _p3_) : default) : ((ParseTypeDefResult st2 = skip_newlines(st)) is var _ ? ((ParseTypeDefResult ctor = new VariantCtorDef(name: ctor_name, fields: fields)) is var _ ? (_p0_) => (_p1_) => parse_variant_ctors(name_tok((acc + new List<object> { ctor }))(st2), _p0_, _p1_) : default) : default));

    public static ParseTypeDefResult unwrap_ctor_field(ParseTypeResult r, Token ctor_name, List<TypeExpr> fields, Token name_tok, List<VariantCtorDef> acc) => r switch { TypeOk(var ty, var st) => ((ParseTypeDefResult st2 = expect(RightParen, st)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => parse_ctor_fields(ctor_name((fields + new List<object> { ty }))(st2(name_tok(acc))), _p0_, _p1_, _p2_, _p3_) : default),  };
    ...
    public static Document try_top_level_type_def(List<Def> defs, List<TypeDef> type_defs, ParseState st) => ((Document td_result = parse_type_def(st)) is var _ ? td_result switch { TypeDefOk(var td, var st2) => (_p0_) => (_p1_) => parse_top_level(defs((type_defs + new List<object> { td }))(skip_newlines(st2)), _p0_, _p1_), TypeDefNone(var st2) => (_p0_) => (_p1_) => try_top_level_def(defs(type_defs(st)), _p0_, _p1_),  } : default);

    public static Document try_top_level_def(List<Def> defs, List<TypeDef> type_defs, ParseState st) => ((Document def_result = parse_definition(st)) is var _ ? def_result switch { DefOk(var d, var st2) => (_p0_) => parse_top_level((defs + new List<object> { d }), type_defs(skip_newlines(st2)), _p0_), DefNone(var st2) => (_p0_) => (_p1_) => parse_top_level(defs(type_defs(skip_newlines(advance(st2)))), _p0_, _p1_),  } : default);
    ...
    public static CheckResult infer_name(UnificationState st, TypeEnv env, string name) => ((_p0_) => env_has(env(name), _p0_) ? ((CheckResult raw = (_p0_) => env_lookup(env(name), _p0_)) is var _ ? ((CheckResult inst = (_p0_) => instantiate_type(st(raw), _p0_)) is var _ ? new CheckResult(inferred_type: inst.var_type, state: inst.state) : default) : default) : new CheckResult(inferred_type: ErrorTy, state: (_p0_) => (_p1_) => add_unify_error(st("CDX2002")(("Unknown name: " + name)), _p0_, _p1_)));

    public static FreshResult instantiate_type(UnificationState st, CodexType ty) => ty switch { ForAllTy(var var_id, var body) => ((FreshResult fr = fresh_and_advance(st)) is var _ ? ((FreshResult substituted = (_p0_) => (_p1_) => subst_type_var(body(var_id(fr.var_type)), _p0_, _p1_)) is var _ ? (_p0_) => instantiate_type(fr.state(substituted), _p0_) : default) : default), _ => new FreshResult(var_type: ty, state: st),  };
    ...
    public static List<CodexType> map_subst_type_var(List<CodexType> args, long var_id, CodexType replacement, long i, long len, List<CodexType> acc) => ((i == len) ? acc : (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => map_subst_type_var(args(var_id(replacement((i + 1))(len((acc + new List<CodexType> { (_p0_) => subst_type_var(list_at(args(i)), var_id(replacement), _p0_) }))))), _p0_, _p1_, _p2_, _p3_, _p4_));

    public static CheckResult infer_binary(UnificationState st, TypeEnv env, AExpr left, BinaryOp op, AExpr right) => ((CheckResult lr = (_p0_) => (_p1_) => infer_expr(st(env(left)), _p0_, _p1_)) is var _ ? ((CheckResult rr = (_p0_) => (_p1_) => infer_expr(lr.state(env(right)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => infer_binary_op(rr.state(lr.inferred_type(rr.inferred_type(op))), _p0_, _p1_, _p2_) : default) : default);
    ...
    public static CheckResult infer_arithmetic(UnificationState st, CodexType lt, CodexType rt) => ((CheckResult r = (_p0_) => (_p1_) => unify(st(lt(rt)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: lt, state: r.state) : default);

    public static CheckResult infer_comparison(UnificationState st, CodexType lt, CodexType rt) => ((CheckResult r = (_p0_) => (_p1_) => unify(st(lt(rt)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: BooleanTy, state: r.state) : default);

    public static CheckResult infer_logical(UnificationState st, CodexType lt, CodexType rt) => ((CheckResult r1 = (_p0_) => (_p1_) => unify(st(lt(BooleanTy)), _p0_, _p1_)) is var _ ? ((CheckResult r2 = (_p0_) => (_p1_) => unify(r1.state(rt(BooleanTy)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: BooleanTy, state: r2.state) : default) : default);

    public static CheckResult infer_append(UnificationState st, CodexType lt, CodexType rt) => ((CheckResult resolved = (_p0_) => resolve(st(lt), _p0_)) is var _ ? resolved switch { TextTy { } => ((CheckResult r = (_p0_) => (_p1_) => unify(st(rt(TextTy)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: TextTy, state: r.state) : default), _ => ((CheckResult r = (_p0_) => (_p1_) => unify(st(lt(rt)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: lt, state: r.state) : default),  } : default);

    public static CheckResult infer_cons(UnificationState st, CodexType lt, CodexType rt) => ((CheckResult list_ty = new ListTy(lt)) is var _ ? ((CheckResult r = (_p0_) => (_p1_) => unify(st(rt(list_ty)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: list_ty, state: r.state) : default) : default);

    public static CheckResult infer_if(UnificationState st, TypeEnv env, AExpr cond, AExpr then_e, AExpr else_e) => ((CheckResult cr = (_p0_) => (_p1_) => infer_expr(st(env(cond)), _p0_, _p1_)) is var _ ? ((CheckResult r1 = (_p0_) => (_p1_) => unify(cr.state(cr.inferred_type(BooleanTy)), _p0_, _p1_)) is var _ ? ((CheckResult tr = (_p0_) => (_p1_) => infer_expr(r1.state(env(then_e)), _p0_, _p1_)) is var _ ? ((CheckResult er = (_p0_) => (_p1_) => infer_expr(tr.state(env(else_e)), _p0_, _p1_)) is var _ ? ((CheckResult r2 = (_p0_) => (_p1_) => unify(er.state(tr.inferred_type(er.inferred_type)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: tr.inferred_type, state: r2.state) : default) : default) : default) : default) : default);

    public static CheckResult infer_let(UnificationState st, TypeEnv env, List<ALetBind> bindings, AExpr body) => ((CheckResult env2 = (_p0_) => (_p1_) => (_p2_) => (_p3_) => infer_let_bindings(st(env(bindings(0)(list_length(bindings)))), _p0_, _p1_, _p2_, _p3_)) is var _ ? (_p0_) => (_p1_) => infer_expr(env2.state(env2.env(body)), _p0_, _p1_) : default);

    public static LetBindResult infer_let_bindings(UnificationState st, TypeEnv env, List<ALetBind> bindings, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((LetBindResult b = list_at(bindings(i))) is var _ ? ((LetBindResult vr = (_p0_) => (_p1_) => infer_expr(st(env(b.value)), _p0_, _p1_)) is var _ ? ((LetBindResult env2 = (_p0_) => (_p1_) => env_bind(env(b.name.value(vr.inferred_type)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => infer_let_bindings(vr.state(env2(bindings((i + 1))(len))), _p0_, _p1_, _p2_, _p3_) : default) : default) : default));

    public static CheckResult infer_lambda(UnificationState st, TypeEnv env, List<Name> @params, AExpr body) => ((CheckResult pr = (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_lambda_params(st(env(@params(0)(list_length(@params))(new List<object>()))), _p0_, _p1_, _p2_, _p3_, _p4_)) is var _ ? ((CheckResult br = (_p0_) => (_p1_) => infer_expr(pr.state(pr.env(body)), _p0_, _p1_)) is var _ ? ((CheckResult fun_ty = (_p0_) => wrap_fun_type(pr.param_types(br.inferred_type), _p0_)) is var _ ? new CheckResult(inferred_type: fun_ty, state: br.state) : default) : default) : default);
    ...
    public static CheckResult infer_application(UnificationState st, TypeEnv env, AExpr func, AExpr arg) => ((CheckResult fr = (_p0_) => (_p1_) => infer_expr(st(env(func)), _p0_, _p1_)) is var _ ? ((CheckResult ar = (_p0_) => (_p1_) => infer_expr(fr.state(env(arg)), _p0_, _p1_)) is var _ ? ((CheckResult ret = fresh_and_advance(ar.state)) is var _ ? ((CheckResult r = (_p0_) => (_p1_) => unify(ret.state(fr.inferred_type(new FunTy(ar.inferred_type(ret.var_type)))), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: ret.var_type, state: r.state) : default) : default) : default) : default);

    public static CheckResult infer_list(UnificationState st, TypeEnv env, List<AExpr> elems) => ((list_length(elems) == 0) ? ((CheckResult fr = fresh_and_advance(st)) is var _ ? new CheckResult(inferred_type: new ListTy(fr.var_type), state: fr.state) : default) : ((CheckResult first = (_p0_) => (_p1_) => infer_expr(st(env(list_at(elems(0)))), _p0_, _p1_)) is var _ ? ((CheckResult st2 = (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => unify_list_elems(first.state(env(elems(first.inferred_type(1)(list_length(elems))))), _p0_, _p1_, _p2_, _p3_, _p4_)) is var _ ? new CheckResult(inferred_type: new ListTy(first.inferred_type), state: st2) : default) : default));

    public static UnificationState unify_list_elems(UnificationState st, TypeEnv env, List<AExpr> elems, CodexType elem_ty, long i, long len) => ((i == len) ? st : ((UnificationState er = (_p0_) => (_p1_) => infer_expr(st(env(list_at(elems(i)))), _p0_, _p1_)) is var _ ? ((UnificationState r = (_p0_) => (_p1_) => unify(er.state(er.inferred_type(elem_ty)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => unify_list_elems(r.state(env(elems(elem_ty((i + 1))(len)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default));

    public static CheckResult infer_match(UnificationState st, TypeEnv env, AExpr scrutinee, List<AMatchArm> arms) => ((CheckResult sr = (_p0_) => (_p1_) => infer_expr(st(env(scrutinee)), _p0_, _p1_)) is var _ ? ((CheckResult fr = fresh_and_advance(sr.state)) is var _ ? ((CheckResult st2 = (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => (_p5_) => infer_match_arms(fr.state(env(sr.inferred_type(fr.var_type(arms(0)(list_length(arms)))))), _p0_, _p1_, _p2_, _p3_, _p4_, _p5_)) is var _ ? new CheckResult(inferred_type: fr.var_type, state: st2) : default) : default) : default);

    public static UnificationState infer_match_arms(UnificationState st, TypeEnv env, CodexType scrut_ty, CodexType result_ty, List<AMatchArm> arms, long i, long len) => ((i == len) ? st : ((UnificationState arm = list_at(arms(i))) is var _ ? ((UnificationState pr = (_p0_) => (_p1_) => (_p2_) => bind_pattern(st(env(arm.pattern(scrut_ty))), _p0_, _p1_, _p2_)) is var _ ? ((UnificationState br = (_p0_) => (_p1_) => infer_expr(pr.state(pr.env(arm.body)), _p0_, _p1_)) is var _ ? ((UnificationState r = (_p0_) => (_p1_) => unify(br.state(br.inferred_type(result_ty)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => (_p5_) => infer_match_arms(r.state(env(scrut_ty(result_ty(arms((i + 1))(len))))), _p0_, _p1_, _p2_, _p3_, _p4_, _p5_) : default) : default) : default) : default));

    public static PatBindResult bind_pattern(UnificationState st, TypeEnv env, APat pat, CodexType ty) => pat switch { AVarPat(var name) => new PatBindResult(state: st, env: (_p0_) => (_p1_) => env_bind(env(name.value(ty)), _p0_, _p1_)), AWildPat { } => new PatBindResult(state: st, env: env), ALitPat(var val, var kind) => new PatBindResult(state: st, env: env), ACtorPat(var ctor_name, var sub_pats) => ((PatBindResult ctor_lookup = (_p0_) => instantiate_type(st((_p0_) => env_lookup(env(ctor_name.value), _p0_)), _p0_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_ctor_sub_patterns(ctor_lookup.state(env(sub_pats(ctor_lookup.var_type(0)(list_length(sub_pats))))), _p0_, _p1_, _p2_, _p3_, _p4_) : default),  };

    public static PatBindResult bind_ctor_sub_patterns(UnificationState st, TypeEnv env, List<APat> sub_pats, CodexType ctor_ty, long i, long len) => ((i == len) ? new PatBindResult(state: st, env: env) : ctor_ty switch { FunTy(var param_ty, var ret_ty) => ((PatBindResult pr = (_p0_) => (_p1_) => (_p2_) => bind_pattern(st(env(list_at(sub_pats(i)))(param_ty)), _p0_, _p1_, _p2_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_ctor_sub_patterns(pr.state(pr.env(sub_pats(ret_ty((i + 1))(len)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default), _ => ((PatBindResult fr = fresh_and_advance(st)) is var _ ? ((PatBindResult pr = (_p0_) => (_p1_) => (_p2_) => bind_pattern(fr.state(env(list_at(sub_pats(i)))(fr.var_type)), _p0_, _p1_, _p2_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_ctor_sub_patterns(pr.state(pr.env(sub_pats(ctor_ty((i + 1))(len)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default),  });
    ...
    public static CheckResult infer_do_loop(UnificationState st, TypeEnv env, List<ADoStmt> stmts, long i, long len, CodexType last_ty) => ((i == len) ? new CheckResult(inferred_type: last_ty, state: st) : ((CheckResult stmt = list_at(stmts(i))) is var _ ? stmt switch { ADoExprStmt(var e) => ((CheckResult er = (_p0_) => (_p1_) => infer_expr(st(env(e)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => infer_do_loop(er.state(env(stmts((i + 1))(len(er.inferred_type)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default), ADoBindStmt(var name, var e) => ((CheckResult er = (_p0_) => (_p1_) => infer_expr(st(env(e)), _p0_, _p1_)) is var _ ? ((CheckResult env2 = (_p0_) => (_p1_) => env_bind(env(name.value(er.inferred_type)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => infer_do_loop(er.state(env2(stmts((i + 1))(len(er.inferred_type)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default),  } : default));

    public static CheckResult infer_expr(UnificationState st, TypeEnv env, AExpr expr) => expr switch { ALitExpr(var val, var kind) => (_p0_) => infer_literal(st(kind), _p0_), ANameExpr(var name) => (_p0_) => (_p1_) => infer_name(st(env(name.value)), _p0_, _p1_), ABinaryExpr(var left, var op, var right) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => infer_binary(st(env(left(op(right)))), _p0_, _p1_, _p2_, _p3_), AUnaryExpr(var operand) => ((CheckResult r = (_p0_) => (_p1_) => infer_expr(st(env(operand)), _p0_, _p1_)) is var _ ? ((CheckResult u = (_p0_) => (_p1_) => unify(r.state(r.inferred_type(IntegerTy)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: IntegerTy, state: u.state) : default) : default), AApplyExpr(var func, var arg) => (_p0_) => (_p1_) => (_p2_) => infer_application(st(env(func(arg))), _p0_, _p1_, _p2_), AIfExpr(var cond, var then_e, var else_e) => (_p0_) => (_p1_) => (_p2_) => (_p3_) => infer_if(st(env(cond(then_e(else_e)))), _p0_, _p1_, _p2_, _p3_), ALetExpr(var bindings, var body) => (_p0_) => (_p1_) => (_p2_) => infer_let(st(env(bindings(body))), _p0_, _p1_, _p2_), ALambdaExpr(var @params, var body) => (_p0_) => (_p1_) => (_p2_) => infer_lambda(st(env(@params(body))), _p0_, _p1_, _p2_), AMatchExpr(var scrutinee, var arms) => (_p0_) => (_p1_) => (_p2_) => infer_match(st(env(scrutinee(arms))), _p0_, _p1_, _p2_), AListExpr(var elems) => (_p0_) => (_p1_) => infer_list(st(env(elems)), _p0_, _p1_), ADoExpr(var stmts) => (_p0_) => (_p1_) => infer_do(st(env(stmts)), _p0_, _p1_), AFieldAccess(var obj, var field) => ((CheckResult r = (_p0_) => (_p1_) => infer_expr(st(env(obj)), _p0_, _p1_)) is var _ ? ((CheckResult resolved = (_p0_) => deep_resolve(r.state(r.inferred_type), _p0_)) is var _ ? resolved switch { RecordTy(var rname, var rfields) => ((CheckResult ftype = (_p0_) => lookup_record_field(rfields(field.value), _p0_)) is var _ ? new CheckResult(inferred_type: ftype, state: r.state) : default), _ => ((CheckResult fr = fresh_and_advance(r.state)) is var _ ? new CheckResult(inferred_type: fr.var_type, state: fr.state) : default), ARecordExpr(var name, var fields) => ((CheckResult st2 = (_p0_) => (_p1_) => (_p2_) => (_p3_) => infer_record_fields(st(env(fields(0)(list_length(fields)))), _p0_, _p1_, _p2_, _p3_)) is var _ ? ((CheckResult ctor_type = ((_p0_) => env_has(env(name.value), _p0_) ? (_p0_) => env_lookup(env(name.value), _p0_) : ErrorTy)) is var _ ? ((CheckResult result_type = strip_fun_args(ctor_type)) is var _ ? new CheckResult(inferred_type: result_type, state: st2) : default) : default) : default), AErrorExpr(var msg) => new CheckResult(inferred_type: ErrorTy, state: st),  } : default) : default),  };

    public static UnificationState infer_record_fields(UnificationState st, TypeEnv env, List<AFieldExpr> fields, long i, long len) => ((i == len) ? st : ((UnificationState f = list_at(fields(i))) is var _ ? ((UnificationState r = (_p0_) => (_p1_) => infer_expr(st(env(f.value)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => infer_record_fields(r.state(env(fields((i + 1))(len))), _p0_, _p1_, _p2_, _p3_) : default) : default));
    ...
    public static List<CodexType> resolve_type_expr_list(List<ATypeExpr> args, long i, long len, List<CodexType> acc) => ((i == len) ? acc : (_p0_) => (_p1_) => (_p2_) => resolve_type_expr_list(args((i + 1))(len((acc + new List<CodexType> { resolve_type_expr(list_at(args(i))) }))), _p0_, _p1_, _p2_));

    public static CodexType resolve_type_name(string name) => ((name == "Integer") ? IntegerTy : ((name == "Number") ? NumberTy : ((name == "Text") ? TextTy : ((name == "Boolean") ? BooleanTy : ((name == "Nothing") ? NothingTy : new ConstructedTy(new Name(value: name), new List<object>()))))));

    public static CheckResult check_def(UnificationState st, TypeEnv env, ADef def) => ((CheckResult declared = (_p0_) => (_p1_) => resolve_declared_type(st(env(def)), _p0_, _p1_)) is var _ ? ((CheckResult env2 = (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_def_params(declared.state(declared.env(def.@params(declared.expected_type(0)(list_length(def.@params))))), _p0_, _p1_, _p2_, _p3_, _p4_)) is var _ ? ((CheckResult body_r = (_p0_) => (_p1_) => infer_expr(env2.state(env2.env(def.body)), _p0_, _p1_)) is var _ ? ((CheckResult u = (_p0_) => (_p1_) => unify(body_r.state(env2.remaining_type(body_r.inferred_type)), _p0_, _p1_)) is var _ ? new CheckResult(inferred_type: declared.expected_type, state: u.state) : default) : default) : default) : default);
    ...
    public static DefParamResult bind_def_params(UnificationState st, TypeEnv env, List<AParam> @params, CodexType remaining, long i, long len) => ((i == len) ? new DefParamResult(state: st, env: env, remaining_type: remaining) : ((DefParamResult p = list_at(@params(i))) is var _ ? remaining switch { FunTy(var param_ty, var ret_ty) => ((DefParamResult env2 = (_p0_) => (_p1_) => env_bind(env(p.name.value(param_ty)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_def_params(st(env2(@params(ret_ty((i + 1))(len)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default), _ => ((DefParamResult fr = fresh_and_advance(st)) is var _ ? ((DefParamResult env2 = (_p0_) => (_p1_) => env_bind(env(p.name.value(fr.var_type)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => bind_def_params(fr.state(env2(@params(remaining((i + 1))(len)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default),  } : default));

    public static ModuleResult check_module(AModule mod) => ((ModuleResult tenv = (_p0_) => (_p1_) => (_p2_) => (_p3_) => register_type_defs(empty_unification_state(builtin_type_env(mod.type_defs(0)(list_length(mod.type_defs)))), _p0_, _p1_, _p2_, _p3_)) is var _ ? ((ModuleResult env = (_p0_) => (_p1_) => (_p2_) => (_p3_) => register_all_defs(tenv.state(tenv.env(mod.defs(0)(list_length(mod.defs)))), _p0_, _p1_, _p2_, _p3_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => check_all_defs(env.state(env.env(mod.defs(0)(list_length(mod.defs))(new List<object>()))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default);

    public static LetBindResult register_all_defs(UnificationState st, TypeEnv env, List<ADef> defs, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((LetBindResult def = list_at(defs(i))) is var _ ? ((LetBindResult ty = ((list_length(def.declared_type) == 0) ? ((object fr = fresh_and_advance(st)) is var _ ? ((object env2 = (_p0_) => (_p1_) => env_bind(env(def.name.value(fr.var_type)), _p0_, _p1_)) is var _ ? new LetBindResult(state: fr.state, env: env2) : default) : default) : ((object resolved = resolve_type_expr(list_at(def.declared_type(0)))) is var _ ? new LetBindResult(state: st, env: (_p0_) => (_p1_) => env_bind(env(def.name.value(resolved)), _p0_, _p1_)) : default))) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => register_all_defs(ty.state(ty.env(defs((i + 1))(len))), _p0_, _p1_, _p2_, _p3_) : default) : default));

    public static ModuleResult check_all_defs(UnificationState st, TypeEnv env, List<ADef> defs, long i, long len, List<TypeBinding> acc) => ((i == len) ? new ModuleResult(types: acc, state: st) : ((ModuleResult def = list_at(defs(i))) is var _ ? ((ModuleResult r = (_p0_) => (_p1_) => check_def(st(env(def)), _p0_, _p1_)) is var _ ? ((ModuleResult resolved = (_p0_) => deep_resolve(r.state(r.inferred_type), _p0_)) is var _ ? ((ModuleResult entry = new TypeBinding(name: def.name.value, bound_type: resolved)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => check_all_defs(r.state(env(defs((i + 1))(len((acc + new List<object> { entry }))))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default) : default) : default));

    public static LetBindResult register_type_defs(UnificationState st, TypeEnv env, List<ATypeDef> tdefs, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((LetBindResult td = list_at(tdefs(i))) is var _ ? ((LetBindResult r = (_p0_) => (_p1_) => register_one_type_def(st(env(td)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => register_type_defs(r.state(r.env(tdefs((i + 1))(len))), _p0_, _p1_, _p2_, _p3_) : default) : default));

    public static LetBindResult register_one_type_def(UnificationState st, TypeEnv env, ATypeDef td) => td switch { AVariantTypeDef(var name, var type_params, var ctors) => ((LetBindResult result_ty = new ConstructedTy(name(new List<object>()))) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => register_variant_ctors(st(env(ctors(result_ty(0)(list_length(ctors))))), _p0_, _p1_, _p2_, _p3_, _p4_) : default), ARecordTypeDef(var name, var type_params, var fields) => ((LetBindResult resolved_fields = (_p0_) => (_p1_) => (_p2_) => build_record_fields(fields(0)(list_length(fields))(new List<object>()), _p0_, _p1_, _p2_)) is var _ ? ((LetBindResult result_ty = new RecordTy(name(resolved_fields))) is var _ ? ((LetBindResult ctor_ty = (_p0_) => (_p1_) => (_p2_) => build_record_ctor_type(fields(result_ty(0)(list_length(fields))), _p0_, _p1_, _p2_)) is var _ ? new LetBindResult(state: st, env: (_p0_) => (_p1_) => env_bind(env(name.value(ctor_ty)), _p0_, _p1_)) : default) : default) : default),  };

    public static List<RecordField> build_record_fields(List<ARecordFieldDef> fields, long i, long len, List<RecordField> acc) => ((i == len) ? acc : ((List<RecordField> f = list_at(fields(i))) is var _ ? ((List<RecordField> rfield = new RecordField(name: f.name, type_val: resolve_type_expr(f.type_expr))) is var _ ? (_p0_) => (_p1_) => (_p2_) => build_record_fields(fields((i + 1))(len((acc + new List<RecordField> { rfield }))), _p0_, _p1_, _p2_) : default) : default));

    public static CodexType lookup_record_field(List<RecordField> fields, string name) => ((list_length(fields) == 0) ? ErrorTy : ((CodexType f = list_at(fields(0))) is var _ ? ((f.name.value == name) ? f.type_val : (_p0_) => (_p1_) => (_p2_) => lookup_record_field_loop(fields(name(1)(list_length(fields))), _p0_, _p1_, _p2_)) : default));

    public static CodexType lookup_record_field_loop(List<RecordField> fields, string name, long i, long len) => ((i == len) ? ErrorTy : ((CodexType f = list_at(fields(i))) is var _ ? ((f.name.value == name) ? f.type_val : (_p0_) => (_p1_) => (_p2_) => lookup_record_field_loop(fields(name((i + 1))(len)), _p0_, _p1_, _p2_)) : default));

    public static LetBindResult register_variant_ctors(UnificationState st, TypeEnv env, List<AVariantCtorDef> ctors, CodexType result_ty, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((LetBindResult ctor = list_at(ctors(i))) is var _ ? ((LetBindResult ctor_ty = (_p0_) => (_p1_) => (_p2_) => build_ctor_type(ctor.fields(result_ty(0)(list_length(ctor.fields))), _p0_, _p1_, _p2_)) is var _ ? ((LetBindResult env2 = (_p0_) => (_p1_) => env_bind(env(ctor.name.value(ctor_ty)), _p0_, _p1_)) is var _ ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => (_p4_) => register_variant_ctors(st(env2(ctors(result_ty((i + 1))(len)))), _p0_, _p1_, _p2_, _p3_, _p4_) : default) : default) : default));

    public static CodexType build_ctor_type(List<ATypeExpr> fields, CodexType result, long i, long len) => ((i == len) ? result : ((CodexType rest = (_p0_) => (_p1_) => (_p2_) => build_ctor_type(fields(result((i + 1))(len)), _p0_, _p1_, _p2_)) is var _ ? new FunTy(resolve_type_expr(list_at(fields(i))), rest) : default));

    public static CodexType build_record_ctor_type(List<ARecordFieldDef> fields, CodexType result, long i, long len) => ((i == len) ? result : ((CodexType f = list_at(fields(i))) is var _ ? ((CodexType rest = (_p0_) => (_p1_) => (_p2_) => build_record_ctor_type(fields(result((i + 1))(len)), _p0_, _p1_, _p2_)) is var _ ? new FunTy(resolve_type_expr(f.type_expr), rest) : default) : default));
    ...
    public static CodexType env_lookup_loop(List<TypeBinding> bindings, string name, long i, long len) => ((i == len) ? ErrorTy : ((CodexType b = list_at(bindings(i))) is var _ ? ((b.name == name) ? b.bound_type : (_p0_) => (_p1_) => (_p2_) => env_lookup_loop(bindings(name((i + 1))(len)), _p0_, _p1_, _p2_)) : default));
    ...
    public static bool env_has_loop(List<TypeBinding> bindings, string name, long i, long len) => ((i == len) ? false : ((bool b = list_at(bindings(i))) is var _ ? ((b.name == name) ? true : (_p0_) => (_p1_) => (_p2_) => env_has_loop(bindings(name((i + 1))(len)), _p0_, _p1_, _p2_)) : default));
    ...
    public static TypeEnv builtin_type_env() => ((TypeEnv e = empty_type_env) is var _ ? ((TypeEnv e2 = (_p0_) => (_p1_) => env_bind(e("negate")(new FunTy(IntegerTy, IntegerTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e3 = (_p0_) => (_p1_) => env_bind(e2("text-length")(new FunTy(TextTy, IntegerTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e4 = (_p0_) => (_p1_) => env_bind(e3("integer-to-text")(new FunTy(IntegerTy, TextTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e5 = (_p0_) => (_p1_) => env_bind(e4("char-at")(new FunTy(TextTy, new FunTy(IntegerTy, TextTy))), _p0_, _p1_)) is var _ ? ((TypeEnv e6 = (_p0_) => (_p1_) => env_bind(e5("substring")(new FunTy(TextTy, new FunTy(IntegerTy, new FunTy(IntegerTy, TextTy)))), _p0_, _p1_)) is var _ ? ((TypeEnv e7 = (_p0_) => (_p1_) => env_bind(e6("is-letter")(new FunTy(TextTy, BooleanTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e8 = (_p0_) => (_p1_) => env_bind(e7("is-digit")(new FunTy(TextTy, BooleanTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e9 = (_p0_) => (_p1_) => env_bind(e8("is-whitespace")(new FunTy(TextTy, BooleanTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e10 = (_p0_) => (_p1_) => env_bind(e9("char-code")(new FunTy(TextTy, IntegerTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e11 = (_p0_) => (_p1_) => env_bind(e10("code-to-char")(new FunTy(IntegerTy, TextTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e12 = (_p0_) => (_p1_) => env_bind(e11("text-replace")(new FunTy(TextTy, new FunTy(TextTy, new FunTy(TextTy, TextTy)))), _p0_, _p1_)) is var _ ? ((TypeEnv e13 = (_p0_) => (_p1_) => env_bind(e12("text-to-integer")(new FunTy(TextTy, IntegerTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e14 = (_p0_) => (_p1_) => env_bind(e13("show")(new ForAllTy(0, new FunTy(new TypeVar(0), TextTy))), _p0_, _p1_)) is var _ ? ((TypeEnv e15 = (_p0_) => (_p1_) => env_bind(e14("print-line")(new FunTy(TextTy, NothingTy)), _p0_, _p1_)) is var _ ? ((TypeEnv e16 = (_p0_) => (_p1_) => env_bind(e15("list-length")(new ForAllTy(0, new FunTy(new ListTy(new TypeVar(0)), IntegerTy))), _p0_, _p1_)) is var _ ? ((TypeEnv e17 = (_p0_) => (_p1_) => env_bind(e16("list-at")(new ForAllTy(0, new FunTy(new ListTy(new TypeVar(0)), new FunTy(IntegerTy, new TypeVar(0))))), _p0_, _p1_)) is var _ ? ((TypeEnv e18 = (_p0_) => (_p1_) => env_bind(e17("map")(new ForAllTy(0, new ForAllTy(1, new FunTy(new FunTy(new TypeVar(0), new TypeVar(1)), new FunTy(new ListTy(new TypeVar(0)), new ListTy(new TypeVar(1))))))), _p0_, _p1_)) is var _ ? ((TypeEnv e19 = (_p0_) => (_p1_) => env_bind(e18("filter")(new ForAllTy(0, new FunTy(new FunTy(new TypeVar(0), BooleanTy), new FunTy(new ListTy(new TypeVar(0)), new ListTy(new TypeVar(0)))))), _p0_, _p1_)) is var _ ? ((TypeEnv e20 = (_p0_) => (_p1_) => env_bind(e19("fold")(new ForAllTy(0, new ForAllTy(1, new FunTy(new FunTy(new TypeVar(1), new FunTy(new TypeVar(0), new TypeVar(1))), new FunTy(new TypeVar(1), new FunTy(new ListTy(new TypeVar(0)), new TypeVar(1))))))), _p0_, _p1_)) is var _ ? ((TypeEnv e21 = (_p0_) => (_p1_) => env_bind(e20("read-line")(TextTy), _p0_, _p1_)) is var _ ? e21 : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default);
    ...
    public static CodexType subst_lookup_loop(long var_id, List<SubstEntry> entries, long i, long len) => ((i == len) ? ErrorTy : ((CodexType entry = list_at(entries(i))) is var _ ? ((entry.var_id == var_id) ? entry.resolved_type : (_p0_) => (_p1_) => (_p2_) => subst_lookup_loop(var_id(entries((i + 1))(len)), _p0_, _p1_, _p2_)) : default));
    ...
    public static bool has_subst_loop(long var_id, List<SubstEntry> entries, long i, long len) => ((i == len) ? false : ((bool entry = list_at(entries(i))) is var _ ? ((entry.var_id == var_id) ? true : (_p0_) => (_p1_) => (_p2_) => has_subst_loop(var_id(entries((i + 1))(len)), _p0_, _p1_, _p2_)) : default));
    ...
    public static bool occurs_in(UnificationState st, long var_id, CodexType ty) => ((bool resolved = (_p0_) => resolve(st(ty), _p0_)) is var _ ? resolved switch { TypeVar(var id) => (id == var_id), FunTy(var param, var ret) => ((_p0_) => (_p1_) => occurs_in(st(var_id(param)), _p0_, _p1_) || (_p0_) => (_p1_) => occurs_in(st(var_id(ret)), _p0_, _p1_)), ListTy(var elem) => (_p0_) => (_p1_) => occurs_in(st(var_id(elem)), _p0_, _p1_), _ => false,  } : default);

    public static UnifyResult unify(UnificationState st, CodexType a, CodexType b) => ((UnifyResult ra = (_p0_) => resolve(st(a), _p0_)) is var _ ? ((UnifyResult rb = (_p0_) => resolve(st(b), _p0_)) is var _ ? (_p0_) => (_p1_) => unify_resolved(st(ra(rb)), _p0_, _p1_) : default) : default);
    ...
    public static UnifyResult unify_constructed_args(UnificationState st, List<CodexType> args_a, List<CodexType> args_b, long i, long len) => ((i == len) ? new UnifyResult(success: true, state: st) : ((i >= list_length(args_b)) ? new UnifyResult(success: true, state: st) : ((UnifyResult r = (_p0_) => (_p1_) => unify(st(list_at(args_a(i)))(list_at(args_b(i))), _p0_, _p1_)) is var _ ? (r.success ? (_p0_) => (_p1_) => (_p2_) => (_p3_) => unify_constructed_args(r.state(args_a(args_b((i + 1))(len))), _p0_, _p1_, _p2_, _p3_) : r) : default)));

    public static UnifyResult unify_fun(UnificationState st, CodexType pa, CodexType ra, CodexType pb, CodexType rb) => ((UnifyResult r1 = (_p0_) => (_p1_) => unify(st(pa(pb)), _p0_, _p1_)) is var _ ? (r1.success ? (_p0_) => (_p1_) => unify(r1.state(ra(rb)), _p0_, _p1_) : r1) : default);
    ...
    public static CodexType deep_resolve(UnificationState st, CodexType ty) => ((CodexType resolved = (_p0_) => resolve(st(ty), _p0_)) is var _ ? resolved switch { FunTy(var param, var ret) => new FunTy((_p0_) => deep_resolve(st(param), _p0_), (_p0_) => deep_resolve(st(ret), _p0_)), ListTy(var elem) => new ListTy((_p0_) => deep_resolve(st(elem), _p0_)), ConstructedTy(var name, var args) => new ConstructedTy(name((_p0_) => (_p1_) => (_p2_) => (_p3_) => deep_resolve_list(st(args(0)(list_length(args))(new List<object>())), _p0_, _p1_, _p2_, _p3_))), ForAllTy(var id, var body) => new ForAllTy(id((_p0_) => deep_resolve(st(body), _p0_))), SumTy(var name, var ctors) => resolved, RecordTy(var name, var fields) => resolved, _ => resolved,  } : default);

    public static List<CodexType> deep_resolve_list(UnificationState st, List<CodexType> args, long i, long len, List<CodexType> acc) => ((i == len) ? acc : (_p0_) => (_p1_) => (_p2_) => (_p3_) => deep_resolve_list(st(args((i + 1))(len((acc + new List<CodexType> { (_p0_) => deep_resolve(st(list_at(args(i))), _p0_) })))), _p0_, _p1_, _p2_, _p3_));

    public static string compile(string source, string module_name) => ((string tokens = tokenize(source)) is var _ ? ((string st = make_parse_state(tokens)) is var _ ? ((string doc = parse_document(st)) is var _ ? ((string ast = (_p0_) => desugar_document(doc(module_name), _p0_)) is var _ ? ((string check_result = check_module(ast)) is var _ ? ((string ir = (_p0_) => (_p1_) => lower_module(ast(check_result.types(check_result.state)), _p0_, _p1_)) is var _ ? (_p0_) => emit_full_module(ir(ast.type_defs), _p0_) : default) : default) : default) : default) : default) : default);

    public static CompileResult compile_checked(string source, string module_name) => ((CompileResult tokens = tokenize(source)) is var _ ? ((CompileResult st = make_parse_state(tokens)) is var _ ? ((CompileResult doc = parse_document(st)) is var _ ? ((CompileResult ast = (_p0_) => desugar_document(doc(module_name), _p0_)) is var _ ? ((CompileResult resolve_result = resolve_module(ast)) is var _ ? ((list_length(resolve_result.errors) > 0) ? new CompileError(resolve_result.errors) : ((CompileResult check_result = check_module(ast)) is var _ ? ((CompileResult ir = (_p0_) => (_p1_) => lower_module(ast(check_result.types(check_result.state)), _p0_, _p1_)) is var _ ? new CompileOk((_p0_) => emit_full_module(ir(ast.type_defs), _p0_), check_result) : default) : default)) : default) : default) : default) : default) : default);

    public static string test_source() => "square : Integer -> Integer\\nsquare (x) = x * x\\nmain = square 5";

    public static [ Console() => Nothing;

    public static T3916 main<T3916>() => { print_line((_p0_) => compile(test_source("test"), _p0_));  };
