...
public static class Codex_Codex_Codex
{
    ...
    public static CodexType binary_result_type(BinaryOp op, CodexType left_ty, CodexType expected_ty) => op switch { OpEq { } => BooleanTy, OpNotEq { } => BooleanTy, OpLt { } => BooleanTy, OpGt { } => BooleanTy, OpLtEq { } => BooleanTy, OpGtEq { } => BooleanTy, OpDefEq { } => BooleanTy, OpAnd { } => BooleanTy, OpOr { } => BooleanTy, OpAppend { } => (is_text_type(left_ty) ? TextTy : (is_text_type(expected_ty) ? TextTy : left_ty)), _ => left_ty,  };

    public static bool is_text_type(CodexType ty) => ty switch { TextTy { } => true, _ => false,  };

    public static IRExpr lower_expr(AExpr e, CodexType ty, LowerCtx ctx) => e switch { ALitExpr(var text, var kind) => lower_literal(text, kind), ANameExpr(var name) => lower_name(name.value, ty, ctx), AApplyExpr(var f, var a) => lower_apply(f, a, ty, ctx), ABinaryExpr(var l, var op, var r) => ((IRExpr left_ir = lower_expr(l, ty, ctx)) is var _ ? ((CodexType left_ty = ir_expr_type(left_ir)) is var _ ? ((IRExpr right_ir = lower_expr(r, ty, ctx)) is var _ ? new IrBinary(lower_bin_op(op, left_ty), left_ir, right_ir, binary_result_type(op, left_ty, ty)) : default) : default) : default), AUnaryExpr(var operand) => new IrNegate(lower_expr(operand, IntegerTy, ctx)), AIfExpr(var c, var t, var e2) => ((IRExpr then_ir = lower_expr(t, ty, ctx)) is var _ ? ((CodexType then_ty = ir_expr_type(then_ir)) is var _ ? ((CodexType result_ty = ty switch { ErrorTy { } => then_ty, _ => ty,  }) is var _ ? ((IRExpr else_ir = lower_expr(e2, result_ty, ctx)) is var _ ? new IrIf(lower_expr(c, BooleanTy, ctx), then_ir, else_ir, result_ty) : default) : default) : default) : default), ALetExpr(var binds, var body) => lower_let(binds, body, ty, ctx), ALambdaExpr(var @params, var body) => lower_lambda(@params, body, ty, ctx), AMatchExpr(var scrut, var arms) => lower_match(scrut, arms, ty, ctx), AListExpr(var elems) => lower_list(elems, ty, ctx), ARecordExpr(var name, var fields) => lower_record(name, fields, ty, ctx), AFieldAccess(var rec, var field) => ((IRExpr rec_ir = lower_expr(rec, ErrorTy, ctx)) is var _ ? ((CodexType rec_ty = deep_resolve(ctx.ust, ir_expr_type(rec_ir))) is var _ ? ((CodexType field_ty = rec_ty switch { RecordTy(var rname, var rfields) => lookup_record_field(rfields, field.value), ConstructedTy(var cname, var cargs) => ((CodexType ctor_raw = lookup_type(ctx.types, cname.value)) is var _ ? ((CodexType resolved_record = ctor_raw switch { ErrorTy { } => ErrorTy, _ => strip_fun_args_lower(deep_resolve(ctx.ust, ctor_raw)),  }) is var _ ? resolved_record switch { RecordTy(var rn, var rf) => lookup_record_field(rf, field.value), _ => ty, _ => ty,  } : default) : default),  }) is var _ ? ((CodexType actual_field_ty = field_ty switch { ErrorTy { } => ty, _ => field_ty,  }) is var _ ? new IrFieldAccess(rec_ir, field.value, actual_field_ty) : default) : default) : default) : default), ADoExpr(var stmts) => lower_do(stmts, ty, ctx), AErrorExpr(var msg) => new IrError(msg, ty),  };
    ...
    public static IRExpr lower_apply(AExpr f, AExpr a, CodexType ty, LowerCtx ctx) => ((IRExpr func_ir = lower_expr(f, ErrorTy, ctx)) is var _ ? ((CodexType func_ty = deep_resolve(ctx.ust, ir_expr_type(func_ir))) is var _ ? ((CodexType arg_ty = peel_fun_param(func_ty)) is var _ ? ((CodexType ret_ty = peel_fun_return(func_ty)) is var _ ? ((IRExpr arg_ir = lower_expr(a, arg_ty, ctx)) is var _ ? ((CodexType resolved_ret = subst_type_vars_from_arg(arg_ty, ir_expr_type(arg_ir), ret_ty)) is var _ ? ((CodexType actual_ret = resolved_ret switch { ErrorTy { } => ty, _ => resolved_ret,  }) is var _ ? new IrApply(func_ir, arg_ir, actual_ret) : default) : default) : default) : default) : default) : default) : default);
    ...
    public static IRExpr lower_match(AExpr scrut, List<AMatchArm> arms, CodexType ty, LowerCtx ctx) => ((IRExpr scrut_ir = lower_expr(scrut, ErrorTy, ctx)) is var _ ? ((CodexType scrut_ty = ir_expr_type(scrut_ir)) is var _ ? ((List<IRBranch> branches = lower_match_arms_loop(arms, ty, scrut_ty, ctx, 0, list_length(arms))) is var _ ? ((CodexType result_ty = ty switch { ErrorTy { } => infer_match_type(branches, 0, list_length(branches)), _ => ty,  }) is var _ ? new IrMatch(scrut_ir, branches, result_ty) : default) : default) : default) : default);

    public static CodexType infer_match_type(List<IRBranch> branches, long i, long len) => ((i == len) ? ErrorTy : ((IRBranch b = list_at(branches)(i)) is var _ ? ((CodexType body_ty = ir_expr_type(b.body)) is var _ ? body_ty switch { ErrorTy { } => infer_match_type(branches, (i + 1), len), _ => body_ty,  } : default) : default));
    ...
    public static IRExpr lower_list(List<AExpr> elems, CodexType ty, LowerCtx ctx) => ((CodexType elem_ty = ty switch { ListTy(var e) => e, _ => ((list_length(elems) == 0) ? ErrorTy : ir_expr_type(lower_expr(list_at(elems)(0), ErrorTy, ctx))),  }) is var _ ? new IrList(lower_list_elems_loop(elems, elem_ty, ctx, 0, list_length(elems)), elem_ty) : default);
    ...
    public static IRExpr lower_record(Name name, List<AFieldExpr> fields, CodexType ty, LowerCtx ctx) => ((CodexType ctor_raw = lookup_type(ctx.types, name.value)) is var _ ? ((CodexType record_ty = ctor_raw switch { ErrorTy { } => ty, _ => strip_fun_args_lower(deep_resolve(ctx.ust, ctor_raw)),  }) is var _ ? ((CodexType actual_ty = record_ty switch { ErrorTy { } => ty, _ => record_ty,  }) is var _ ? new IrRecord(name.value, lower_record_fields_typed(fields, actual_ty, ctx, 0, list_length(fields)), actual_ty) : default) : default) : default);

    public static List<IRFieldVal> lower_record_fields_typed(List<AFieldExpr> fields, CodexType record_ty, LowerCtx ctx, long i, long len) => ((i == len) ? new List<IRFieldVal>() : ((AFieldExpr f = list_at(fields)(i)) is var _ ? ((CodexType field_expected = record_ty switch { RecordTy(var rname, var rfields) => lookup_record_field(rfields, f.name.value), _ => ErrorTy,  }) is var _ ? (new List<IRFieldVal> { new IRFieldVal(name: f.name.value, value: lower_expr(f.value, field_expected, ctx)) } + lower_record_fields_typed(fields, record_ty, ctx, (i + 1), len)) : default) : default));
    ...
    public static IRModule lower_module(AModule m, List<TypeBinding> types, UnificationState ust) => ((List<TypeBinding> ctor_types = collect_ctor_bindings(m.type_defs, 0, list_length(m.type_defs), new List<TypeBinding>())) is var _ ? ((List<TypeBinding> all_types = (ctor_types + (types + builtin_type_env.bindings))) is var _ ? new IRModule(name: m.name, defs: lower_defs(m.defs, all_types, ust, 0)) : default) : default);
    ...
    public static LexResult scan_multi_char_operator(LexState s) => ((string ch = peek_char(s)) is var _ ? ((LexState next = advance_char(s)) is var _ ? ((string next_ch = (is_at_end(next) ? "" : peek_char(next))) is var _ ? ((ch == "+") ? ((next_ch == "+") ? new LexToken(make_token(PlusPlus, "++", s), advance_char(next)) : new LexToken(make_token(Plus, "+", s), next)) : ((ch == "-") ? ((next_ch == ">") ? new LexToken(make_token(Arrow, "->", s), advance_char(next)) : new LexToken(make_token(Minus, "-", s), next)) : ((ch == "*") ? new LexToken(make_token(Star, "*", s), next) : ((ch == "/") ? ((next_ch == "=") ? new LexToken(make_token(NotEquals, "/=", s), advance_char(next)) : new LexToken(make_token(Slash, "/", s), next)) : ((ch == "=") ? ((next_ch == "=") ? ((LexState next2 = advance_char(next)) is var _ ? ((string next2_ch = (is_at_end(next2) ? "" : peek_char(next2))) is var _ ? ((next2_ch == "=") ? new LexToken(make_token(TripleEquals, "===", s), advance_char(next2)) : new LexToken(make_token(DoubleEquals, "==", s), next2)) : default) : default) : new LexToken(make_token(Equals, "=", s), next)) : ((ch == ":") ? ((next_ch == ":") ? new LexToken(make_token(ColonColon, "::", s), advance_char(next)) : new LexToken(make_token(Colon, ":", s), next)) : ((ch == "|") ? ((next_ch == "-") ? new LexToken(make_token(Turnstile, "|-", s), advance_char(next)) : new LexToken(make_token(Pipe, "|", s), next)) : ((ch == "<") ? ((next_ch == "=") ? new LexToken(make_token(LessOrEqual, "<=", s), advance_char(next)) : ((next_ch == "-") ? new LexToken(make_token(LeftArrow, "<-", s), advance_char(next)) : new LexToken(make_token(LessThan, "<", s), next))) : ((ch == ">") ? ((next_ch == "=") ? new LexToken(make_token(GreaterOrEqual, ">=", s), advance_char(next)) : new LexToken(make_token(GreaterThan, ">", s), next)) : new LexToken(make_token(ErrorToken, char_at(s.source)(s.offset), s), next)))))))))) : default) : default) : default);
