...
public static class Codex_Codex_Codex
{
    ...
    public static T432 desugar_do_stmt<T432>(object s) => s switch { DoBindStmt(var tok, var val) => ADoBindStmt(make_name(tok.text))(desugar_expr(val)), DoExprStmt(var e) => ADoExprStmt(desugar_expr(e)),  };
    ...
    public static T1432 desugar_type_expr<T1432>(object t) => t switch { NamedType(var tok) => ANamedType(make_name(tok.text)), FunType(var param, var ret) => AFunType(desugar_type_expr(param))(desugar_type_expr(ret)), AppType(var ctor, var args) => AAppType(desugar_type_expr(ctor))(map_list(desugar_type_expr)(args)), ParenType(var inner) => desugar_type_expr(inner), ListType(var elem) => AAppType(ANamedType(make_name("List")))(new List<object> { desugar_type_expr(elem) }), LinearTypeExpr(var inner) => desugar_type_expr(inner),  };
    ...
    public static T500 desugar_type_def<T500>(object td) => td.body switch { RecordBody(var fields) => ARecordTypeDef(make_name(td.name.text))(map_list(make_type_param_name)(td.type_params))(map_list(desugar_record_field_def)(fields)), VariantBody(var ctors) => AVariantTypeDef(make_name(td.name.text))(map_list(make_type_param_name)(td.type_params))(map_list(desugar_variant_ctor_def)(ctors)),  };

    public static T545 make_type_param_name<T545>(object tok) => make_name(tok.text);
    ...
    public static List<T576> map_list<T576>(object f, object xs) => map_list_loop(f)(xs)(0)(list_length(xs))(new List<object>());

    public static List<T576> map_list_loop<T576>(object f, object xs, object i, object len, object acc) => ((i == len) ? acc : map_list_loop(f)(xs)((i + 1))(len)((acc + new List<object> { f(list_at(xs)(i)) })));

    public static T604 fold_list<T604>(object f, object z, object xs) => fold_list_loop(f)(z)(xs)(0)(list_length(xs));

    public static T604 fold_list_loop<T604>(object f, object z, object xs, object i, object len) => ((i == len) ? z : fold_list_loop(f)(f(z)(list_at(xs)(i)))(xs)((i + 1))(len));
    ...
    public static T624 name_value<T624>(object n) => n.value;
    ...
    public static T638 span_length<T638>(object span) => (span.end.offset - span.start.offset);
    ...
    public static List<T1103> list_append_int<T1103>(object xs, object n) => (xs + new List<object> { n });
    ...
    public static object get_return_type(object ty, object n) => ((n == 0) ? strip_forall(ty) : strip_forall(ty) switch { FunTy(var p, var r) => get_return_type(r)((n - 1)), _ => ty,  });

    public static object strip_forall(object ty) => ty switch { ForAllTy(var id, var body) => strip_forall(body), _ => ty,  };
    ...
    public static object lookup_type(object bindings, object name) => lookup_type_loop(bindings)(name)(0)(list_length(bindings));
    ...
    public static object strip_forall_ty(object ty) => ty switch { ForAllTy(var id, var body) => strip_forall_ty(body), _ => ty,  };
    ...
    public static T1251 lower_expr<T1251>(object e, object ty) => e switch { ALitExpr(var text, var kind) => lower_literal(text)(kind), ANameExpr(var name) => IrName(name.value)(ty), AApplyExpr(var f, var a) => lower_apply(f)(a)(ty), ABinaryExpr(var l, var op, var r) => IrBinary(lower_bin_op(op)(ty))(lower_expr(l)(ty))(lower_expr(r)(ty))(ty), AUnaryExpr(var operand) => IrNegate(lower_expr(operand)(IntegerTy)), AIfExpr(var c, var t, var e2) => IrIf(lower_expr(c)(BooleanTy))(lower_expr(t)(ty))(lower_expr(e2)(ty))(ty), ALetExpr(var binds, var body) => lower_let(binds)(body)(ty), ALambdaExpr(var @params, var body) => lower_lambda(@params)(body)(ty), AMatchExpr(var scrut, var arms) => lower_match(scrut)(arms)(ty), AListExpr(var elems) => lower_list(elems)(ty), ARecordExpr(var name, var fields) => lower_record(name)(fields)(ty), AFieldAccess(var rec, var field) => IrFieldAccess(lower_expr(rec)(ty))(field.value)(ty), ADoExpr(var stmts) => lower_do(stmts)(ty), AErrorExpr(var msg) => IrError(msg)(ty),  };

    public static T1310 lower_literal<T1310>(object text, object kind) => kind switch { IntLit { } => IrIntLit(text_to_integer(text)), NumLit { } => IrIntLit(text_to_integer(text)), TextLit { } => IrTextLit(text), BoolLit { } => IrBoolLit((text == "True")),  };

    public static T1327 lower_apply<T1327>(object f, object a, object ty) => IrApply(lower_expr(f)(ty))(lower_expr(a)(ty))(ty);

    public static T1528 lower_let<T1528>(object binds, object body, object ty) => ((list_length(binds) == 0) ? lower_expr(body)(ty) : ((object b = list_at(binds)(0)) is var _ ? IrLet(b.name.value)(ty)(lower_expr(b.value)(ErrorTy))(lower_let_rest(binds)(body)(ty)(1)) : default));

    public static T1528 lower_let_rest<T1528>(object binds, object body, object ty, object i) => ((i == list_length(binds)) ? lower_expr(body)(ty) : ((object b = list_at(binds)(i)) is var _ ? IrLet(b.name.value)(ty)(lower_expr(b.value)(ErrorTy))(lower_let_rest(binds)(body)(ty)((i + 1))) : default));

    public static T1393 lower_lambda<T1393>(object @params, object body, object ty) => ((object stripped = strip_forall_ty(ty)) is var _ ? IrLambda(lower_lambda_params(@params)(stripped)(0))(lower_expr(body)(get_lambda_return(stripped)(list_length(@params))))(ty) : default);
    ...
    public static T1427 lower_match<T1427>(object scrut, object arms, object ty) => IrMatch(lower_expr(scrut)(ty))(map_list(lower_arm(ty))(arms))(ty);
    ...
    public static T1458 lower_list<T1458>(object elems, object ty) => ((object elem_ty = ty switch { ListTy(var e) => e, _ => ErrorTy,  }) is var _ ? IrList(map_list(lower_elem(elem_ty))(elems))(elem_ty) : default);

    public static T1528 lower_elem<T1528>(object ty, object e) => lower_expr(e)(ty);

    public static T1474 lower_record<T1474>(object name, object fields, object ty) => IrRecord(name.value)(map_list(lower_field_val(ty))(fields))(ty);

    public static IRFieldVal lower_field_val(object ty, object f) => new IRFieldVal(name: f.name.value, value: lower_expr(f.value)(ty));

    public static T1490 lower_do<T1490>(object stmts, object ty) => IrDo(map_list(lower_do_stmt(ty))(stmts))(ty);

    public static T1494 lower_do_stmt<T1494>(object ty, object s) => s switch { ADoBindStmt(var name, var val) => IrDoBind(name.value)(ty)(lower_expr(val)(ty)), ADoExprStmt(var e) => IrDoExec(lower_expr(e)(ty)),  };
    ...
    public static List<T1575> lower_defs<T1575>(object defs, object types, object ust, object i) => ((i == list_length(defs)) ? new List<object>() : (new List<object> { lower_def(list_at(defs)(i))(types)(ust) } + lower_defs(defs)(types)(ust)((i + 1))));
    ...
    public static List<T2040> collect_variant_ctors<T2040>(object ctors, object i, object len, object acc) => ((i == len) ? acc : ((object ctor = list_at(ctors)(i)) is var _ ? collect_variant_ctors(ctors)((i + 1))(len)((acc + new List<object> { ctor.name.value })) : default));

    public static T1731 build_all_names_scope<T1731>(object top_names, object ctor_names, object builtins) => ((object sc = add_names_to_scope(empty_scope)(top_names)(0)(list_length(top_names))) is var _ ? ((object sc2 = add_names_to_scope(sc)(ctor_names)(0)(list_length(ctor_names))) is var _ ? add_names_to_scope(sc2)(builtins)(0)(list_length(builtins)) : default) : default);

    public static T2046 add_names_to_scope<T2046>(object sc, object names, object i, object len) => ((i == len) ? sc : add_names_to_scope(scope_add(sc)(list_at(names)(i)))(names)((i + 1))(len));

    public static List<T4687> resolve_expr<T4687>(object sc, object expr) => expr switch { ALitExpr(var val, var kind) => new List<object>(), ANameExpr(var name) => ((scope_has(sc)(name.value) || is_type_name(name.value)) ? new List<object>() : new List<object> { make_error("CDX3002")(("Undefined name: " + name.value)) }), ABinaryExpr(var left, var op, var right) => (resolve_expr(sc)(left) + resolve_expr(sc)(right)), AUnaryExpr(var operand) => resolve_expr(sc)(operand), AApplyExpr(var func, var arg) => (resolve_expr(sc)(func) + resolve_expr(sc)(arg)), AIfExpr(var cond, var then_e, var else_e) => ((resolve_expr(sc)(cond) + resolve_expr(sc)(then_e)) + resolve_expr(sc)(else_e)), ALetExpr(var bindings, var body) => resolve_let(sc)(bindings)(body)(0)(list_length(bindings))(new List<object>()), ALambdaExpr(var @params, var body) => ((object sc2 = add_lambda_params(sc)(@params)(0)(list_length(@params))) is var _ ? resolve_expr(sc2)(body) : default), AMatchExpr(var scrutinee, var arms) => (resolve_expr(sc)(scrutinee) + resolve_match_arms(sc)(arms)(0)(list_length(arms))(new List<object>())), AListExpr(var elems) => resolve_list_elems(sc)(elems)(0)(list_length(elems))(new List<object>()), ARecordExpr(var name, var fields) => resolve_record_fields(sc)(fields)(0)(list_length(fields))(new List<object>()), AFieldAccess(var obj, var field) => resolve_expr(sc)(obj), ADoExpr(var stmts) => resolve_do_stmts(sc)(stmts)(0)(list_length(stmts))(new List<object>()), AErrorExpr(var msg) => new List<object>(),  };

    public static List<T4687> resolve_let<T4687>(object sc, object bindings, object body, object i, object len, object errs) => ((i == len) ? (errs + resolve_expr(sc)(body)) : ((object b = list_at(bindings)(i)) is var _ ? ((object bind_errs = resolve_expr(sc)(b.value)) is var _ ? ((object sc2 = scope_add(sc)(b.name.value)) is var _ ? resolve_let(sc2)(bindings)(body)((i + 1))(len)((errs + bind_errs)) : default) : default) : default));

    public static T2046 add_lambda_params<T2046>(object sc, object @params, object i, object len) => ((i == len) ? sc : ((object p = list_at(@params)(i)) is var _ ? add_lambda_params(scope_add(sc)(p.value))(@params)((i + 1))(len) : default));

    public static List<T4687> resolve_match_arms<T4687>(object sc, object arms, object i, object len, object errs) => ((i == len) ? errs : ((object arm = list_at(arms)(i)) is var _ ? ((object sc2 = collect_pattern_names(sc)(arm.pattern)) is var _ ? ((object arm_errs = resolve_expr(sc2)(arm.body)) is var _ ? resolve_match_arms(sc)(arms)((i + 1))(len)((errs + arm_errs)) : default) : default) : default));

    public static T2046 collect_pattern_names<T2046>(object sc, object pat) => pat switch { AVarPat(var name) => scope_add(sc)(name.value), ACtorPat(var name, var subs) => collect_ctor_pat_names(sc)(subs)(0)(list_length(subs)), ALitPat(var val, var kind) => sc, AWildPat { } => sc,  };

    public static T2046 collect_ctor_pat_names<T2046>(object sc, object subs, object i, object len) => ((i == len) ? sc : ((object sub = list_at(subs)(i)) is var _ ? collect_ctor_pat_names(collect_pattern_names(sc)(sub))(subs)((i + 1))(len) : default));

    public static List<T4687> resolve_list_elems<T4687>(object sc, object elems, object i, object len, object errs) => ((i == len) ? errs : ((object errs2 = resolve_expr(sc)(list_at(elems)(i))) is var _ ? resolve_list_elems(sc)(elems)((i + 1))(len)((errs + errs2)) : default));

    public static List<T4687> resolve_record_fields<T4687>(object sc, object fields, object i, object len, object errs) => ((i == len) ? errs : ((object f = list_at(fields)(i)) is var _ ? ((object errs2 = resolve_expr(sc)(f.value)) is var _ ? resolve_record_fields(sc)(fields)((i + 1))(len)((errs + errs2)) : default) : default));

    public static List<T4687> resolve_do_stmts<T4687>(object sc, object stmts, object i, object len, object errs) => ((i == len) ? errs : ((object stmt = list_at(stmts)(i)) is var _ ? stmt switch { ADoExprStmt(var e) => ((object errs2 = resolve_expr(sc)(e)) is var _ ? resolve_do_stmts(sc)(stmts)((i + 1))(len)((errs + errs2)) : default), ADoBindStmt(var name, var e) => ((object errs2 = resolve_expr(sc)(e)) is var _ ? ((object sc2 = scope_add(sc)(name.value)) is var _ ? resolve_do_stmts(sc2)(stmts)((i + 1))(len)((errs + errs2)) : default) : default),  } : default));

    public static List<T4687> resolve_all_defs<T4687>(object sc, object defs, object i, object len, object errs) => ((i == len) ? errs : ((object def = list_at(defs)(i)) is var _ ? ((object def_scope = add_def_params(sc)(def.@params)(0)(list_length(def.@params))) is var _ ? ((object errs2 = resolve_expr(def_scope)(def.body)) is var _ ? resolve_all_defs(sc)(defs)((i + 1))(len)((errs + errs2)) : default) : default) : default));

    public static T2046 add_def_params<T2046>(object sc, object @params, object i, object len) => ((i == len) ? sc : ((object p = list_at(@params)(i)) is var _ ? add_def_params(scope_add(sc)(p.name.value))(@params)((i + 1))(len) : default));
    ...
    public static T2414 skip_spaces<T2414>(object st) => (is_at_end(st) ? st : ((peek_char(st) == " ") ? skip_spaces(advance_char(st)) : st));

    public static T2414 scan_ident_rest<T2414>(object st) => (is_at_end(st) ? st : ((object ch = peek_char(st)) is var _ ? (is_letter(ch) ? scan_ident_rest(advance_char(st)) : (is_digit(ch) ? scan_ident_rest(advance_char(st)) : ((ch == "_") ? scan_ident_rest(advance_char(st)) : ((ch == "-") ? ((object next = advance_char(st)) is var _ ? (is_at_end(next) ? st : (is_letter(peek_char(next)) ? scan_ident_rest(next) : st)) : default) : st)))) : default));

    public static T2414 scan_digits<T2414>(object st) => (is_at_end(st) ? st : ((object ch = peek_char(st)) is var _ ? (is_digit(ch) ? scan_digits(advance_char(st)) : ((ch == "_") ? scan_digits(advance_char(st)) : st)) : default));

    public static T2414 scan_string_body<T2414>(object st) => (is_at_end(st) ? st : ((object ch = peek_char(st)) is var _ ? ((ch == "\\\"") ? advance_char(st) : ((ch == "\\n") ? st : ((ch == "\\\\") ? scan_string_body(advance_char(advance_char(st))) : scan_string_body(advance_char(st))))) : default));
    ...
    public static T2290 scan_operator<T2290>(object s) => ((object ch = peek_char(s)) is var _ ? ((object next = advance_char(s)) is var _ ? ((ch == "(") ? LexToken(make_token(LeftParen)("(")(s))(next) : ((ch == ")") ? LexToken(make_token(RightParen)(")")(s))(next) : ((ch == "[") ? LexToken(make_token(LeftBracket)("[")(s))(next) : ((ch == "]") ? LexToken(make_token(RightBracket)("]")(s))(next) : ((ch == "{") ? LexToken(make_token(LeftBrace)("{")(s))(next) : ((ch == "}") ? LexToken(make_token(RightBrace)("}")(s))(next) : ((ch == ",") ? LexToken(make_token(Comma)(",")(s))(next) : ((ch == ".") ? LexToken(make_token(Dot)(".")(s))(next) : ((ch == "^") ? LexToken(make_token(Caret)("^")(s))(next) : ((ch == "&") ? LexToken(make_token(Ampersand)("&")(s))(next) : scan_multi_char_operator(s))))))))))) : default) : default);

    public static T2412 scan_multi_char_operator<T2412>(object s) => ((object ch = peek_char(s)) is var _ ? ((object next = advance_char(s)) is var _ ? ((object next_ch = (is_at_end(next) ? "" : peek_char(next))) is var _ ? ((ch == "+") ? ((next_ch == "+") ? LexToken(make_token(PlusPlus)("++")(s))(advance_char(next)) : LexToken(make_token(Plus)("+")(s))(next)) : ((ch == "-") ? ((next_ch == ">") ? LexToken(make_token(Arrow)("->")(s))(advance_char(next)) : LexToken(make_token(Minus)("-")(s))(next)) : ((ch == "*") ? LexToken(make_token(Star)("*")(s))(next) : ((ch == "/") ? ((next_ch == "=") ? LexToken(make_token(NotEquals)("/=")(s))(advance_char(next)) : LexToken(make_token(Slash)("/")(s))(next)) : ((ch == "=") ? ((next_ch == "=") ? ((object next2 = advance_char(next)) is var _ ? ((object next2_ch = (is_at_end(next2) ? "" : peek_char(next2))) is var _ ? ((next2_ch == "=") ? LexToken(make_token(TripleEquals)("===")(s))(advance_char(next2)) : LexToken(make_token(DoubleEquals)("==")(s))(next2)) : default) : default) : LexToken(make_token(Equals)("=")(s))(next)) : ((ch == ":") ? ((next_ch == ":") ? LexToken(make_token(ColonColon)("::")(s))(advance_char(next)) : LexToken(make_token(Colon)(":")(s))(next)) : ((ch == "|") ? ((next_ch == "-") ? LexToken(make_token(Turnstile)("|-")(s))(advance_char(next)) : LexToken(make_token(Pipe)("|")(s))(next)) : ((ch == "<") ? ((next_ch == "=") ? LexToken(make_token(LessOrEqual)("<=")(s))(advance_char(next)) : ((next_ch == "-") ? LexToken(make_token(LeftArrow)("<-")(s))(advance_char(next)) : LexToken(make_token(LessThan)("<")(s))(next))) : ((ch == ">") ? ((next_ch == "=") ? LexToken(make_token(GreaterOrEqual)(">=")(s))(advance_char(next)) : LexToken(make_token(GreaterThan)(">")(s))(next)) : LexToken(make_token(ErrorToken)(char_at(s.source)(s.offset))(s))(next)))))))))) : default) : default) : default);
    ...
    public static List<object> tokenize(object src) => tokenize_loop(make_lex_state(src))(new List<object>());

    public static ParseState make_parse_state(object toks) => new ParseState(tokens: toks, pos: 0);

    public static T2438 current<T2438>(object st) => list_at(st.tokens)(st.pos);

    public static T2442 current_kind<T2442>(object st) => current(st).kind;
    ...
    public static T2459 peek_kind<T2459>(object st, object offset) => list_at(st.tokens)((st.pos + offset)).kind;
    ...
    public static object expect(object kind, object st) => (is_done(st) ? st : advance(st));

    public static object skip_newlines(object st) => (is_done(st) ? st : current_kind(st) switch { Newline { } => skip_newlines(advance(st)), Indent { } => skip_newlines(advance(st)), Dedent { } => skip_newlines(advance(st)), _ => st,  });

    public static T2576 parse_type<T2576>(object st) => ((object result = parse_type_atom(st)) is var _ ? unwrap_type_ok(result)(parse_type_continue) : default);

    public static T2576 parse_type_continue<T2576>(object left, object st) => (is_arrow(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? ((object right_result = parse_type(st2)) is var _ ? unwrap_type_ok(right_result)(make_fun_type(left)) : default) : default) : TypeOk(left)(st));

    public static T2584 make_fun_type<T2584>(object left, object right, object st) => TypeOk(FunType(left)(right))(st);

    public static T2588 unwrap_type_ok<T2588>(object r, object f) => r switch { TypeOk(var t, var st) => f(t)(st),  };

    public static T2655 parse_type_atom<T2655>(object st) => (is_ident(current_kind(st)) ? ((object tok = current(st)) is var _ ? parse_type_args(NamedType(tok))(advance(st)) : default) : (is_type_ident(current_kind(st)) ? ((object tok = current(st)) is var _ ? parse_type_args(NamedType(tok))(advance(st)) : default) : (is_left_paren(current_kind(st)) ? parse_paren_type(advance(st)) : ((object tok = current(st)) is var _ ? TypeOk(NamedType(tok))(advance(st)) : default))));

    public static T2620 parse_paren_type<T2620>(object st) => ((object inner = parse_type(st)) is var _ ? unwrap_type_ok(inner)(finish_paren_type) : default);

    public static T2628 finish_paren_type<T2628>(object t, object st) => ((object st2 = expect(RightParen)(st)) is var _ ? TypeOk(ParenType(t))(st2) : default);

    public static T2640 parse_type_args<T2640>(object base_type, object st) => (is_done(st) ? TypeOk(base_type)(st) : (is_type_arg_start(current_kind(st)) ? parse_type_arg_next(base_type)(st) : TypeOk(base_type)(st)));

    public static T2647 parse_type_arg_next<T2647>(object base_type, object st) => ((object arg_result = parse_type_atom(st)) is var _ ? unwrap_type_ok(arg_result)(continue_type_args(base_type)) : default);

    public static T2655 continue_type_args<T2655>(object base_type, object arg, object st) => parse_type_args(AppType(base_type)(new List<object> { arg }))(st);

    public static T2717 parse_pattern<T2717>(object st) => (is_underscore(current_kind(st)) ? PatOk(WildPat(current(st)))(advance(st)) : (is_literal(current_kind(st)) ? PatOk(LitPat(current(st)))(advance(st)) : (is_type_ident(current_kind(st)) ? ((object tok = current(st)) is var _ ? parse_ctor_pattern_fields(tok)(new List<object>())(advance(st)) : default) : (is_ident(current_kind(st)) ? PatOk(VarPat(current(st)))(advance(st)) : PatOk(WildPat(current(st)))(advance(st))))));

    public static T2707 parse_ctor_pattern_fields<T2707>(object ctor, object acc, object st) => (is_left_paren(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? ((object sub = parse_pattern(st2)) is var _ ? unwrap_pat_ok(sub)(continue_ctor_fields(ctor)(acc)) : default) : default) : PatOk(CtorPat(ctor)(acc))(st));

    public static T2717 continue_ctor_fields<T2717>(object ctor, object acc, object p, object st) => ((object st2 = expect(RightParen)(st)) is var _ ? parse_ctor_pattern_fields(ctor)((acc + new List<object> { p }))(st2) : default);

    public static T2721 unwrap_pat_ok<T2721>(object r, object f) => r switch { PatOk(var p, var st) => f(p)(st),  };

    public static T3318 parse_expr<T3318>(object st) => parse_binary(st)(0);

    public static T2731 unwrap_expr_ok<T2731>(object r, object f) => r switch { ExprOk(var e, var st) => f(e)(st),  };

    public static T3215 parse_binary<T3215>(object st, object min_prec) => ((object left_result = parse_unary(st)) is var _ ? unwrap_expr_ok(left_result)(start_binary_loop(min_prec)) : default);

    public static T2780 start_binary_loop<T2780>(object min_prec, object left, object st) => parse_binary_loop(left)(st)(min_prec);

    public static T3215 parse_binary_loop<T3215>(object left, object st, object min_prec) => (is_done(st) ? ExprOk(left)(st) : ((object prec = operator_precedence(current_kind(st))) is var _ ? ((prec < min_prec) ? ExprOk(left)(st) : ((object op = current(st)) is var _ ? ((object st2 = skip_newlines(advance(st))) is var _ ? ((object right_result = parse_binary(st2)((prec + 1))) is var _ ? unwrap_expr_ok(right_result)(continue_binary(left)(op)(min_prec)) : default) : default) : default)) : default));

    public static T2780 continue_binary<T2780>(object left, object op, object min_prec, object right, object st) => parse_binary_loop(BinExpr(left)(op)(right))(st)(min_prec);

    public static T2791 parse_unary<T2791>(object st) => (is_minus(current_kind(st)) ? ((object op = current(st)) is var _ ? ((object result = parse_unary(advance(st))) is var _ ? unwrap_expr_ok(result)(finish_unary(op)) : default) : default) : parse_application(st));

    public static T2799 finish_unary<T2799>(object op, object operand, object st) => ExprOk(UnaryExpr(op)(operand))(st);

    public static T3215 parse_application<T3215>(object st) => ((object func_result = parse_atom(st)) is var _ ? unwrap_expr_ok(func_result)(parse_app_loop) : default);

    public static T2886 parse_app_loop<T2886>(object func, object st) => (is_compound(func) ? parse_field_access(func)(st) : (is_done(st) ? ExprOk(func)(st) : (is_app_start(current_kind(st)) ? ((object arg_result = parse_atom(st)) is var _ ? unwrap_expr_ok(arg_result)(continue_app(func)) : default) : parse_field_access(func)(st))));

    public static T2829 continue_app<T2829>(object func, object arg, object st) => parse_app_loop(AppExpr(func)(arg))(st);

    public static T2886 parse_atom<T2886>(object st) => (is_literal(current_kind(st)) ? ExprOk(LitExpr(current(st)))(advance(st)) : (is_ident(current_kind(st)) ? parse_field_access(NameExpr(current(st)))(advance(st)) : (is_type_ident(current_kind(st)) ? parse_atom_type_ident(st) : (is_left_paren(current_kind(st)) ? parse_paren_expr(advance(st)) : (is_left_bracket(current_kind(st)) ? parse_list_expr(st) : (is_if_keyword(current_kind(st)) ? parse_if_expr(st) : (is_let_keyword(current_kind(st)) ? parse_let_expr(st) : (is_when_keyword(current_kind(st)) ? parse_match_expr(st) : (is_do_keyword(current_kind(st)) ? parse_do_expr(st) : ExprOk(ErrExpr(current(st)))(advance(st)))))))))));

    public static T2886 parse_field_access<T2886>(object node, object st) => (is_dot(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? ((object field = current(st2)) is var _ ? ((object st3 = advance(st2)) is var _ ? parse_field_access(FieldExpr(node)(field))(st3) : default) : default) : default) : ExprOk(node)(st));

    public static T2897 parse_atom_type_ident<T2897>(object st) => ((object tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? (is_left_brace(current_kind(st2)) ? parse_record_expr(tok)(st2) : ExprOk(NameExpr(tok))(st2)) : default) : default);

    public static T3215 parse_paren_expr<T3215>(object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object inner = parse_expr(st2)) is var _ ? unwrap_expr_ok(inner)(finish_paren_expr) : default) : default);

    public static T2912 finish_paren_expr<T2912>(object e, object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object st3 = expect(RightParen)(st2)) is var _ ? ExprOk(ParenExpr(e))(st3) : default) : default);

    public static T2921 parse_record_expr<T2921>(object type_name, object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? parse_record_expr_fields(type_name)(new List<object>())(st3) : default) : default);

    public static T2941 parse_record_expr_fields<T2941>(object type_name, object acc, object st) => (is_right_brace(current_kind(st)) ? ExprOk(RecordExpr(type_name)(acc))(advance(st)) : (is_ident(current_kind(st)) ? parse_record_field(type_name)(acc)(st) : ExprOk(RecordExpr(type_name)(acc))(st)));

    public static T2955 parse_record_field<T2955>(object type_name, object acc, object st) => ((object field_name = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Equals)(st2)) is var _ ? ((object val_result = parse_expr(st3)) is var _ ? unwrap_expr_ok(val_result)(finish_record_field(type_name)(acc)(field_name)) : default) : default) : default) : default);

    public static T2972 finish_record_field<T2972>(object type_name, object acc, object field_name, object v, object st) => ((object field = new RecordFieldExpr(name: field_name, value: v)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_record_expr_fields(type_name)((acc + new List<object> { field }))(skip_newlines(advance(st2))) : parse_record_expr_fields(type_name)((acc + new List<object> { field }))(st2)) : default) : default);

    public static T3005 parse_list_expr<T3005>(object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? parse_list_elements(new List<object>())(st3) : default) : default);

    public static T3215 parse_list_elements<T3215>(object acc, object st) => (is_right_bracket(current_kind(st)) ? ExprOk(ListExpr(acc))(advance(st)) : ((object elem = parse_expr(st)) is var _ ? unwrap_expr_ok(elem)(finish_list_element(acc)) : default));

    public static T3005 finish_list_element<T3005>(object acc, object e, object st) => ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_list_elements((acc + new List<object> { e }))(skip_newlines(advance(st2))) : parse_list_elements((acc + new List<object> { e }))(st2)) : default);

    public static T3012 parse_if_expr<T3012>(object st) => ((object st2 = skip_newlines(advance(st))) is var _ ? ((object cond = parse_expr(st2)) is var _ ? unwrap_expr_ok(cond)(parse_if_then) : default) : default);

    public static T3215 parse_if_then<T3215>(object c, object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object st3 = expect(ThenKeyword)(st2)) is var _ ? ((object st4 = skip_newlines(st3)) is var _ ? ((object then_result = parse_expr(st4)) is var _ ? unwrap_expr_ok(then_result)(parse_if_else(c)) : default) : default) : default) : default);

    public static T3215 parse_if_else<T3215>(object c, object t, object st) => ((object st2 = skip_newlines(st)) is var _ ? ((object st3 = expect(ElseKeyword)(st2)) is var _ ? ((object st4 = skip_newlines(st3)) is var _ ? ((object else_result = parse_expr(st4)) is var _ ? unwrap_expr_ok(else_result)(finish_if(c)(t)) : default) : default) : default) : default);

    public static T3046 finish_if<T3046>(object c, object t, object e, object st) => ExprOk(IfExpr(c)(t)(e))(st);

    public static T3106 parse_let_expr<T3106>(object st) => ((object st2 = skip_newlines(advance(st))) is var _ ? parse_let_bindings(new List<object>())(st2) : default);

    public static T3215 parse_let_bindings<T3215>(object acc, object st) => (is_ident(current_kind(st)) ? parse_let_binding(acc)(st) : (is_in_keyword(current_kind(st)) ? ((object st2 = skip_newlines(advance(st))) is var _ ? ((object body = parse_expr(st2)) is var _ ? unwrap_expr_ok(body)(finish_let(acc)) : default) : default) : ((object body = parse_expr(st)) is var _ ? unwrap_expr_ok(body)(finish_let(acc)) : default)));

    public static T3080 finish_let<T3080>(object acc, object b, object st) => ExprOk(LetExpr(acc)(b))(st);

    public static T3215 parse_let_binding<T3215>(object acc, object st) => ((object name_tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Equals)(st2)) is var _ ? ((object val_result = parse_expr(st3)) is var _ ? unwrap_expr_ok(val_result)(finish_let_binding(acc)(name_tok)) : default) : default) : default) : default);

    public static T3106 finish_let_binding<T3106>(object acc, object name_tok, object v, object st) => ((object binding = new LetBind(name: name_tok, value: v)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_let_bindings((acc + new List<object> { binding }))(skip_newlines(advance(st2))) : parse_let_bindings((acc + new List<object> { binding }))(st2)) : default) : default);

    public static T3215 parse_match_expr<T3215>(object st) => ((object st2 = advance(st)) is var _ ? ((object scrut = parse_expr(st2)) is var _ ? unwrap_expr_ok(scrut)(start_match_branches) : default) : default);

    public static T3120 start_match_branches<T3120>(object s, object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_match_branches(s)(new List<object>())(st2) : default);

    public static T3133 parse_match_branches<T3133>(object scrut, object acc, object st) => (is_if_keyword(current_kind(st)) ? parse_one_match_branch(scrut)(acc)(st) : ExprOk(MatchExpr(scrut)(acc))(st));

    public static T3137 unwrap_pat_for_expr<T3137>(object r, object f) => r switch { PatOk(var p, var st) => f(p)(st),  };

    public static T3149 parse_one_match_branch<T3149>(object scrut, object acc, object st) => ((object st2 = advance(st)) is var _ ? ((object pat = parse_pattern(st2)) is var _ ? unwrap_pat_for_expr(pat)(parse_match_branch_body(scrut)(acc)) : default) : default);

    public static T3163 parse_match_branch_body<T3163>(object scrut, object acc, object p, object st) => ((object st2 = expect(Arrow)(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? ((object body = parse_expr(st3)) is var _ ? unwrap_expr_ok(body)(finish_match_branch(scrut)(acc)(p)) : default) : default) : default);

    public static T3173 finish_match_branch<T3173>(object scrut, object acc, object p, object b, object st) => ((object arm = new MatchArm(pattern: p, body: b)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? parse_match_branches(scrut)((acc + new List<object> { arm }))(st2) : default) : default);

    public static T3225 parse_do_expr<T3225>(object st) => ((object st2 = skip_newlines(advance(st))) is var _ ? parse_do_stmts(new List<object>())(st2) : default);

    public static T3197 parse_do_stmts<T3197>(object acc, object st) => (is_done(st) ? ExprOk(DoExpr(acc))(st) : (is_dedent(current_kind(st)) ? ExprOk(DoExpr(acc))(st) : (is_do_bind(st) ? parse_do_bind_stmt(acc)(st) : parse_do_expr_stmt(acc)(st))));

    public static bool is_do_bind(object st) => (is_ident(current_kind(st)) ? is_left_arrow(peek_kind(st)(1)) : false);

    public static T3215 parse_do_bind_stmt<T3215>(object acc, object st) => ((object name_tok = current(st)) is var _ ? ((object st2 = advance(advance(st))) is var _ ? ((object val_result = parse_expr(st2)) is var _ ? unwrap_expr_ok(val_result)(finish_do_bind(acc)(name_tok)) : default) : default) : default);

    public static T3225 finish_do_bind<T3225>(object acc, object name_tok, object v, object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_do_stmts((acc + new List<object> { DoBindStmt(name_tok)(v) }))(st2) : default);

    public static T3232 parse_do_expr_stmt<T3232>(object acc, object st) => ((object expr_result = parse_expr(st)) is var _ ? unwrap_expr_ok(expr_result)(finish_do_expr(acc)) : default);

    public static T3240 finish_do_expr<T3240>(object acc, object e, object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_do_stmts((acc + new List<object> { DoExprStmt(e) }))(st2) : default);

    public static T3449 parse_type_annotation<T3449>(object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Colon)(st2)) is var _ ? parse_type(st3) : default) : default);

    public static T3257 parse_definition<T3257>(object st) => (is_done(st) ? DefNone(st) : (is_ident(current_kind(st)) ? try_parse_def(st) : (is_type_ident(current_kind(st)) ? try_parse_def(st) : DefNone(st))));

    public static T3270 try_parse_def<T3270>(object st) => (is_colon(peek_kind(st)(1)) ? ((object ann_result = parse_type_annotation(st)) is var _ ? unwrap_type_for_def(ann_result) : default) : parse_def_body_with_ann(new List<object>())(st));

    public static T3270 unwrap_type_for_def<T3270>(object r) => r switch { TypeOk(var ann_type, var st) => ((object name_tok = new Token(kind: Identifier, text: "", offset: 0, line: 0, column: 0)) is var _ ? ((object ann = new List<object> { new TypeAnn(name: name_tok, type_expr: ann_type) }) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? parse_def_body_with_ann(ann)(st2) : default) : default) : default),  };

    public static T3309 parse_def_body_with_ann<T3309>(object ann, object st) => ((object name_tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? parse_def_params_then(ann)(name_tok)(new List<object>())(st2) : default) : default);

    public static T3309 parse_def_params_then<T3309>(object ann, object name_tok, object acc, object st) => (is_left_paren(current_kind(st)) ? ((object st2 = advance(st)) is var _ ? (is_ident(current_kind(st2)) ? ((object param = current(st2)) is var _ ? ((object st3 = advance(st2)) is var _ ? ((object st4 = expect(RightParen)(st3)) is var _ ? parse_def_params_then(ann)(name_tok)((acc + new List<object> { param }))(st4) : default) : default) : default) : finish_def(ann)(name_tok)(acc)(st)) : default) : finish_def(ann)(name_tok)(acc)(st));

    public static T3322 finish_def<T3322>(object ann, object name_tok, object @params, object st) => ((object st2 = expect(Equals)(st)) is var _ ? ((object st3 = skip_newlines(st2)) is var _ ? ((object body_result = parse_expr(st3)) is var _ ? unwrap_def_body(body_result)(ann)(name_tok)(@params) : default) : default) : default);

    public static T3328 unwrap_def_body<T3328>(object r, object ann, object name_tok, object @params) => r switch { ExprOk(var b, var st) => DefOk(new Def(name: name_tok, @params: @params, ann: ann, body: b))(st),  };

    public static T3351 parse_type_def<T3351>(object st) => (is_type_ident(current_kind(st)) ? ((object name_tok = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? (is_equals(current_kind(st2)) ? ((object st3 = skip_newlines(advance(st2))) is var _ ? (is_record_keyword(current_kind(st3)) ? parse_record_type(name_tok)(st3) : (is_pipe(current_kind(st3)) ? parse_variant_type(name_tok)(st3) : TypeDefNone(st))) : default) : TypeDefNone(st)) : default) : default) : TypeDefNone(st));

    public static T3401 parse_record_type<T3401>(object name_tok, object st) => ((object st2 = advance(st)) is var _ ? ((object st3 = expect(LeftBrace)(st2)) is var _ ? ((object st4 = skip_newlines(st3)) is var _ ? parse_record_fields_loop(name_tok)(new List<object>())(st4) : default) : default) : default);

    public static T3382 parse_record_fields_loop<T3382>(object name_tok, object acc, object st) => (is_right_brace(current_kind(st)) ? TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: RecordBody(acc)))(advance(st)) : (is_ident(current_kind(st)) ? parse_one_record_field(name_tok)(acc)(st) : TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: RecordBody(acc)))(st)));

    public static T3395 parse_one_record_field<T3395>(object name_tok, object acc, object st) => ((object field_name = current(st)) is var _ ? ((object st2 = advance(st)) is var _ ? ((object st3 = expect(Colon)(st2)) is var _ ? ((object field_type_result = parse_type(st3)) is var _ ? unwrap_record_field_type(name_tok)(acc)(field_name)(field_type_result) : default) : default) : default) : default);

    public static T3401 unwrap_record_field_type<T3401>(object name_tok, object acc, object field_name, object r) => r switch { TypeOk(var ft, var st) => ((object field = new RecordFieldDef(name: field_name, type_expr: ft)) is var _ ? ((object st2 = skip_newlines(st)) is var _ ? (is_comma(current_kind(st2)) ? parse_record_fields_loop(name_tok)((acc + new List<object> { field }))(skip_newlines(advance(st2))) : parse_record_fields_loop(name_tok)((acc + new List<object> { field }))(st2)) : default) : default),  };

    public static T3458 parse_variant_type<T3458>(object name_tok, object st) => parse_variant_ctors(name_tok)(new List<object>())(st);

    public static T3465 parse_variant_ctors<T3465>(object name_tok, object acc, object st) => (is_pipe(current_kind(st)) ? ((object st2 = skip_newlines(advance(st))) is var _ ? ((object ctor_name = current(st2)) is var _ ? ((object st3 = advance(st2)) is var _ ? parse_ctor_fields(ctor_name)(new List<object>())(st3)(name_tok)(acc) : default) : default) : default) : TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: VariantBody(acc)))(st));

    public static T3458 parse_ctor_fields<T3458>(object ctor_name, object fields, object st, object name_tok, object acc) => (is_left_paren(current_kind(st)) ? ((object field_result = parse_type(advance(st))) is var _ ? unwrap_ctor_field(field_result)(ctor_name)(fields)(name_tok)(acc) : default) : ((object st2 = skip_newlines(st)) is var _ ? ((object ctor = new VariantCtorDef(name: ctor_name, fields: fields)) is var _ ? parse_variant_ctors(name_tok)((acc + new List<object> { ctor }))(st2) : default) : default));

    public static T3465 unwrap_ctor_field<T3465>(object r, object ctor_name, object fields, object name_tok, object acc) => r switch { TypeOk(var ty, var st) => ((object st2 = expect(RightParen)(st)) is var _ ? parse_ctor_fields(ctor_name)((fields + new List<object> { ty }))(st2)(name_tok)(acc) : default),  };

    public static T3516 parse_document<T3516>(object st) => ((object st2 = skip_newlines(st)) is var _ ? parse_top_level(new List<object>())(new List<object>())(st2) : default);
    ...
    public static T4227 infer_let<T4227>(object st, object env, object bindings, object body) => ((object env2 = infer_let_bindings(st)(env)(bindings)(0)(list_length(bindings))) is var _ ? infer_expr(env2.state)(env2.env)(body) : default);
    ...
    public static T3869 wrap_fun_type<T3869>(object param_types, object result) => wrap_fun_type_loop(param_types)(result)((list_length(param_types) - 1));

    public static T3869 wrap_fun_type_loop<T3869>(object param_types, object result, object i) => ((i < 0) ? result : wrap_fun_type_loop(param_types)(FunTy(list_at(param_types)(i))(result))((i - 1)));
    ...
    public static T4313 unify_list_elems<T4313>(object st, object env, object elems, object elem_ty, object i, object len) => ((i == len) ? st : ((object er = infer_expr(st)(env)(list_at(elems)(i))) is var _ ? ((object r = unify(er.state)(er.inferred_type)(elem_ty)) is var _ ? unify_list_elems(r.state)(env)(elems)(elem_ty)((i + 1))(len) : default) : default));
    ...
    public static T4313 infer_match_arms<T4313>(object st, object env, object scrut_ty, object result_ty, object arms, object i, object len) => ((i == len) ? st : ((object arm = list_at(arms)(i)) is var _ ? ((object env2 = bind_pattern(env)(arm.pattern)(scrut_ty)) is var _ ? ((object br = infer_expr(st)(env2)(arm.body)) is var _ ? ((object r = unify(br.state)(br.inferred_type)(result_ty)) is var _ ? infer_match_arms(r.state)(env)(scrut_ty)(result_ty)(arms)((i + 1))(len) : default) : default) : default) : default));

    public static T4594 bind_pattern<T4594>(object env, object pat, object ty) => pat switch { AVarPat(var name) => env_bind(env)(name.value)(ty), AWildPat { } => env, _ => env,  };
    ...
    public static T4313 infer_record_fields<T4313>(object st, object env, object fields, object i, object len) => ((i == len) ? st : ((object f = list_at(fields)(i)) is var _ ? ((object r = infer_expr(st)(env)(f.value)) is var _ ? infer_record_fields(r.state)(env)(fields)((i + 1))(len) : default) : default));

    public static T4152 resolve_type_expr<T4152>(object texpr) => texpr switch { ANamedType(var name) => resolve_type_name(name.value), AFunType(var param, var ret) => FunTy(resolve_type_expr(param))(resolve_type_expr(ret)), AAppType(var ctor, var args) => resolve_applied_type(ctor)(args),  };

    public static T4336 resolve_applied_type<T4336>(object ctor, object args) => ctor switch { ANamedType(var name) => ((name.value == "List") ? ((list_length(args) == 1) ? ListTy(resolve_type_expr(list_at(args)(0))) : ListTy(ErrorTy)) : ConstructedTy(name)(resolve_type_expr_list(args)(0)(list_length(args))(new List<object>()))), _ => resolve_type_expr(ctor),  };

    public static List<T4336> resolve_type_expr_list<T4336>(object args, object i, object len, object acc) => ((i == len) ? acc : resolve_type_expr_list(args)((i + 1))(len)((acc + new List<object> { resolve_type_expr(list_at(args)(i)) })));
    ...
    public static ModuleResult check_module(object mod) => ((object env = register_all_defs(empty_unification_state)(builtin_type_env)(mod.defs)(0)(list_length(mod.defs))) is var _ ? check_all_defs(env.state)(env.env)(mod.defs)(0)(list_length(mod.defs))(new List<object>()) : default);
    ...
    public static object env_lookup(object env, object name) => env_lookup_loop(env.bindings)(name)(0)(list_length(env.bindings));
    ...
    public static T4597 builtin_type_env<T4597>() => ((T4597 e = empty_type_env) is var _ ? ((T4597 e2 = env_bind(e)("negate")(FunTy(IntegerTy)(IntegerTy))) is var _ ? ((T4597 e3 = env_bind(e2)("text-length")(FunTy(TextTy)(IntegerTy))) is var _ ? ((T4597 e4 = env_bind(e3)("integer-to-text")(FunTy(IntegerTy)(TextTy))) is var _ ? ((T4597 e5 = env_bind(e4)("char-at")(FunTy(TextTy)(FunTy(IntegerTy)(TextTy)))) is var _ ? ((T4597 e6 = env_bind(e5)("substring")(FunTy(TextTy)(FunTy(IntegerTy)(FunTy(IntegerTy)(TextTy))))) is var _ ? ((T4597 e7 = env_bind(e6)("is-letter")(FunTy(TextTy)(BooleanTy))) is var _ ? ((T4597 e8 = env_bind(e7)("is-digit")(FunTy(TextTy)(BooleanTy))) is var _ ? ((T4597 e9 = env_bind(e8)("is-whitespace")(FunTy(TextTy)(BooleanTy))) is var _ ? ((T4597 e10 = env_bind(e9)("char-code")(FunTy(TextTy)(IntegerTy))) is var _ ? ((T4597 e11 = env_bind(e10)("code-to-char")(FunTy(IntegerTy)(TextTy))) is var _ ? ((T4597 e12 = env_bind(e11)("text-replace")(FunTy(TextTy)(FunTy(TextTy)(FunTy(TextTy)(TextTy))))) is var _ ? ((T4597 e13 = env_bind(e12)("text-to-integer")(FunTy(TextTy)(IntegerTy))) is var _ ? ((T4597 e14 = env_bind(e13)("show")(ForAllTy(0)(FunTy(TypeVar(0))(TextTy)))) is var _ ? ((T4597 e15 = env_bind(e14)("print-line")(FunTy(TextTy)(NothingTy))) is var _ ? ((T4597 e16 = env_bind(e15)("list-length")(ForAllTy(0)(FunTy(ListTy(TypeVar(0)))(IntegerTy)))) is var _ ? ((T4597 e17 = env_bind(e16)("list-at")(ForAllTy(0)(FunTy(ListTy(TypeVar(0)))(FunTy(IntegerTy)(TypeVar(0)))))) is var _ ? ((T4597 e18 = env_bind(e17)("map")(ForAllTy(0)(ForAllTy(1)(FunTy(FunTy(TypeVar(0))(TypeVar(1)))(FunTy(ListTy(TypeVar(0)))(ListTy(TypeVar(1)))))))) is var _ ? ((T4597 e19 = env_bind(e18)("filter")(ForAllTy(0)(FunTy(FunTy(TypeVar(0))(BooleanTy))(FunTy(ListTy(TypeVar(0)))(ListTy(TypeVar(0))))))) is var _ ? ((T4597 e20 = env_bind(e19)("fold")(ForAllTy(0)(ForAllTy(1)(FunTy(FunTy(TypeVar(1))(FunTy(TypeVar(0))(TypeVar(1))))(FunTy(TypeVar(1))(FunTy(ListTy(TypeVar(0)))(TypeVar(1)))))))) is var _ ? ((T4597 e21 = env_bind(e20)("read-line")(TextTy)) is var _ ? e21 : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default) : default);
    ...
    public static T4604 fresh_var<T4604>(object st) => TypeVar(st.next_id);
    ...
    public static object subst_lookup(object var_id, object entries) => subst_lookup_loop(var_id)(entries)(0)(list_length(entries));
    ...
    public static T4905 resolve<T4905>(object st, object ty) => ty switch { TypeVar(var id) => (has_subst(id)(st.substitutions) ? resolve(st)(subst_lookup(id)(st.substitutions)) : ty), _ => ty,  };
    ...
    public static T4714 unify<T4714>(object st, object a, object b) => ((object ra = resolve(st)(a)) is var _ ? ((object rb = resolve(st)(b)) is var _ ? unify_resolved(st)(ra)(rb) : default) : default);
