...
public static class Codex_Codex_Codex
{
    ...
    public static ADef desugar_def(Def d) => ((Func<ADef, ADef>)((ann_types) => new ADef(name: make_name(d.name.text), @params: map_list(desugar_param)(d.@params), declared_type: ann_types, body: desugar_expr(d.body))))(desugar_annotations(d.ann));

    public static List<ATypeExpr> desugar_annotations(List<TypeAnn> anns) => ((list_length(anns) == 0) ? new List<ATypeExpr>() : ((Func<List<ATypeExpr>, List<ATypeExpr>>)((a) => new List<ATypeExpr> { desugar_type_expr(a.type_expr) }))(list_at(anns)(0)));
    ...
    public static List<b> map_list_loop(Func<a, b> f, List<a> xs, long i, long len, List<b> acc) => ((i == len) ? acc : map_list_loop(f)(xs)((i + 1))(len)(Enumerable.Concat(acc, new List<b> { f(list_at(xs)(i)) }).ToList()));
    ...
    public static string diagnostic_display(Diagnostic d) => string.Concat(string.Concat(string.Concat(string.Concat(severity_label(d.severity), " "), d.code), ": "), d.message);
    ...
    public static string sanitize(string name) => ((Func<string, string>)((s) => (is_cs_keyword(s) ? string.Concat("@", s) : s)))(text_replace(name)("-")("_"));

    public static string cs_type(CodexType ty) => ty switch { IntegerTy { } => "long", NumberTy { } => "decimal", TextTy { } => "string", BooleanTy { } => "bool", VoidTy { } => "void", NothingTy { } => "object", ErrorTy { } => "object", FunTy(var p, var r) => string.Concat(string.Concat(string.Concat(string.Concat("Func<", cs_type(p)), ", "), cs_type(r)), ">"), ListTy(var elem) => string.Concat(string.Concat("List<", cs_type(elem)), ">"), TypeVar(var id) => string.Concat("T", integer_to_text(id)), ForAllTy(var id, var body) => cs_type(body), SumTy(var name, var ctors) => sanitize(name.value), RecordTy(var name, var fields) => sanitize(name.value), ConstructedTy(var name, var args) => sanitize(name.value),  };

    public static string emit_expr(IRExpr e) => e switch { IrIntLit(var n) => integer_to_text(n), IrNumLit(var n) => integer_to_text(n), IrTextLit(var s) => string.Concat(string.Concat("\\\"", escape_text(s)), "\\\""), IrBoolLit(var b) => (b ? "true" : "false"), IrName(var n, var ty) => sanitize(n), IrBinary(var op, var l, var r, var ty) => emit_binary(op)(l)(r)(ty), IrNegate(var operand) => string.Concat(string.Concat("(-", emit_expr(operand)), ")"), IrIf(var c, var t, var el, var ty) => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("(", emit_expr(c)), " ? "), emit_expr(t)), " : "), emit_expr(el)), ")"), IrLet(var name, var ty, var val, var body) => emit_let(name)(ty)(val)(body), IrApply(var f, var a, var ty) => string.Concat(string.Concat(string.Concat(emit_expr(f), "("), emit_expr(a)), ")"), IrLambda(var @params, var body, var ty) => emit_lambda(@params)(body), IrList(var elems, var ty) => emit_list(elems)(ty), IrMatch(var scrut, var branches, var ty) => emit_match(scrut)(branches)(ty), IrDo(var stmts, var ty) => emit_do(stmts), IrRecord(var name, var fields, var ty) => emit_record(name)(fields), IrFieldAccess(var rec, var field, var ty) => string.Concat(string.Concat(emit_expr(rec), "."), sanitize(field)), IrError(var msg, var ty) => string.Concat(string.Concat("/* error: ", msg), " */ default"),  };

    public static string emit_binary(IRBinaryOp op, IRExpr l, IRExpr r, CodexType ty) => op switch { IrAppendText { } => string.Concat(string.Concat(string.Concat(string.Concat("string.Concat(", emit_expr(l)), ", "), emit_expr(r)), ")"), IrAppendList { } => string.Concat(string.Concat(string.Concat(string.Concat("Enumerable.Concat(", emit_expr(l)), ", "), emit_expr(r)), ").ToList()"), IrConsList { } => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("new List<", cs_type(ir_expr_type(l))), "> { "), emit_expr(l)), " }.Concat("), emit_expr(r)), ").ToList()"), _ => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("(", emit_expr(l)), " "), emit_bin_op(op)), " "), emit_expr(r)), ")"),  };

    public static string escape_text(string s) => text_replace(text_replace(s)("\\\\")("\\\\\\\\"))("\\\"")("\\\\\\\"");

    public static CodexType ir_expr_type(IRExpr e) => e switch { IrIntLit(var n) => IntegerTy, IrNumLit(var n) => NumberTy, IrTextLit(var s) => TextTy, IrBoolLit(var b) => BooleanTy, IrName(var n, var ty) => ty, IrBinary(var op, var l, var r, var ty) => ty, IrNegate(var operand) => IntegerTy, IrIf(var c, var t, var el, var ty) => ty, IrLet(var name, var ty, var val, var body) => ir_expr_type(body), IrApply(var f, var a, var ty) => ty, IrLambda(var @params, var body, var ty) => ty, IrList(var elems, var ty) => ListTy(ty), IrMatch(var scrut, var branches, var ty) => ty, IrDo(var stmts, var ty) => ty, IrRecord(var name, var fields, var ty) => ty, IrFieldAccess(var rec, var field, var ty) => ty, IrError(var msg, var ty) => ty,  };
    ...
    public static string emit_let(string name, CodexType ty, IRExpr val, IRExpr body) => ((Func<string, string>)((body_ty) => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("((Func<", cs_type(ty)), ", "), cs_type(body_ty)), ">)(("), sanitize(name)), ") => "), emit_expr(body)), "))("), emit_expr(val)), ")")))(ir_expr_type(body));

    public static string emit_lambda(List<IRParam> @params, IRExpr body) => ((list_length(@params) == 0) ? string.Concat(string.Concat("(() => ", emit_expr(body)), ")") : ((list_length(@params) == 1) ? ((Func<string, string>)((p) => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("((", cs_type(p.type_val)), " "), sanitize(p.name)), ") => "), emit_expr(body)), ")")))(list_at(@params)(0)) : string.Concat(string.Concat("(() => ", emit_expr(body)), ")")));

    public static string emit_list(List<IRExpr> elems, CodexType ty) => ((list_length(elems) == 0) ? string.Concat(string.Concat("new List<", cs_type(ty)), ">()") : string.Concat(string.Concat(string.Concat(string.Concat("new List<", cs_type(ty)), "> { "), emit_list_elems(elems)(0)), " }"));

    public static string emit_list_elems(List<IRExpr> elems, long i) => ((i == list_length(elems)) ? "" : ((i == (list_length(elems) - 1)) ? emit_expr(list_at(elems)(i)) : string.Concat(string.Concat(emit_expr(list_at(elems)(i)), ", "), emit_list_elems(elems)((i + 1)))));

    public static string emit_match(IRExpr scrut, List<IRBranch> branches, CodexType ty) => string.Concat(string.Concat(string.Concat(emit_expr(scrut), " switch { "), emit_match_arms(branches)(0)), " }");

    public static string emit_match_arms(List<IRBranch> branches, long i) => ((i == list_length(branches)) ? "" : ((Func<string, string>)((arm) => string.Concat(string.Concat(string.Concat(string.Concat(emit_pattern(arm.pattern), " => "), emit_expr(arm.body)), ", "), emit_match_arms(branches)((i + 1)))))(list_at(branches)(i)));

    public static string emit_pattern(IRPat p) => p switch { IrVarPat(var name, var ty) => string.Concat(string.Concat(cs_type(ty), " "), sanitize(name)), IrLitPat(var text, var ty) => text, IrCtorPat(var name, var subs, var ty) => ((list_length(subs) == 0) ? string.Concat(sanitize(name), " { }") : string.Concat(string.Concat(string.Concat(sanitize(name), "("), emit_sub_patterns(subs)(0)), ")")), IrWildPat { } => "_",  };

    public static string emit_sub_patterns(List<IRPat> subs, long i) => ((i == list_length(subs)) ? "" : ((Func<string, string>)((sub) => string.Concat(string.Concat(emit_sub_pattern(sub), ((i < (list_length(subs) - 1)) ? ", " : "")), emit_sub_patterns(subs)((i + 1)))))(list_at(subs)(i)));

    public static string emit_sub_pattern(IRPat p) => p switch { IrVarPat(var name, var ty) => string.Concat("var ", sanitize(name)), IrCtorPat(var name, var subs, var ty) => emit_pattern(p), IrWildPat { } => "_", IrLitPat(var text, var ty) => text,  };

    public static string emit_do(List<IRDoStmt> stmts) => string.Concat(string.Concat("{ ", emit_do_stmts(stmts)(0)), " }");

    public static string emit_do_stmts(List<IRDoStmt> stmts, long i) => ((i == list_length(stmts)) ? "" : ((Func<string, string>)((s) => string.Concat(string.Concat(emit_do_stmt(s), " "), emit_do_stmts(stmts)((i + 1)))))(list_at(stmts)(i)));

    public static string emit_do_stmt(IRDoStmt s) => s switch { IrDoBind(var name, var ty, var val) => string.Concat(string.Concat(string.Concat(string.Concat("var ", sanitize(name)), " = "), emit_expr(val)), ";"), IrDoExec(var e) => string.Concat(emit_expr(e), ";"),  };

    public static string emit_record(string name, List<IRFieldVal> fields) => string.Concat(string.Concat(string.Concat(string.Concat("new ", sanitize(name)), "("), emit_record_fields(fields)(0)), ")");

    public static string emit_record_fields(List<IRFieldVal> fields, long i) => ((i == list_length(fields)) ? "" : ((Func<string, string>)((f) => string.Concat(string.Concat(string.Concat(string.Concat(sanitize(f.name), ": "), emit_expr(f.value)), ((i < (list_length(fields) - 1)) ? ", " : "")), emit_record_fields(fields)((i + 1)))))(list_at(fields)(i)));

    public static string emit_type_defs(List<ATypeDef> tds, long i) => ((i == list_length(tds)) ? "" : string.Concat(string.Concat(emit_type_def(list_at(tds)(i)), "\\n"), emit_type_defs(tds)((i + 1))));

    public static string emit_type_def(ATypeDef td) => td switch { ARecordTypeDef(var name, var tparams, var fields) => ((Func<string, string>)((gen) => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("public sealed record ", sanitize(name.value)), gen), "("), emit_record_field_defs(fields)(tparams)(0)), ");\\n")))(emit_tparam_suffix(tparams)), AVariantTypeDef(var name, var tparams, var ctors) => ((Func<string, string>)((gen) => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("public abstract record ", sanitize(name.value)), gen), ";\\n"), emit_variant_ctors(ctors)(name)(tparams)(0)), "\\n")))(emit_tparam_suffix(tparams)),  };

    public static string emit_tparam_suffix(List<Name> tparams) => ((list_length(tparams) == 0) ? "" : string.Concat(string.Concat("<", emit_tparam_names(tparams)(0)), ">"));

    public static string emit_tparam_names(List<Name> tparams, long i) => ((i == list_length(tparams)) ? "" : ((i == (list_length(tparams) - 1)) ? string.Concat("T", integer_to_text(i)) : string.Concat(string.Concat(string.Concat("T", integer_to_text(i)), ", "), emit_tparam_names(tparams)((i + 1)))));

    public static string emit_record_field_defs(List<ARecordFieldDef> fields, List<Name> tparams, long i) => ((i == list_length(fields)) ? "" : ((Func<string, string>)((f) => string.Concat(string.Concat(string.Concat(string.Concat(emit_type_expr_tp(f.type_expr)(tparams), " "), sanitize(f.name.value)), ((i < (list_length(fields) - 1)) ? ", " : "")), emit_record_field_defs(fields)(tparams)((i + 1)))))(list_at(fields)(i)));

    public static string emit_variant_ctors(List<AVariantCtorDef> ctors, Name base_name, List<Name> tparams, long i) => ((i == list_length(ctors)) ? "" : ((Func<string, string>)((c) => string.Concat(emit_variant_ctor(c)(base_name)(tparams), emit_variant_ctors(ctors)(base_name)(tparams)((i + 1)))))(list_at(ctors)(i)));

    public static string emit_variant_ctor(AVariantCtorDef c, Name base_name, List<Name> tparams) => ((Func<string, string>)((gen) => ((list_length(c.fields) == 0) ? string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("public sealed record ", sanitize(c.name.value)), gen), " : "), sanitize(base_name.value)), gen), ";\\n") : string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("public sealed record ", sanitize(c.name.value)), gen), "("), emit_ctor_fields(c.fields)(tparams)(0)), ") : "), sanitize(base_name.value)), gen), ";\\n"))))(emit_tparam_suffix(tparams));

    public static string emit_ctor_fields(List<ATypeExpr> fields, List<Name> tparams, long i) => ((i == list_length(fields)) ? "" : string.Concat(string.Concat(string.Concat(string.Concat(emit_type_expr_tp(list_at(fields)(i))(tparams), " Field"), integer_to_text(i)), ((i < (list_length(fields) - 1)) ? ", " : "")), emit_ctor_fields(fields)(tparams)((i + 1))));

    public static string emit_type_expr(ATypeExpr te) => emit_type_expr_tp(te)(new List<object>());

    public static string emit_type_expr_tp(ATypeExpr te, List<Name> tparams) => te switch { ANamedType(var name) => ((Func<string, string>)((idx) => ((idx >= 0) ? string.Concat("T", integer_to_text(idx)) : when_type_name(name.value))))(find_tparam_index(tparams)(name.value)(0)), AFunType(var p, var r) => string.Concat(string.Concat(string.Concat(string.Concat("Func<", emit_type_expr_tp(p)(tparams)), ", "), emit_type_expr_tp(r)(tparams)), ">"), AAppType(var @base, var args) => string.Concat(string.Concat(string.Concat(emit_type_expr_tp(@base)(tparams), "<"), emit_type_expr_list_tp(args)(tparams)(0)), ">"),  };
    ...
    public static string emit_type_expr_list(List<ATypeExpr> args, long i) => ((i == list_length(args)) ? "" : string.Concat(string.Concat(emit_type_expr(list_at(args)(i)), ((i < (list_length(args) - 1)) ? ", " : "")), emit_type_expr_list(args)((i + 1))));

    public static string emit_type_expr_list_tp(List<ATypeExpr> args, List<Name> tparams, long i) => ((i == list_length(args)) ? "" : string.Concat(string.Concat(emit_type_expr_tp(list_at(args)(i))(tparams), ((i < (list_length(args) - 1)) ? ", " : "")), emit_type_expr_list_tp(args)(tparams)((i + 1))));
    ...
    public static string generic_suffix(CodexType ty) => ((Func<string, string>)((ids) => ((list_length(ids) == 0) ? "" : string.Concat(string.Concat("<", emit_type_params(ids)(0)), ">"))))(collect_type_var_ids(ty)(new List<object>()));

    public static string emit_type_params(List<long> ids, long i) => ((i == list_length(ids)) ? "" : ((i == (list_length(ids) - 1)) ? string.Concat("T", integer_to_text(list_at(ids)(i))) : string.Concat(string.Concat(string.Concat("T", integer_to_text(list_at(ids)(i))), ", "), emit_type_params(ids)((i + 1)))));

    public static string emit_def(IRDef d) => ((Func<string, string>)((ret) => ((Func<string, string>)((gen) => string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("    public static ", cs_type(ret)), " "), sanitize(d.name)), gen), "("), emit_def_params(d.@params)(0)), ") => "), emit_expr(d.body)), ";\\n")))(generic_suffix(d.type_val))))(get_return_type(d.type_val)(list_length(d.@params)));
    ...
    public static string emit_def_params(List<IRParam> @params, long i) => ((i == list_length(@params)) ? "" : ((Func<string, string>)((p) => string.Concat(string.Concat(string.Concat(string.Concat(cs_type(p.type_val), " "), sanitize(p.name)), ((i < (list_length(@params) - 1)) ? ", " : "")), emit_def_params(@params)((i + 1)))))(list_at(@params)(i)));

    public static string emit_full_module(IRModule m, List<ATypeDef> type_defs) => string.Concat(string.Concat(string.Concat(string.Concat("using System;\\nusing System.Collections.Generic;\\nusing System.Linq;\\n\\n", emit_type_defs(type_defs)(0)), emit_class_header(m.name.value)), emit_defs(m.defs)(0)), "}\\n");

    public static string emit_module(IRModule m) => string.Concat(string.Concat(string.Concat("using System;\\nusing System.Collections.Generic;\\nusing System.Linq;\\n\\n", emit_class_header(m.name.value)), emit_defs(m.defs)(0)), "}\\n");

    public static string emit_class_header(string name) => string.Concat(string.Concat("public static class Codex_", sanitize(name)), "\\n{\\n");

    public static string emit_defs(List<IRDef> defs, long i) => ((i == list_length(defs)) ? "" : string.Concat(string.Concat(emit_def(list_at(defs)(i)), "\\n"), emit_defs(defs)((i + 1))));
    ...
    public static CodexType lookup_type_loop(List<TypeBinding> bindings, string name, long i, long len) => ((i == len) ? ErrorTy : ((Func<CodexType, CodexType>)((b) => ((b.name == name) ? b.bound_type : lookup_type_loop(bindings)(name)((i + 1))(len))))(list_at(bindings)(i)));
    ...
    public static IRExpr lower_let(List<ALetBind> binds, AExpr body, CodexType ty) => ((list_length(binds) == 0) ? lower_expr(body)(ty) : ((Func<IRExpr, IRExpr>)((b) => IrLet(b.name.value)(ty)(lower_expr(b.value)(ErrorTy))(lower_let_rest(binds)(body)(ty)(1))))(list_at(binds)(0)));

    public static IRExpr lower_let_rest(List<ALetBind> binds, AExpr body, CodexType ty, long i) => ((i == list_length(binds)) ? lower_expr(body)(ty) : ((Func<IRExpr, IRExpr>)((b) => IrLet(b.name.value)(ty)(lower_expr(b.value)(ErrorTy))(lower_let_rest(binds)(body)(ty)((i + 1)))))(list_at(binds)(i)));

    public static IRExpr lower_lambda(List<Name> @params, AExpr body, CodexType ty) => ((Func<IRExpr, IRExpr>)((stripped) => IrLambda(lower_lambda_params(@params)(stripped)(0))(lower_expr(body)(get_lambda_return(stripped)(list_length(@params))))(ty)))(strip_forall_ty(ty));

    public static List<IRParam> lower_lambda_params(List<Name> @params, CodexType ty, long i) => ((i == list_length(@params)) ? new List<IRParam>() : ((Func<List<IRParam>, List<IRParam>>)((p) => ((Func<List<IRParam>, List<IRParam>>)((param_ty) => ((Func<List<IRParam>, List<IRParam>>)((rest_ty) => Enumerable.Concat(new List<IRParam> { new IRParam(name: p.value, type_val: param_ty) }, lower_lambda_params(@params)(rest_ty)((i + 1))).ToList()))(peel_fun_return(ty))))(peel_fun_param(ty))))(list_at(@params)(i)));
    ...
    public static IRExpr lower_list(List<AExpr> elems, CodexType ty) => ((Func<IRExpr, IRExpr>)((elem_ty) => IrList(map_list(lower_elem(elem_ty))(elems))(elem_ty)))(ty switch { ListTy(var e) => e, _ => ErrorTy,  });
    ...
    public static IRDef lower_def(ADef d, List<TypeBinding> types, UnificationState ust) => ((Func<IRDef, IRDef>)((raw_type) => ((Func<IRDef, IRDef>)((full_type) => ((Func<IRDef, IRDef>)((stripped) => ((Func<IRDef, IRDef>)((@params) => ((Func<IRDef, IRDef>)((ret_type) => new IRDef(name: d.name.value, @params: @params, type_val: full_type, body: lower_expr(d.body)(ret_type))))(get_return_type_n(stripped)(list_length(d.@params)))))(lower_def_params(d.@params)(stripped)(0))))(strip_forall_ty(full_type))))(deep_resolve(ust)(raw_type))))(lookup_type(types)(d.name.value));

    public static List<IRParam> lower_def_params(List<AParam> @params, CodexType ty, long i) => ((i == list_length(@params)) ? new List<IRParam>() : ((Func<List<IRParam>, List<IRParam>>)((p) => ((Func<List<IRParam>, List<IRParam>>)((param_ty) => ((Func<List<IRParam>, List<IRParam>>)((rest_ty) => Enumerable.Concat(new List<IRParam> { new IRParam(name: p.name.value, type_val: param_ty) }, lower_def_params(@params)(rest_ty)((i + 1))).ToList()))(peel_fun_return(ty))))(peel_fun_param(ty))))(list_at(@params)(i)));
    ...
    public static Scope scope_add(Scope sc, string name) => new Scope(names: Enumerable.Concat(new List<object> { name }, sc.names).ToList());
    ...
    public static bool is_upper_char(string c) => ((Func<bool, bool>)((code) => ((code >= 65) && (code <= 90))))(char_code(c));

    public static CollectResult collect_top_level_names(List<ADef> defs, long i, long len, List<string> acc, List<Diagnostic> errs) => ((i == len) ? new CollectResult(names: acc, errors: errs) : ((Func<CollectResult, CollectResult>)((def) => ((Func<CollectResult, CollectResult>)((name) => (list_contains(acc)(name) ? collect_top_level_names(defs)((i + 1))(len)(acc)(Enumerable.Concat(errs, new List<object> { make_error("CDX3001")(Enumerable.Concat("Duplicate definition: ", name).ToList()) }).ToList()) : collect_top_level_names(defs)((i + 1))(len)(Enumerable.Concat(acc, new List<object> { name }).ToList())(errs))))(def.name.value)))(list_at(defs)(i)));
    ...
    public static CtorCollectResult collect_ctor_names(List<ATypeDef> type_defs, long i, long len, List<string> type_acc, List<string> ctor_acc) => ((i == len) ? new CtorCollectResult(type_names: type_acc, ctor_names: ctor_acc) : ((Func<CtorCollectResult, CtorCollectResult>)((td) => td switch { AVariantTypeDef(var name, var @params, var ctors) => ((Func<CtorCollectResult, CtorCollectResult>)((new_type_acc) => ((Func<CtorCollectResult, CtorCollectResult>)((new_ctor_acc) => collect_ctor_names(type_defs)((i + 1))(len)(new_type_acc)(new_ctor_acc)))(collect_variant_ctors(ctors)(0)(list_length(ctors))(ctor_acc))))(Enumerable.Concat(type_acc, new List<object> { name.value }).ToList()), ARecordTypeDef(var name, var @params, var fields) => collect_ctor_names(type_defs)((i + 1))(len)(Enumerable.Concat(type_acc, new List<object> { name.value }).ToList())(ctor_acc),  }))(list_at(type_defs)(i)));

    public static List<string> collect_variant_ctors(List<AVariantCtorDef> ctors, long i, long len, List<string> acc) => ((i == len) ? acc : ((Func<List<string>, List<string>>)((ctor) => collect_variant_ctors(ctors)((i + 1))(len)(Enumerable.Concat(acc, new List<string> { ctor.name.value }).ToList())))(list_at(ctors)(i)));
    ...
    public static List<Diagnostic> resolve_expr(Scope sc, AExpr expr) => expr switch { ALitExpr(var val, var kind) => new List<Diagnostic>(), ANameExpr(var name) => ((scope_has(sc)(name.value) || is_type_name(name.value)) ? new List<Diagnostic>() : new List<Diagnostic> { make_error("CDX3002")(Enumerable.Concat("Undefined name: ", name.value).ToList()) }), ABinaryExpr(var left, var op, var right) => Enumerable.Concat(resolve_expr(sc)(left), resolve_expr(sc)(right)).ToList(), AUnaryExpr(var operand) => resolve_expr(sc)(operand), AApplyExpr(var func, var arg) => Enumerable.Concat(resolve_expr(sc)(func), resolve_expr(sc)(arg)).ToList(), AIfExpr(var cond, var then_e, var else_e) => Enumerable.Concat(Enumerable.Concat(resolve_expr(sc)(cond), resolve_expr(sc)(then_e)).ToList(), resolve_expr(sc)(else_e)).ToList(), ALetExpr(var bindings, var body) => resolve_let(sc)(bindings)(body)(0)(list_length(bindings))(new List<Diagnostic>()), ALambdaExpr(var @params, var body) => ((Func<List<Diagnostic>, List<Diagnostic>>)((sc2) => resolve_expr(sc2)(body)))(add_lambda_params(sc)(@params)(0)(list_length(@params))), AMatchExpr(var scrutinee, var arms) => Enumerable.Concat(resolve_expr(sc)(scrutinee), resolve_match_arms(sc)(arms)(0)(list_length(arms))(new List<Diagnostic>())).ToList(), AListExpr(var elems) => resolve_list_elems(sc)(elems)(0)(list_length(elems))(new List<Diagnostic>()), ARecordExpr(var name, var fields) => resolve_record_fields(sc)(fields)(0)(list_length(fields))(new List<Diagnostic>()), AFieldAccess(var obj, var field) => resolve_expr(sc)(obj), ADoExpr(var stmts) => resolve_do_stmts(sc)(stmts)(0)(list_length(stmts))(new List<Diagnostic>()), AErrorExpr(var msg) => new List<Diagnostic>(),  };

    public static List<Diagnostic> resolve_let(Scope sc, List<ALetBind> bindings, AExpr body, long i, long len, List<Diagnostic> errs) => ((i == len) ? Enumerable.Concat(errs, resolve_expr(sc)(body)).ToList() : ((Func<List<Diagnostic>, List<Diagnostic>>)((b) => ((Func<List<Diagnostic>, List<Diagnostic>>)((bind_errs) => ((Func<List<Diagnostic>, List<Diagnostic>>)((sc2) => resolve_let(sc2)(bindings)(body)((i + 1))(len)(Enumerable.Concat(errs, bind_errs).ToList())))(scope_add(sc)(b.name.value))))(resolve_expr(sc)(b.value))))(list_at(bindings)(i)));

    public static Scope add_lambda_params(Scope sc, List<Name> @params, long i, long len) => ((i == len) ? sc : ((Func<Scope, Scope>)((p) => add_lambda_params(scope_add(sc)(p.value))(@params)((i + 1))(len)))(list_at(@params)(i)));

    public static List<Diagnostic> resolve_match_arms(Scope sc, List<AMatchArm> arms, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((Func<List<Diagnostic>, List<Diagnostic>>)((arm) => ((Func<List<Diagnostic>, List<Diagnostic>>)((sc2) => ((Func<List<Diagnostic>, List<Diagnostic>>)((arm_errs) => resolve_match_arms(sc)(arms)((i + 1))(len)(Enumerable.Concat(errs, arm_errs).ToList())))(resolve_expr(sc2)(arm.body))))(collect_pattern_names(sc)(arm.pattern))))(list_at(arms)(i)));
    ...
    public static Scope collect_ctor_pat_names(Scope sc, List<APat> subs, long i, long len) => ((i == len) ? sc : ((Func<Scope, Scope>)((sub) => collect_ctor_pat_names(collect_pattern_names(sc)(sub))(subs)((i + 1))(len)))(list_at(subs)(i)));

    public static List<Diagnostic> resolve_list_elems(Scope sc, List<AExpr> elems, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((Func<List<Diagnostic>, List<Diagnostic>>)((errs2) => resolve_list_elems(sc)(elems)((i + 1))(len)(Enumerable.Concat(errs, errs2).ToList())))(resolve_expr(sc)(list_at(elems)(i))));

    public static List<Diagnostic> resolve_record_fields(Scope sc, List<AFieldExpr> fields, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((Func<List<Diagnostic>, List<Diagnostic>>)((f) => ((Func<List<Diagnostic>, List<Diagnostic>>)((errs2) => resolve_record_fields(sc)(fields)((i + 1))(len)(Enumerable.Concat(errs, errs2).ToList())))(resolve_expr(sc)(f.value))))(list_at(fields)(i)));

    public static List<Diagnostic> resolve_do_stmts(Scope sc, List<ADoStmt> stmts, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((Func<List<Diagnostic>, List<Diagnostic>>)((stmt) => stmt switch { ADoExprStmt(var e) => ((Func<List<Diagnostic>, List<Diagnostic>>)((errs2) => resolve_do_stmts(sc)(stmts)((i + 1))(len)(Enumerable.Concat(errs, errs2).ToList())))(resolve_expr(sc)(e)), ADoBindStmt(var name, var e) => ((Func<List<Diagnostic>, List<Diagnostic>>)((errs2) => ((Func<List<Diagnostic>, List<Diagnostic>>)((sc2) => resolve_do_stmts(sc2)(stmts)((i + 1))(len)(Enumerable.Concat(errs, errs2).ToList())))(scope_add(sc)(name.value))))(resolve_expr(sc)(e)),  }))(list_at(stmts)(i)));

    public static List<Diagnostic> resolve_all_defs(Scope sc, List<ADef> defs, long i, long len, List<Diagnostic> errs) => ((i == len) ? errs : ((Func<List<Diagnostic>, List<Diagnostic>>)((def) => ((Func<List<Diagnostic>, List<Diagnostic>>)((def_scope) => ((Func<List<Diagnostic>, List<Diagnostic>>)((errs2) => resolve_all_defs(sc)(defs)((i + 1))(len)(Enumerable.Concat(errs, errs2).ToList())))(resolve_expr(def_scope)(def.body))))(add_def_params(sc)(def.@params)(0)(list_length(def.@params)))))(list_at(defs)(i)));

    public static Scope add_def_params(Scope sc, List<AParam> @params, long i, long len) => ((i == len) ? sc : ((Func<Scope, Scope>)((p) => add_def_params(scope_add(sc)(p.name.value))(@params)((i + 1))(len)))(list_at(@params)(i)));

    public static ResolveResult resolve_module(AModule mod) => ((Func<ResolveResult, ResolveResult>)((top) => ((Func<ResolveResult, ResolveResult>)((ctors) => ((Func<ResolveResult, ResolveResult>)((sc) => ((Func<ResolveResult, ResolveResult>)((expr_errs) => new ResolveResult(errors: Enumerable.Concat(top.errors, expr_errs).ToList(), top_level_names: top.names, type_names: ctors.type_names, ctor_names: ctors.ctor_names)))(resolve_all_defs(sc)(mod.defs)(0)(list_length(mod.defs))(new List<object>()))))(build_all_names_scope(top.names)(ctors.ctor_names)(builtin_names))))(collect_ctor_names(mod.type_defs)(0)(list_length(mod.type_defs))(new List<object>())(new List<object>()))))(collect_top_level_names(mod.defs)(0)(list_length(mod.defs))(new List<object>())(new List<object>()));
    ...
    public static LexState scan_ident_rest(LexState st) => (is_at_end(st) ? st : ((Func<LexState, LexState>)((ch) => (is_letter(ch) ? scan_ident_rest(advance_char(st)) : (is_digit(ch) ? scan_ident_rest(advance_char(st)) : ((ch == "_") ? scan_ident_rest(advance_char(st)) : ((ch == "-") ? ((Func<LexState, LexState>)((next) => (is_at_end(next) ? st : (is_letter(peek_char(next)) ? scan_ident_rest(next) : st))))(advance_char(st)) : st))))))(peek_char(st)));

    public static LexState scan_digits(LexState st) => (is_at_end(st) ? st : ((Func<LexState, LexState>)((ch) => (is_digit(ch) ? scan_digits(advance_char(st)) : ((ch == "_") ? scan_digits(advance_char(st)) : st))))(peek_char(st)));

    public static LexState scan_string_body(LexState st) => (is_at_end(st) ? st : ((Func<LexState, LexState>)((ch) => ((ch == "\\\"") ? advance_char(st) : ((ch == "\\n") ? st : ((ch == "\\\\") ? scan_string_body(advance_char(advance_char(st))) : scan_string_body(advance_char(st)))))))(peek_char(st)));

    public static TokenKind classify_word(string w) => ((w == "let") ? LetKeyword : ((w == "in") ? InKeyword : ((w == "if") ? IfKeyword : ((w == "then") ? ThenKeyword : ((w == "else") ? ElseKeyword : ((w == "when") ? WhenKeyword : ((w == "where") ? WhereKeyword : ((w == "do") ? DoKeyword : ((w == "record") ? RecordKeyword : ((w == "import") ? ImportKeyword : ((w == "export") ? ExportKeyword : ((w == "claim") ? ClaimKeyword : ((w == "proof") ? ProofKeyword : ((w == "forall") ? ForAllKeyword : ((w == "exists") ? ThereExistsKeyword : ((w == "linear") ? LinearKeyword : ((w == "True") ? TrueKeyword : ((w == "False") ? FalseKeyword : ((Func<TokenKind, TokenKind>)((first_code) => ((first_code >= 65) ? ((first_code <= 90) ? TypeIdentifier : Identifier) : Identifier)))(char_code(char_at(w)(0)))))))))))))))))))));
    ...
    public static LexResult scan_token(LexState st) => ((Func<LexResult, LexResult>)((s) => (is_at_end(s) ? LexEnd : ((Func<LexResult, LexResult>)((ch) => ((ch == "\\n") ? LexToken(make_token(Newline)("\\n")(s))(advance_char(s)) : ((ch == "\\\"") ? ((Func<LexResult, LexResult>)((start) => ((Func<LexResult, LexResult>)((after) => ((Func<LexResult, LexResult>)((text_len) => LexToken(make_token(TextLiteral)(substring(s.source)(start)(text_len))(s))(after)))(((after.offset - start) - 1))))(scan_string_body(advance_char(s)))))((s.offset + 1)) : (is_letter(ch) ? ((Func<LexResult, LexResult>)((start) => ((Func<LexResult, LexResult>)((after) => ((Func<LexResult, LexResult>)((word) => LexToken(make_token(classify_word(word))(word)(s))(after)))(extract_text(s)(start)(after))))(scan_ident_rest(advance_char(s)))))(s.offset) : ((ch == "_") ? ((Func<LexResult, LexResult>)((start) => ((Func<LexResult, LexResult>)((after) => ((Func<LexResult, LexResult>)((word) => ((text_length(word) == 1) ? LexToken(make_token(Underscore)("_")(s))(after) : LexToken(make_token(classify_word(word))(word)(s))(after))))(extract_text(s)(start)(after))))(scan_ident_rest(advance_char(s)))))(s.offset) : (is_digit(ch) ? ((Func<LexResult, LexResult>)((start) => ((Func<LexResult, LexResult>)((after) => (is_at_end(after) ? LexToken(make_token(IntegerLiteral)(extract_text(s)(start)(after))(s))(after) : ((peek_char(after) == ".") ? ((Func<LexResult, LexResult>)((after2) => LexToken(make_token(NumberLiteral)(extract_text(s)(start)(after2))(s))(after2)))(scan_digits(advance_char(after))) : LexToken(make_token(IntegerLiteral)(extract_text(s)(start)(after))(s))(after)))))(scan_digits(advance_char(s)))))(s.offset) : scan_operator(s))))))))(peek_char(s)))))(skip_spaces(st));

    public static LexResult scan_operator(LexState s) => ((Func<LexResult, LexResult>)((ch) => ((Func<LexResult, LexResult>)((next) => ((ch == "(") ? LexToken(make_token(LeftParen)("(")(s))(next) : ((ch == ")") ? LexToken(make_token(RightParen)(")")(s))(next) : ((ch == "[") ? LexToken(make_token(LeftBracket)("[")(s))(next) : ((ch == "]") ? LexToken(make_token(RightBracket)("]")(s))(next) : ((ch == "{") ? LexToken(make_token(LeftBrace)("{")(s))(next) : ((ch == "}") ? LexToken(make_token(RightBrace)("}")(s))(next) : ((ch == ",") ? LexToken(make_token(Comma)(",")(s))(next) : ((ch == ".") ? LexToken(make_token(Dot)(".")(s))(next) : ((ch == "^") ? LexToken(make_token(Caret)("^")(s))(next) : ((ch == "&") ? LexToken(make_token(Ampersand)("&")(s))(next) : scan_multi_char_operator(s)))))))))))))(advance_char(s))))(peek_char(s));

    public static LexResult scan_multi_char_operator(LexState s) => ((Func<LexResult, LexResult>)((ch) => ((Func<LexResult, LexResult>)((next) => ((Func<LexResult, LexResult>)((next_ch) => ((ch == "+") ? ((next_ch == "+") ? LexToken(make_token(PlusPlus)("++")(s))(advance_char(next)) : LexToken(make_token(Plus)("+")(s))(next)) : ((ch == "-") ? ((next_ch == ">") ? LexToken(make_token(Arrow)("->")(s))(advance_char(next)) : LexToken(make_token(Minus)("-")(s))(next)) : ((ch == "*") ? LexToken(make_token(Star)("*")(s))(next) : ((ch == "/") ? ((next_ch == "=") ? LexToken(make_token(NotEquals)("/=")(s))(advance_char(next)) : LexToken(make_token(Slash)("/")(s))(next)) : ((ch == "=") ? ((next_ch == "=") ? ((Func<LexResult, LexResult>)((next2) => ((Func<LexResult, LexResult>)((next2_ch) => ((next2_ch == "=") ? LexToken(make_token(TripleEquals)("===")(s))(advance_char(next2)) : LexToken(make_token(DoubleEquals)("==")(s))(next2))))((is_at_end(next2) ? "" : peek_char(next2)))))(advance_char(next)) : LexToken(make_token(Equals)("=")(s))(next)) : ((ch == ":") ? ((next_ch == ":") ? LexToken(make_token(ColonColon)("::")(s))(advance_char(next)) : LexToken(make_token(Colon)(":")(s))(next)) : ((ch == "|") ? ((next_ch == "-") ? LexToken(make_token(Turnstile)("|-")(s))(advance_char(next)) : LexToken(make_token(Pipe)("|")(s))(next)) : ((ch == "<") ? ((next_ch == "=") ? LexToken(make_token(LessOrEqual)("<=")(s))(advance_char(next)) : ((next_ch == "-") ? LexToken(make_token(LeftArrow)("<-")(s))(advance_char(next)) : LexToken(make_token(LessThan)("<")(s))(next))) : ((ch == ">") ? ((next_ch == "=") ? LexToken(make_token(GreaterOrEqual)(">=")(s))(advance_char(next)) : LexToken(make_token(GreaterThan)(">")(s))(next)) : LexToken(make_token(ErrorToken)(char_at(s.source)(s.offset))(s))(next))))))))))))((is_at_end(next) ? "" : peek_char(next)))))(advance_char(s))))(peek_char(s));

    public static List<Token> tokenize_loop(LexState st, List<Token> acc) => scan_token(st) switch { LexToken(var tok, var next) => ((tok.kind == EndOfFile) ? Enumerable.Concat(acc, new List<Token> { tok }).ToList() : tokenize_loop(next)(Enumerable.Concat(acc, new List<Token> { tok }).ToList())), LexEnd { } => Enumerable.Concat(acc, new List<Token> { make_token(EndOfFile)("")(st) }).ToList(),  };
    ...
    public static ParseTypeResult parse_type(ParseState st) => ((Func<ParseTypeResult, ParseTypeResult>)((result) => unwrap_type_ok(result)(parse_type_continue)))(parse_type_atom(st));

    public static ParseTypeResult parse_type_continue(TypeExpr left, ParseState st) => (is_arrow(current_kind(st)) ? ((Func<ParseTypeResult, ParseTypeResult>)((st2) => ((Func<ParseTypeResult, ParseTypeResult>)((right_result) => unwrap_type_ok(right_result)(make_fun_type(left))))(parse_type(st2))))(advance(st)) : TypeOk(left)(st));
    ...
    public static ParseTypeResult parse_type_atom(ParseState st) => (is_ident(current_kind(st)) ? ((Func<ParseTypeResult, ParseTypeResult>)((tok) => parse_type_args(NamedType(tok))(advance(st))))(current(st)) : (is_type_ident(current_kind(st)) ? ((Func<ParseTypeResult, ParseTypeResult>)((tok) => parse_type_args(NamedType(tok))(advance(st))))(current(st)) : (is_left_paren(current_kind(st)) ? parse_paren_type(advance(st)) : ((Func<ParseTypeResult, ParseTypeResult>)((tok) => TypeOk(NamedType(tok))(advance(st))))(current(st)))));

    public static ParseTypeResult parse_paren_type(ParseState st) => ((Func<ParseTypeResult, ParseTypeResult>)((inner) => unwrap_type_ok(inner)(finish_paren_type)))(parse_type(st));

    public static ParseTypeResult finish_paren_type(TypeExpr t, ParseState st) => ((Func<ParseTypeResult, ParseTypeResult>)((st2) => TypeOk(ParenType(t))(st2)))(expect(RightParen)(st));
    ...
    public static ParseTypeResult parse_type_arg_next(TypeExpr base_type, ParseState st) => ((Func<ParseTypeResult, ParseTypeResult>)((arg_result) => unwrap_type_ok(arg_result)(continue_type_args(base_type))))(parse_type_atom(st));
    ...
    public static ParsePatResult parse_pattern(ParseState st) => (is_underscore(current_kind(st)) ? PatOk(WildPat(current(st)))(advance(st)) : (is_literal(current_kind(st)) ? PatOk(LitPat(current(st)))(advance(st)) : (is_type_ident(current_kind(st)) ? ((Func<ParsePatResult, ParsePatResult>)((tok) => parse_ctor_pattern_fields(tok)(new List<object>())(advance(st))))(current(st)) : (is_ident(current_kind(st)) ? PatOk(VarPat(current(st)))(advance(st)) : PatOk(WildPat(current(st)))(advance(st))))));

    public static ParsePatResult parse_ctor_pattern_fields(Token ctor, List<Pat> acc, ParseState st) => (is_left_paren(current_kind(st)) ? ((Func<ParsePatResult, ParsePatResult>)((st2) => ((Func<ParsePatResult, ParsePatResult>)((sub) => unwrap_pat_ok(sub)(continue_ctor_fields(ctor)(acc))))(parse_pattern(st2))))(advance(st)) : PatOk(CtorPat(ctor)(acc))(st));

    public static ParsePatResult continue_ctor_fields(Token ctor, List<Pat> acc, Pat p, ParseState st) => ((Func<ParsePatResult, ParsePatResult>)((st2) => parse_ctor_pattern_fields(ctor)(Enumerable.Concat(acc, new List<object> { p }).ToList())(st2)))(expect(RightParen)(st));
    ...
    public static ParseExprResult parse_unary(ParseState st) => (is_minus(current_kind(st)) ? ((Func<ParseExprResult, ParseExprResult>)((op) => ((Func<ParseExprResult, ParseExprResult>)((result) => unwrap_expr_ok(result)(finish_unary(op))))(parse_unary(advance(st)))))(current(st)) : parse_application(st));

    public static ParseExprResult finish_unary(Token op, Expr operand, ParseState st) => ExprOk(UnaryExpr(op)(operand))(st);

    public static ParseExprResult parse_application(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((func_result) => unwrap_expr_ok(func_result)(parse_app_loop)))(parse_atom(st));

    public static ParseExprResult parse_app_loop(Expr func, ParseState st) => (is_compound(func) ? parse_field_access(func)(st) : (is_done(st) ? ExprOk(func)(st) : (is_app_start(current_kind(st)) ? ((Func<ParseExprResult, ParseExprResult>)((arg_result) => unwrap_expr_ok(arg_result)(continue_app(func))))(parse_atom(st)) : parse_field_access(func)(st))));
    ...
    public static ParseExprResult parse_field_access(Expr node, ParseState st) => (is_dot(current_kind(st)) ? ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((field) => ((Func<ParseExprResult, ParseExprResult>)((st3) => parse_field_access(FieldExpr(node)(field))(st3)))(advance(st2))))(current(st2))))(advance(st)) : ExprOk(node)(st));

    public static ParseExprResult parse_atom_type_ident(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((tok) => ((Func<ParseExprResult, ParseExprResult>)((st2) => (is_left_brace(current_kind(st2)) ? parse_record_expr(tok)(st2) : ExprOk(NameExpr(tok))(st2))))(advance(st))))(current(st));

    public static ParseExprResult parse_paren_expr(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((inner) => unwrap_expr_ok(inner)(finish_paren_expr)))(parse_expr(st2))))(skip_newlines(st));

    public static ParseExprResult finish_paren_expr(Expr e, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => ExprOk(ParenExpr(e))(st3)))(expect(RightParen)(st2))))(skip_newlines(st));

    public static ParseExprResult parse_record_expr(Token type_name, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => parse_record_expr_fields(type_name)(new List<object>())(st3)))(skip_newlines(st2))))(advance(st));
    ...
    public static ParseExprResult parse_record_field(Token type_name, List<RecordFieldExpr> acc, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((field_name) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => ((Func<ParseExprResult, ParseExprResult>)((val_result) => unwrap_expr_ok(val_result)(finish_record_field(type_name)(acc)(field_name))))(parse_expr(st3))))(expect(Equals)(st2))))(advance(st))))(current(st));

    public static ParseExprResult finish_record_field(Token type_name, List<RecordFieldExpr> acc, Token field_name, Expr v, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((field) => ((Func<ParseExprResult, ParseExprResult>)((st2) => (is_comma(current_kind(st2)) ? parse_record_expr_fields(type_name)(Enumerable.Concat(acc, new List<object> { field }).ToList())(skip_newlines(advance(st2))) : parse_record_expr_fields(type_name)(Enumerable.Concat(acc, new List<object> { field }).ToList())(st2))))(skip_newlines(st))))(new RecordFieldExpr(name: field_name, value: v));

    public static ParseExprResult parse_list_expr(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => parse_list_elements(new List<object>())(st3)))(skip_newlines(st2))))(advance(st));

    public static ParseExprResult parse_list_elements(List<Expr> acc, ParseState st) => (is_right_bracket(current_kind(st)) ? ExprOk(ListExpr(acc))(advance(st)) : ((Func<ParseExprResult, ParseExprResult>)((elem) => unwrap_expr_ok(elem)(finish_list_element(acc))))(parse_expr(st)));

    public static ParseExprResult finish_list_element(List<Expr> acc, Expr e, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => (is_comma(current_kind(st2)) ? parse_list_elements(Enumerable.Concat(acc, new List<object> { e }).ToList())(skip_newlines(advance(st2))) : parse_list_elements(Enumerable.Concat(acc, new List<object> { e }).ToList())(st2))))(skip_newlines(st));

    public static ParseExprResult parse_if_expr(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((cond) => unwrap_expr_ok(cond)(parse_if_then)))(parse_expr(st2))))(skip_newlines(advance(st)));

    public static ParseExprResult parse_if_then(Expr c, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => ((Func<ParseExprResult, ParseExprResult>)((st4) => ((Func<ParseExprResult, ParseExprResult>)((then_result) => unwrap_expr_ok(then_result)(parse_if_else(c))))(parse_expr(st4))))(skip_newlines(st3))))(expect(ThenKeyword)(st2))))(skip_newlines(st));

    public static ParseExprResult parse_if_else(Expr c, Expr t, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => ((Func<ParseExprResult, ParseExprResult>)((st4) => ((Func<ParseExprResult, ParseExprResult>)((else_result) => unwrap_expr_ok(else_result)(finish_if(c)(t))))(parse_expr(st4))))(skip_newlines(st3))))(expect(ElseKeyword)(st2))))(skip_newlines(st));

    public static ParseExprResult finish_if(Expr c, Expr t, Expr e, ParseState st) => ExprOk(IfExpr(c)(t)(e))(st);

    public static ParseExprResult parse_let_expr(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => parse_let_bindings(new List<object>())(st2)))(skip_newlines(advance(st)));

    public static ParseExprResult parse_let_bindings(List<LetBind> acc, ParseState st) => (is_ident(current_kind(st)) ? parse_let_binding(acc)(st) : (is_in_keyword(current_kind(st)) ? ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((body) => unwrap_expr_ok(body)(finish_let(acc))))(parse_expr(st2))))(skip_newlines(advance(st))) : ((Func<ParseExprResult, ParseExprResult>)((body) => unwrap_expr_ok(body)(finish_let(acc))))(parse_expr(st))));

    public static ParseExprResult finish_let(List<LetBind> acc, Expr b, ParseState st) => ExprOk(LetExpr(acc)(b))(st);

    public static ParseExprResult parse_let_binding(List<LetBind> acc, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((name_tok) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => ((Func<ParseExprResult, ParseExprResult>)((val_result) => unwrap_expr_ok(val_result)(finish_let_binding(acc)(name_tok))))(parse_expr(st3))))(expect(Equals)(st2))))(advance(st))))(current(st));

    public static ParseExprResult finish_let_binding(List<LetBind> acc, Token name_tok, Expr v, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((binding) => ((Func<ParseExprResult, ParseExprResult>)((st2) => (is_comma(current_kind(st2)) ? parse_let_bindings(Enumerable.Concat(acc, new List<object> { binding }).ToList())(skip_newlines(advance(st2))) : parse_let_bindings(Enumerable.Concat(acc, new List<object> { binding }).ToList())(st2))))(skip_newlines(st))))(new LetBind(name: name_tok, value: v));

    public static ParseExprResult parse_match_expr(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((scrut) => unwrap_expr_ok(scrut)(start_match_branches)))(parse_expr(st2))))(advance(st));

    public static ParseExprResult start_match_branches(Expr s, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => parse_match_branches(s)(new List<object>())(st2)))(skip_newlines(st));
    ...
    public static ParseExprResult parse_one_match_branch(Expr scrut, List<MatchArm> acc, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((pat) => unwrap_pat_for_expr(pat)(parse_match_branch_body(scrut)(acc))))(parse_pattern(st2))))(advance(st));

    public static ParseExprResult parse_match_branch_body(Expr scrut, List<MatchArm> acc, Pat p, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((st3) => ((Func<ParseExprResult, ParseExprResult>)((body) => unwrap_expr_ok(body)(finish_match_branch(scrut)(acc)(p))))(parse_expr(st3))))(skip_newlines(st2))))(expect(Arrow)(st));

    public static ParseExprResult finish_match_branch(Expr scrut, List<MatchArm> acc, Pat p, Expr b, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((arm) => ((Func<ParseExprResult, ParseExprResult>)((st2) => parse_match_branches(scrut)(Enumerable.Concat(acc, new List<object> { arm }).ToList())(st2)))(skip_newlines(st))))(new MatchArm(pattern: p, body: b));

    public static ParseExprResult parse_do_expr(ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => parse_do_stmts(new List<object>())(st2)))(skip_newlines(advance(st)));
    ...
    public static ParseExprResult parse_do_bind_stmt(List<DoStmt> acc, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((name_tok) => ((Func<ParseExprResult, ParseExprResult>)((st2) => ((Func<ParseExprResult, ParseExprResult>)((val_result) => unwrap_expr_ok(val_result)(finish_do_bind(acc)(name_tok))))(parse_expr(st2))))(advance(advance(st)))))(current(st));

    public static ParseExprResult finish_do_bind(List<DoStmt> acc, Token name_tok, Expr v, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => parse_do_stmts(Enumerable.Concat(acc, new List<object> { DoBindStmt(name_tok)(v) }).ToList())(st2)))(skip_newlines(st));

    public static ParseExprResult parse_do_expr_stmt(List<DoStmt> acc, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((expr_result) => unwrap_expr_ok(expr_result)(finish_do_expr(acc))))(parse_expr(st));

    public static ParseExprResult finish_do_expr(List<DoStmt> acc, Expr e, ParseState st) => ((Func<ParseExprResult, ParseExprResult>)((st2) => parse_do_stmts(Enumerable.Concat(acc, new List<object> { DoExprStmt(e) }).ToList())(st2)))(skip_newlines(st));

    public static ParseTypeResult parse_type_annotation(ParseState st) => ((Func<ParseTypeResult, ParseTypeResult>)((st2) => ((Func<ParseTypeResult, ParseTypeResult>)((st3) => parse_type(st3)))(expect(Colon)(st2))))(advance(st));
    ...
    public static ParseDefResult try_parse_def(ParseState st) => (is_colon(peek_kind(st)(1)) ? ((Func<ParseDefResult, ParseDefResult>)((ann_result) => unwrap_type_for_def(ann_result)))(parse_type_annotation(st)) : parse_def_body_with_ann(new List<object>())(st));

    public static ParseDefResult unwrap_type_for_def(ParseTypeResult r) => r switch { TypeOk(var ann_type, var st) => ((Func<ParseDefResult, ParseDefResult>)((name_tok) => ((Func<ParseDefResult, ParseDefResult>)((ann) => ((Func<ParseDefResult, ParseDefResult>)((st2) => parse_def_body_with_ann(ann)(st2)))(skip_newlines(st))))(new List<object> { new TypeAnn(name: name_tok, type_expr: ann_type) })))(new Token(kind: Identifier, text: "", offset: 0, line: 0, column: 0)),  };

    public static ParseDefResult parse_def_body_with_ann(List<TypeAnn> ann, ParseState st) => ((Func<ParseDefResult, ParseDefResult>)((name_tok) => ((Func<ParseDefResult, ParseDefResult>)((st2) => parse_def_params_then(ann)(name_tok)(new List<object>())(st2)))(advance(st))))(current(st));

    public static ParseDefResult parse_def_params_then(List<TypeAnn> ann, Token name_tok, List<Token> acc, ParseState st) => (is_left_paren(current_kind(st)) ? ((Func<ParseDefResult, ParseDefResult>)((st2) => (is_ident(current_kind(st2)) ? ((Func<ParseDefResult, ParseDefResult>)((param) => ((Func<ParseDefResult, ParseDefResult>)((st3) => ((Func<ParseDefResult, ParseDefResult>)((st4) => parse_def_params_then(ann)(name_tok)(Enumerable.Concat(acc, new List<object> { param }).ToList())(st4)))(expect(RightParen)(st3))))(advance(st2))))(current(st2)) : finish_def(ann)(name_tok)(acc)(st))))(advance(st)) : finish_def(ann)(name_tok)(acc)(st));

    public static ParseDefResult finish_def(List<TypeAnn> ann, Token name_tok, List<Token> @params, ParseState st) => ((Func<ParseDefResult, ParseDefResult>)((st2) => ((Func<ParseDefResult, ParseDefResult>)((st3) => ((Func<ParseDefResult, ParseDefResult>)((body_result) => unwrap_def_body(body_result)(ann)(name_tok)(@params)))(parse_expr(st3))))(skip_newlines(st2))))(expect(Equals)(st));
    ...
    public static ParseTypeDefResult parse_type_def(ParseState st) => (is_type_ident(current_kind(st)) ? ((Func<ParseTypeDefResult, ParseTypeDefResult>)((name_tok) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => (is_equals(current_kind(st2)) ? ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st3) => (is_record_keyword(current_kind(st3)) ? parse_record_type(name_tok)(st3) : (is_pipe(current_kind(st3)) ? parse_variant_type(name_tok)(st3) : TypeDefNone(st)))))(skip_newlines(advance(st2))) : TypeDefNone(st))))(advance(st))))(current(st)) : TypeDefNone(st));

    public static ParseTypeDefResult parse_record_type(Token name_tok, ParseState st) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st3) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st4) => parse_record_fields_loop(name_tok)(new List<object>())(st4)))(skip_newlines(st3))))(expect(LeftBrace)(st2))))(advance(st));
    ...
    public static ParseTypeDefResult parse_one_record_field(Token name_tok, List<RecordFieldDef> acc, ParseState st) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((field_name) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st3) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((field_type_result) => unwrap_record_field_type(name_tok)(acc)(field_name)(field_type_result)))(parse_type(st3))))(expect(Colon)(st2))))(advance(st))))(current(st));

    public static ParseTypeDefResult unwrap_record_field_type(Token name_tok, List<RecordFieldDef> acc, Token field_name, ParseTypeResult r) => r switch { TypeOk(var ft, var st) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((field) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => (is_comma(current_kind(st2)) ? parse_record_fields_loop(name_tok)(Enumerable.Concat(acc, new List<object> { field }).ToList())(skip_newlines(advance(st2))) : parse_record_fields_loop(name_tok)(Enumerable.Concat(acc, new List<object> { field }).ToList())(st2))))(skip_newlines(st))))(new RecordFieldDef(name: field_name, type_expr: ft)),  };
    ...
    public static ParseTypeDefResult parse_variant_ctors(Token name_tok, List<VariantCtorDef> acc, ParseState st) => (is_pipe(current_kind(st)) ? ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((ctor_name) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st3) => parse_ctor_fields(ctor_name)(new List<object>())(st3)(name_tok)(acc)))(advance(st2))))(current(st2))))(skip_newlines(advance(st))) : TypeDefOk(new TypeDef(name: name_tok, type_params: new List<object>(), body: VariantBody(acc)))(st));

    public static ParseTypeDefResult parse_ctor_fields(Token ctor_name, List<TypeExpr> fields, ParseState st, Token name_tok, List<VariantCtorDef> acc) => (is_left_paren(current_kind(st)) ? ((Func<ParseTypeDefResult, ParseTypeDefResult>)((field_result) => unwrap_ctor_field(field_result)(ctor_name)(fields)(name_tok)(acc)))(parse_type(advance(st))) : ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((ctor) => parse_variant_ctors(name_tok)(Enumerable.Concat(acc, new List<object> { ctor }).ToList())(st2)))(new VariantCtorDef(name: ctor_name, fields: fields))))(skip_newlines(st)));

    public static ParseTypeDefResult unwrap_ctor_field(ParseTypeResult r, Token ctor_name, List<TypeExpr> fields, Token name_tok, List<VariantCtorDef> acc) => r switch { TypeOk(var ty, var st) => ((Func<ParseTypeDefResult, ParseTypeDefResult>)((st2) => parse_ctor_fields(ctor_name)(Enumerable.Concat(fields, new List<object> { ty }).ToList())(st2)(name_tok)(acc)))(expect(RightParen)(st)),  };

    public static Document parse_document(ParseState st) => ((Func<Document, Document>)((st2) => parse_top_level(new List<object>())(new List<object>())(st2)))(skip_newlines(st));
    ...
    public static Document try_top_level_type_def(List<Def> defs, List<TypeDef> type_defs, ParseState st) => ((Func<Document, Document>)((td_result) => td_result switch { TypeDefOk(var td, var st2) => parse_top_level(defs)(Enumerable.Concat(type_defs, new List<object> { td }).ToList())(skip_newlines(st2)), TypeDefNone(var st2) => try_top_level_def(defs)(type_defs)(st),  }))(parse_type_def(st));

    public static Document try_top_level_def(List<Def> defs, List<TypeDef> type_defs, ParseState st) => ((Func<Document, Document>)((def_result) => def_result switch { DefOk(var d, var st2) => parse_top_level(Enumerable.Concat(defs, new List<object> { d }).ToList())(type_defs)(skip_newlines(st2)), DefNone(var st2) => parse_top_level(defs)(type_defs)(skip_newlines(advance(st2))),  }))(parse_definition(st));
    ...
    public static CheckResult infer_name(UnificationState st, TypeEnv env, string name) => (env_has(env)(name) ? ((Func<CheckResult, CheckResult>)((raw) => ((Func<CheckResult, CheckResult>)((inst) => new CheckResult(inferred_type: inst.var_type, state: inst.state)))(instantiate_type(st)(raw))))(env_lookup(env)(name)) : new CheckResult(inferred_type: ErrorTy, state: add_unify_error(st)("CDX2002")(Enumerable.Concat("Unknown name: ", name).ToList())));

    public static FreshResult instantiate_type(UnificationState st, CodexType ty) => ty switch { ForAllTy(var var_id, var body) => ((Func<FreshResult, FreshResult>)((fr) => ((Func<FreshResult, FreshResult>)((substituted) => instantiate_type(fr.state)(substituted)))(subst_type_var(body)(var_id)(fr.var_type))))(fresh_and_advance(st)), _ => new FreshResult(var_type: ty, state: st),  };
    ...
    public static List<CodexType> map_subst_type_var(List<CodexType> args, long var_id, CodexType replacement, long i, long len, List<CodexType> acc) => ((i == len) ? acc : map_subst_type_var(args)(var_id)(replacement)((i + 1))(len)(Enumerable.Concat(acc, new List<CodexType> { subst_type_var(list_at(args)(i))(var_id)(replacement) }).ToList()));
    ...
    public static CheckResult infer_arithmetic(UnificationState st, CodexType lt, CodexType rt) => ((Func<CheckResult, CheckResult>)((r) => new CheckResult(inferred_type: lt, state: r.state)))(unify(st)(lt)(rt));

    public static CheckResult infer_comparison(UnificationState st, CodexType lt, CodexType rt) => ((Func<CheckResult, CheckResult>)((r) => new CheckResult(inferred_type: BooleanTy, state: r.state)))(unify(st)(lt)(rt));

    public static CheckResult infer_logical(UnificationState st, CodexType lt, CodexType rt) => ((Func<CheckResult, CheckResult>)((r1) => ((Func<CheckResult, CheckResult>)((r2) => new CheckResult(inferred_type: BooleanTy, state: r2.state)))(unify(r1.state)(rt)(BooleanTy))))(unify(st)(lt)(BooleanTy));

    public static CheckResult infer_append(UnificationState st, CodexType lt, CodexType rt) => ((Func<CheckResult, CheckResult>)((resolved) => resolved switch { TextTy { } => ((Func<CheckResult, CheckResult>)((r) => new CheckResult(inferred_type: TextTy, state: r.state)))(unify(st)(rt)(TextTy)), _ => ((Func<CheckResult, CheckResult>)((r) => new CheckResult(inferred_type: lt, state: r.state)))(unify(st)(lt)(rt)),  }))(resolve(st)(lt));

    public static CheckResult infer_cons(UnificationState st, CodexType lt, CodexType rt) => ((Func<CheckResult, CheckResult>)((list_ty) => ((Func<CheckResult, CheckResult>)((r) => new CheckResult(inferred_type: list_ty, state: r.state)))(unify(st)(rt)(list_ty))))(ListTy(lt));

    public static CheckResult infer_if(UnificationState st, TypeEnv env, AExpr cond, AExpr then_e, AExpr else_e) => ((Func<CheckResult, CheckResult>)((cr) => ((Func<CheckResult, CheckResult>)((r1) => ((Func<CheckResult, CheckResult>)((tr) => ((Func<CheckResult, CheckResult>)((er) => ((Func<CheckResult, CheckResult>)((r2) => new CheckResult(inferred_type: tr.inferred_type, state: r2.state)))(unify(er.state)(tr.inferred_type)(er.inferred_type))))(infer_expr(tr.state)(env)(else_e))))(infer_expr(r1.state)(env)(then_e))))(unify(cr.state)(cr.inferred_type)(BooleanTy))))(infer_expr(st)(env)(cond));

    public static CheckResult infer_let(UnificationState st, TypeEnv env, List<ALetBind> bindings, AExpr body) => ((Func<CheckResult, CheckResult>)((env2) => infer_expr(env2.state)(env2.env)(body)))(infer_let_bindings(st)(env)(bindings)(0)(list_length(bindings)));

    public static LetBindResult infer_let_bindings(UnificationState st, TypeEnv env, List<ALetBind> bindings, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((Func<LetBindResult, LetBindResult>)((b) => ((Func<LetBindResult, LetBindResult>)((vr) => ((Func<LetBindResult, LetBindResult>)((env2) => infer_let_bindings(vr.state)(env2)(bindings)((i + 1))(len)))(env_bind(env)(b.name.value)(vr.inferred_type))))(infer_expr(st)(env)(b.value))))(list_at(bindings)(i)));

    public static CheckResult infer_lambda(UnificationState st, TypeEnv env, List<Name> @params, AExpr body) => ((Func<CheckResult, CheckResult>)((pr) => ((Func<CheckResult, CheckResult>)((br) => ((Func<CheckResult, CheckResult>)((fun_ty) => new CheckResult(inferred_type: fun_ty, state: br.state)))(wrap_fun_type(pr.param_types)(br.inferred_type))))(infer_expr(pr.state)(pr.env)(body))))(bind_lambda_params(st)(env)(@params)(0)(list_length(@params))(new List<object>()));

    public static LambdaBindResult bind_lambda_params(UnificationState st, TypeEnv env, List<Name> @params, long i, long len, List<CodexType> acc) => ((i == len) ? new LambdaBindResult(state: st, env: env, param_types: acc) : ((Func<LambdaBindResult, LambdaBindResult>)((p) => ((Func<LambdaBindResult, LambdaBindResult>)((fr) => ((Func<LambdaBindResult, LambdaBindResult>)((env2) => bind_lambda_params(fr.state)(env2)(@params)((i + 1))(len)(Enumerable.Concat(acc, new List<object> { fr.var_type }).ToList())))(env_bind(env)(p.value)(fr.var_type))))(fresh_and_advance(st))))(list_at(@params)(i)));
    ...
    public static CheckResult infer_application(UnificationState st, TypeEnv env, AExpr func, AExpr arg) => ((Func<CheckResult, CheckResult>)((fr) => ((Func<CheckResult, CheckResult>)((ar) => ((Func<CheckResult, CheckResult>)((ret) => ((Func<CheckResult, CheckResult>)((r) => new CheckResult(inferred_type: ret.var_type, state: r.state)))(unify(ret.state)(fr.inferred_type)(FunTy(ar.inferred_type)(ret.var_type)))))(fresh_and_advance(ar.state))))(infer_expr(fr.state)(env)(arg))))(infer_expr(st)(env)(func));

    public static CheckResult infer_list(UnificationState st, TypeEnv env, List<AExpr> elems) => ((list_length(elems) == 0) ? ((Func<CheckResult, CheckResult>)((fr) => new CheckResult(inferred_type: ListTy(fr.var_type), state: fr.state)))(fresh_and_advance(st)) : ((Func<CheckResult, CheckResult>)((first) => ((Func<CheckResult, CheckResult>)((st2) => new CheckResult(inferred_type: ListTy(first.inferred_type), state: st2)))(unify_list_elems(first.state)(env)(elems)(first.inferred_type)(1)(list_length(elems)))))(infer_expr(st)(env)(list_at(elems)(0))));

    public static UnificationState unify_list_elems(UnificationState st, TypeEnv env, List<AExpr> elems, CodexType elem_ty, long i, long len) => ((i == len) ? st : ((Func<UnificationState, UnificationState>)((er) => ((Func<UnificationState, UnificationState>)((r) => unify_list_elems(r.state)(env)(elems)(elem_ty)((i + 1))(len)))(unify(er.state)(er.inferred_type)(elem_ty))))(infer_expr(st)(env)(list_at(elems)(i))));

    public static CheckResult infer_match(UnificationState st, TypeEnv env, AExpr scrutinee, List<AMatchArm> arms) => ((Func<CheckResult, CheckResult>)((sr) => ((Func<CheckResult, CheckResult>)((fr) => ((Func<CheckResult, CheckResult>)((st2) => new CheckResult(inferred_type: fr.var_type, state: st2)))(infer_match_arms(fr.state)(env)(sr.inferred_type)(fr.var_type)(arms)(0)(list_length(arms)))))(fresh_and_advance(sr.state))))(infer_expr(st)(env)(scrutinee));

    public static UnificationState infer_match_arms(UnificationState st, TypeEnv env, CodexType scrut_ty, CodexType result_ty, List<AMatchArm> arms, long i, long len) => ((i == len) ? st : ((Func<UnificationState, UnificationState>)((arm) => ((Func<UnificationState, UnificationState>)((pr) => ((Func<UnificationState, UnificationState>)((br) => ((Func<UnificationState, UnificationState>)((r) => infer_match_arms(r.state)(env)(scrut_ty)(result_ty)(arms)((i + 1))(len)))(unify(br.state)(br.inferred_type)(result_ty))))(infer_expr(pr.state)(pr.env)(arm.body))))(bind_pattern(st)(env)(arm.pattern)(scrut_ty))))(list_at(arms)(i)));

    public static PatBindResult bind_pattern(UnificationState st, TypeEnv env, APat pat, CodexType ty) => pat switch { AVarPat(var name) => new PatBindResult(state: st, env: env_bind(env)(name.value)(ty)), AWildPat { } => new PatBindResult(state: st, env: env), ALitPat(var val, var kind) => new PatBindResult(state: st, env: env), ACtorPat(var ctor_name, var sub_pats) => ((Func<PatBindResult, PatBindResult>)((ctor_lookup) => bind_ctor_sub_patterns(ctor_lookup.state)(env)(sub_pats)(ctor_lookup.var_type)(0)(list_length(sub_pats))))(instantiate_type(st)(env_lookup(env)(ctor_name.value))),  };

    public static PatBindResult bind_ctor_sub_patterns(UnificationState st, TypeEnv env, List<APat> sub_pats, CodexType ctor_ty, long i, long len) => ((i == len) ? new PatBindResult(state: st, env: env) : ctor_ty switch { FunTy(var param_ty, var ret_ty) => ((Func<PatBindResult, PatBindResult>)((pr) => bind_ctor_sub_patterns(pr.state)(pr.env)(sub_pats)(ret_ty)((i + 1))(len)))(bind_pattern(st)(env)(list_at(sub_pats)(i))(param_ty)), _ => ((Func<PatBindResult, PatBindResult>)((fr) => ((Func<PatBindResult, PatBindResult>)((pr) => bind_ctor_sub_patterns(pr.state)(pr.env)(sub_pats)(ctor_ty)((i + 1))(len)))(bind_pattern(fr.state)(env)(list_at(sub_pats)(i))(fr.var_type))))(fresh_and_advance(st)),  });
    ...
    public static CheckResult infer_do_loop(UnificationState st, TypeEnv env, List<ADoStmt> stmts, long i, long len, CodexType last_ty) => ((i == len) ? new CheckResult(inferred_type: last_ty, state: st) : ((Func<CheckResult, CheckResult>)((stmt) => stmt switch { ADoExprStmt(var e) => ((Func<CheckResult, CheckResult>)((er) => infer_do_loop(er.state)(env)(stmts)((i + 1))(len)(er.inferred_type)))(infer_expr(st)(env)(e)), ADoBindStmt(var name, var e) => ((Func<CheckResult, CheckResult>)((er) => ((Func<CheckResult, CheckResult>)((env2) => infer_do_loop(er.state)(env2)(stmts)((i + 1))(len)(er.inferred_type)))(env_bind(env)(name.value)(er.inferred_type))))(infer_expr(st)(env)(e)),  }))(list_at(stmts)(i)));

    public static CheckResult infer_expr(UnificationState st, TypeEnv env, AExpr expr) => expr switch { ALitExpr(var val, var kind) => infer_literal(st)(kind), ANameExpr(var name) => infer_name(st)(env)(name.value), ABinaryExpr(var left, var op, var right) => infer_binary(st)(env)(left)(op)(right), AUnaryExpr(var operand) => ((Func<CheckResult, CheckResult>)((r) => ((Func<CheckResult, CheckResult>)((u) => new CheckResult(inferred_type: IntegerTy, state: u.state)))(unify(r.state)(r.inferred_type)(IntegerTy))))(infer_expr(st)(env)(operand)), AApplyExpr(var func, var arg) => infer_application(st)(env)(func)(arg), AIfExpr(var cond, var then_e, var else_e) => infer_if(st)(env)(cond)(then_e)(else_e), ALetExpr(var bindings, var body) => infer_let(st)(env)(bindings)(body), ALambdaExpr(var @params, var body) => infer_lambda(st)(env)(@params)(body), AMatchExpr(var scrutinee, var arms) => infer_match(st)(env)(scrutinee)(arms), AListExpr(var elems) => infer_list(st)(env)(elems), ADoExpr(var stmts) => infer_do(st)(env)(stmts), AFieldAccess(var obj, var field) => ((Func<CheckResult, CheckResult>)((r) => ((Func<CheckResult, CheckResult>)((fr) => new CheckResult(inferred_type: fr.var_type, state: fr.state)))(fresh_and_advance(r.state))))(infer_expr(st)(env)(obj)), ARecordExpr(var name, var fields) => ((Func<CheckResult, CheckResult>)((st2) => new CheckResult(inferred_type: ConstructedTy(name)(new List<object>()), state: st2)))(infer_record_fields(st)(env)(fields)(0)(list_length(fields))), AErrorExpr(var msg) => new CheckResult(inferred_type: ErrorTy, state: st),  };

    public static UnificationState infer_record_fields(UnificationState st, TypeEnv env, List<AFieldExpr> fields, long i, long len) => ((i == len) ? st : ((Func<UnificationState, UnificationState>)((f) => ((Func<UnificationState, UnificationState>)((r) => infer_record_fields(r.state)(env)(fields)((i + 1))(len)))(infer_expr(st)(env)(f.value))))(list_at(fields)(i)));
    ...
    public static List<CodexType> resolve_type_expr_list(List<ATypeExpr> args, long i, long len, List<CodexType> acc) => ((i == len) ? acc : resolve_type_expr_list(args)((i + 1))(len)(Enumerable.Concat(acc, new List<CodexType> { resolve_type_expr(list_at(args)(i)) }).ToList()));
    ...
    public static CheckResult check_def(UnificationState st, TypeEnv env, ADef def) => ((Func<CheckResult, CheckResult>)((declared) => ((Func<CheckResult, CheckResult>)((env2) => ((Func<CheckResult, CheckResult>)((body_r) => ((Func<CheckResult, CheckResult>)((u) => new CheckResult(inferred_type: declared.expected_type, state: u.state)))(unify(body_r.state)(env2.remaining_type)(body_r.inferred_type))))(infer_expr(env2.state)(env2.env)(def.body))))(bind_def_params(declared.state)(declared.env)(def.@params)(declared.expected_type)(0)(list_length(def.@params)))))(resolve_declared_type(st)(env)(def));

    public static DefSetup resolve_declared_type(UnificationState st, TypeEnv env, ADef def) => ((list_length(def.declared_type) == 0) ? ((Func<DefSetup, DefSetup>)((fr) => new DefSetup(expected_type: fr.var_type, remaining_type: fr.var_type, state: fr.state, env: env)))(fresh_and_advance(st)) : ((Func<DefSetup, DefSetup>)((ty) => new DefSetup(expected_type: ty, remaining_type: ty, state: st, env: env)))(resolve_type_expr(list_at(def.declared_type)(0))));

    public static DefParamResult bind_def_params(UnificationState st, TypeEnv env, List<AParam> @params, CodexType remaining, long i, long len) => ((i == len) ? new DefParamResult(state: st, env: env, remaining_type: remaining) : ((Func<DefParamResult, DefParamResult>)((p) => remaining switch { FunTy(var param_ty, var ret_ty) => ((Func<DefParamResult, DefParamResult>)((env2) => bind_def_params(st)(env2)(@params)(ret_ty)((i + 1))(len)))(env_bind(env)(p.name.value)(param_ty)), _ => ((Func<DefParamResult, DefParamResult>)((fr) => ((Func<DefParamResult, DefParamResult>)((env2) => bind_def_params(fr.state)(env2)(@params)(remaining)((i + 1))(len)))(env_bind(env)(p.name.value)(fr.var_type))))(fresh_and_advance(st)),  }))(list_at(@params)(i)));

    public static ModuleResult check_module(AModule mod) => ((Func<ModuleResult, ModuleResult>)((tenv) => ((Func<ModuleResult, ModuleResult>)((env) => check_all_defs(env.state)(env.env)(mod.defs)(0)(list_length(mod.defs))(new List<object>())))(register_all_defs(tenv.state)(tenv.env)(mod.defs)(0)(list_length(mod.defs)))))(register_type_defs(empty_unification_state)(builtin_type_env)(mod.type_defs)(0)(list_length(mod.type_defs)));

    public static LetBindResult register_all_defs(UnificationState st, TypeEnv env, List<ADef> defs, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((Func<LetBindResult, LetBindResult>)((def) => ((Func<LetBindResult, LetBindResult>)((ty) => register_all_defs(ty.state)(ty.env)(defs)((i + 1))(len)))(((list_length(def.declared_type) == 0) ? ((Func<object, object>)((fr) => ((Func<object, object>)((env2) => new LetBindResult(state: fr.state, env: env2)))(env_bind(env)(def.name.value)(fr.var_type))))(fresh_and_advance(st)) : ((Func<object, object>)((resolved) => new LetBindResult(state: st, env: env_bind(env)(def.name.value)(resolved))))(resolve_type_expr(list_at(def.declared_type)(0)))))))(list_at(defs)(i)));

    public static ModuleResult check_all_defs(UnificationState st, TypeEnv env, List<ADef> defs, long i, long len, List<TypeBinding> acc) => ((i == len) ? new ModuleResult(types: acc, state: st) : ((Func<ModuleResult, ModuleResult>)((def) => ((Func<ModuleResult, ModuleResult>)((r) => ((Func<ModuleResult, ModuleResult>)((resolved) => ((Func<ModuleResult, ModuleResult>)((entry) => check_all_defs(r.state)(env)(defs)((i + 1))(len)(Enumerable.Concat(acc, new List<object> { entry }).ToList())))(new TypeBinding(name: def.name.value, bound_type: resolved))))(deep_resolve(r.state)(r.inferred_type))))(check_def(st)(env)(def))))(list_at(defs)(i)));

    public static LetBindResult register_type_defs(UnificationState st, TypeEnv env, List<ATypeDef> tdefs, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((Func<LetBindResult, LetBindResult>)((td) => ((Func<LetBindResult, LetBindResult>)((r) => register_type_defs(r.state)(r.env)(tdefs)((i + 1))(len)))(register_one_type_def(st)(env)(td))))(list_at(tdefs)(i)));

    public static LetBindResult register_one_type_def(UnificationState st, TypeEnv env, ATypeDef td) => td switch { AVariantTypeDef(var name, var type_params, var ctors) => ((Func<LetBindResult, LetBindResult>)((result_ty) => register_variant_ctors(st)(env)(ctors)(result_ty)(0)(list_length(ctors))))(ConstructedTy(name)(new List<object>())), ARecordTypeDef(var name, var type_params, var fields) => ((Func<LetBindResult, LetBindResult>)((result_ty) => ((Func<LetBindResult, LetBindResult>)((ctor_ty) => new LetBindResult(state: st, env: env_bind(env)(name.value)(ctor_ty))))(build_record_ctor_type(fields)(result_ty)(0)(list_length(fields)))))(ConstructedTy(name)(new List<object>())),  };

    public static LetBindResult register_variant_ctors(UnificationState st, TypeEnv env, List<AVariantCtorDef> ctors, CodexType result_ty, long i, long len) => ((i == len) ? new LetBindResult(state: st, env: env) : ((Func<LetBindResult, LetBindResult>)((ctor) => ((Func<LetBindResult, LetBindResult>)((ctor_ty) => ((Func<LetBindResult, LetBindResult>)((env2) => register_variant_ctors(st)(env2)(ctors)(result_ty)((i + 1))(len)))(env_bind(env)(ctor.name.value)(ctor_ty))))(build_ctor_type(ctor.fields)(result_ty)(0)(list_length(ctor.fields)))))(list_at(ctors)(i)));

    public static CodexType build_ctor_type(List<ATypeExpr> fields, CodexType result, long i, long len) => ((i == len) ? result : ((Func<CodexType, CodexType>)((rest) => FunTy(resolve_type_expr(list_at(fields)(i)))(rest)))(build_ctor_type(fields)(result)((i + 1))(len)));

    public static CodexType build_record_ctor_type(List<ARecordFieldDef> fields, CodexType result, long i, long len) => ((i == len) ? result : ((Func<CodexType, CodexType>)((f) => ((Func<CodexType, CodexType>)((rest) => FunTy(resolve_type_expr(f.type_expr))(rest)))(build_record_ctor_type(fields)(result)((i + 1))(len))))(list_at(fields)(i)));
    ...
    public static CodexType env_lookup_loop(List<TypeBinding> bindings, string name, long i, long len) => ((i == len) ? ErrorTy : ((Func<CodexType, CodexType>)((b) => ((b.name == name) ? b.bound_type : env_lookup_loop(bindings)(name)((i + 1))(len))))(list_at(bindings)(i)));

    public static bool env_has(TypeEnv env, string name) => env_has_loop(env.bindings)(name)(0)(list_length(env.bindings));

    public static bool env_has_loop(List<TypeBinding> bindings, string name, long i, long len) => ((i == len) ? false : ((Func<bool, bool>)((b) => ((b.name == name) ? true : env_has_loop(bindings)(name)((i + 1))(len))))(list_at(bindings)(i)));

    public static TypeEnv env_bind(TypeEnv env, string name, CodexType ty) => new TypeEnv(bindings: Enumerable.Concat(new List<object> { new TypeBinding(name: name, bound_type: ty) }, env.bindings).ToList());

    public static TypeEnv builtin_type_env() => ((Func<TypeEnv, TypeEnv>)((e) => ((Func<TypeEnv, TypeEnv>)((e2) => ((Func<TypeEnv, TypeEnv>)((e3) => ((Func<TypeEnv, TypeEnv>)((e4) => ((Func<TypeEnv, TypeEnv>)((e5) => ((Func<TypeEnv, TypeEnv>)((e6) => ((Func<TypeEnv, TypeEnv>)((e7) => ((Func<TypeEnv, TypeEnv>)((e8) => ((Func<TypeEnv, TypeEnv>)((e9) => ((Func<TypeEnv, TypeEnv>)((e10) => ((Func<TypeEnv, TypeEnv>)((e11) => ((Func<TypeEnv, TypeEnv>)((e12) => ((Func<TypeEnv, TypeEnv>)((e13) => ((Func<TypeEnv, TypeEnv>)((e14) => ((Func<TypeEnv, TypeEnv>)((e15) => ((Func<TypeEnv, TypeEnv>)((e16) => ((Func<TypeEnv, TypeEnv>)((e17) => ((Func<TypeEnv, TypeEnv>)((e18) => ((Func<TypeEnv, TypeEnv>)((e19) => ((Func<TypeEnv, TypeEnv>)((e20) => ((Func<TypeEnv, TypeEnv>)((e21) => e21))(env_bind(e20)("read-line")(TextTy))))(env_bind(e19)("fold")(ForAllTy(0)(ForAllTy(1)(FunTy(FunTy(TypeVar(1))(FunTy(TypeVar(0))(TypeVar(1))))(FunTy(TypeVar(1))(FunTy(ListTy(TypeVar(0)))(TypeVar(1))))))))))(env_bind(e18)("filter")(ForAllTy(0)(FunTy(FunTy(TypeVar(0))(BooleanTy))(FunTy(ListTy(TypeVar(0)))(ListTy(TypeVar(0)))))))))(env_bind(e17)("map")(ForAllTy(0)(ForAllTy(1)(FunTy(FunTy(TypeVar(0))(TypeVar(1)))(FunTy(ListTy(TypeVar(0)))(ListTy(TypeVar(1))))))))))(env_bind(e16)("list-at")(ForAllTy(0)(FunTy(ListTy(TypeVar(0)))(FunTy(IntegerTy)(TypeVar(0))))))))(env_bind(e15)("list-length")(ForAllTy(0)(FunTy(ListTy(TypeVar(0)))(IntegerTy))))))(env_bind(e14)("print-line")(FunTy(TextTy)(NothingTy)))))(env_bind(e13)("show")(ForAllTy(0)(FunTy(TypeVar(0))(TextTy))))))(env_bind(e12)("text-to-integer")(FunTy(TextTy)(IntegerTy)))))(env_bind(e11)("text-replace")(FunTy(TextTy)(FunTy(TextTy)(FunTy(TextTy)(TextTy)))))))(env_bind(e10)("code-to-char")(FunTy(IntegerTy)(TextTy)))))(env_bind(e9)("char-code")(FunTy(TextTy)(IntegerTy)))))(env_bind(e8)("is-whitespace")(FunTy(TextTy)(BooleanTy)))))(env_bind(e7)("is-digit")(FunTy(TextTy)(BooleanTy)))))(env_bind(e6)("is-letter")(FunTy(TextTy)(BooleanTy)))))(env_bind(e5)("substring")(FunTy(TextTy)(FunTy(IntegerTy)(FunTy(IntegerTy)(TextTy)))))))(env_bind(e4)("char-at")(FunTy(TextTy)(FunTy(IntegerTy)(TextTy))))))(env_bind(e3)("integer-to-text")(FunTy(IntegerTy)(TextTy)))))(env_bind(e2)("text-length")(FunTy(TextTy)(IntegerTy)))))(env_bind(e)("negate")(FunTy(IntegerTy)(IntegerTy)))))(empty_type_env);
    ...
    public static CodexType subst_lookup_loop(long var_id, List<SubstEntry> entries, long i, long len) => ((i == len) ? ErrorTy : ((Func<CodexType, CodexType>)((entry) => ((entry.var_id == var_id) ? entry.resolved_type : subst_lookup_loop(var_id)(entries)((i + 1))(len))))(list_at(entries)(i)));

    public static bool has_subst(long var_id, List<SubstEntry> entries) => has_subst_loop(var_id)(entries)(0)(list_length(entries));

    public static bool has_subst_loop(long var_id, List<SubstEntry> entries, long i, long len) => ((i == len) ? false : ((Func<bool, bool>)((entry) => ((entry.var_id == var_id) ? true : has_subst_loop(var_id)(entries)((i + 1))(len))))(list_at(entries)(i)));
    ...
    public static UnificationState add_subst(UnificationState st, long var_id, CodexType ty) => new UnificationState(substitutions: Enumerable.Concat(st.substitutions, new List<object> { new SubstEntry(var_id: var_id, resolved_type: ty) }).ToList(), next_id: st.next_id, errors: st.errors);

    public static UnificationState add_unify_error(UnificationState st, string code, string msg) => new UnificationState(substitutions: st.substitutions, next_id: st.next_id, errors: Enumerable.Concat(st.errors, new List<object> { make_error(code)(msg) }).ToList());

    public static bool occurs_in(UnificationState st, long var_id, CodexType ty) => ((Func<bool, bool>)((resolved) => resolved switch { TypeVar(var id) => (id == var_id), FunTy(var param, var ret) => (occurs_in(st)(var_id)(param) || occurs_in(st)(var_id)(ret)), ListTy(var elem) => occurs_in(st)(var_id)(elem), _ => false,  }))(resolve(st)(ty));

    public static UnifyResult unify(UnificationState st, CodexType a, CodexType b) => ((Func<UnifyResult, UnifyResult>)((ra) => ((Func<UnifyResult, UnifyResult>)((rb) => unify_resolved(st)(ra)(rb)))(resolve(st)(b))))(resolve(st)(a));
    ...
    public static CodexType deep_resolve(UnificationState st, CodexType ty) => ((Func<CodexType, CodexType>)((resolved) => resolved switch { FunTy(var param, var ret) => FunTy(deep_resolve(st)(param))(deep_resolve(st)(ret)), ListTy(var elem) => ListTy(deep_resolve(st)(elem)), ConstructedTy(var name, var args) => ConstructedTy(name)(deep_resolve_list(st)(args)(0)(list_length(args))(new List<object>())), ForAllTy(var id, var body) => ForAllTy(id)(deep_resolve(st)(body)), SumTy(var name, var ctors) => resolved, RecordTy(var name, var fields) => resolved, _ => resolved,  }))(resolve(st)(ty));

    public static List<CodexType> deep_resolve_list(UnificationState st, List<CodexType> args, long i, long len, List<CodexType> acc) => ((i == len) ? acc : deep_resolve_list(st)(args)((i + 1))(len)(Enumerable.Concat(acc, new List<CodexType> { deep_resolve(st)(list_at(args)(i)) }).ToList()));

    public static string compile(string source, string module_name) => ((Func<string, string>)((tokens) => ((Func<string, string>)((st) => ((Func<string, string>)((doc) => ((Func<string, string>)((ast) => ((Func<string, string>)((check_result) => ((Func<string, string>)((ir) => emit_full_module(ir)(ast.type_defs)))(lower_module(ast)(check_result.types)(check_result.state))))(check_module(ast))))(desugar_document(doc)(module_name))))(parse_document(st))))(make_parse_state(tokens))))(tokenize(source));

    public static CompileResult compile_checked(string source, string module_name) => ((Func<CompileResult, CompileResult>)((tokens) => ((Func<CompileResult, CompileResult>)((st) => ((Func<CompileResult, CompileResult>)((doc) => ((Func<CompileResult, CompileResult>)((ast) => ((Func<CompileResult, CompileResult>)((resolve_result) => ((list_length(resolve_result.errors) > 0) ? CompileError(resolve_result.errors) : ((Func<CompileResult, CompileResult>)((check_result) => ((Func<CompileResult, CompileResult>)((ir) => CompileOk(emit_full_module(ir)(ast.type_defs))(check_result)))(lower_module(ast)(check_result.types)(check_result.state))))(check_module(ast)))))(resolve_module(ast))))(desugar_document(doc)(module_name))))(parse_document(st))))(make_parse_state(tokens))))(tokenize(source));
